...
TYPE CLONE
REAL(KIND=JPRD) ,POINTER :: COMMSBUF(:) ! for communicating packed bufferfly_structs
...
TYPE(CLONE),INTENT(OUT) :: YD_CLONE            ! for communicating packed bufferfly_structs
...
TYPE(CLONE),OPTIONAL,INTENT(IN) :: YD_CLONE          ! for communicating packed bufferfly_structs
TYPE(SHAREDMEM),OPTIONAL,INTENT(INOUT) :: YDMEMBUF   ! Memory buffer
INTEGER(KIND=JPIM) :: ILEN,I,JL,JIK,JIJ,J,J1,J2
REAL(KIND=JPRD),POINTER :: ZBUF(:)
LOGICAL :: LLMEMBUF
!--------------------------------------------------------------------------------
...
ENDIF
YD_STRUCT%M_ORDER     = ZBUF(1)
YD_STRUCT%N_ORDER     = ZBUF(2)
YD_STRUCT%N_CMAX      = ZBUF(3)
YD_STRUCT%N_LEVELS    = ZBUF(4)
YD_STRUCT%IBETALEN_MAX = ZBUF(5)
I=I+5
...
DO JL=0,YD_STRUCT%N_LEVELS
  ...
  ENDIF
  YD_STRUCT%SLEV(JL)%IJ      =ZBUF(1)
  YD_STRUCT%SLEV(JL)%IK      =ZBUF(2)
  YD_STRUCT%SLEV(JL)%IBETALEN=ZBUF(3)
  I=I+3
  ALLOCATE(YD_STRUCT%SLEV(JL)%NODE(YD_STRUCT%SLEV(JL)%IJ,YD_STRUCT%SLEV(JL)%IK))
  DO JIK=1,YD_STRUCT%SLEV(JL)%IK
    DO JIJ=1,YD_STRUCT%SLEV(JL)%IJ
      ...
      ENDIF
      YD_STRUCT%SLEV(JL)%NODE(JIJ,JIK)%ILEV    = ZBUF(1)
      YD_STRUCT%SLEV(JL)%NODE(JIJ,JIK)%IFCOL   = ZBUF(2)
      YD_STRUCT%SLEV(JL)%NODE(JIJ,JIK)%ILCOL   = ZBUF(3)
      YD_STRUCT%SLEV(JL)%NODE(JIJ,JIK)%IFROW   = ZBUF(4)
      YD_STRUCT%SLEV(JL)%NODE(JIJ,JIK)%ILROW   = ZBUF(5)
      YD_STRUCT%SLEV(JL)%NODE(JIJ,JIK)%ICOLS   = ZBUF(6)
      YD_STRUCT%SLEV(JL)%NODE(JIJ,JIK)%IROWS   = ZBUF(7)
      YD_STRUCT%SLEV(JL)%NODE(JIJ,JIK)%IRANK   = ZBUF(8)
      YD_STRUCT%SLEV(JL)%NODE(JIJ,JIK)%IOFFBETA= ZBUF(9)
      J = ZBUF(10)
      I=I+10
      ...
      IF( J > 0 )THEN
        ...
        ENDIF
        YD_STRUCT%SLEV(JL)%NODE(JIJ,JIK)%ICLIST(:)=ZBUF(1:J)
        I=I+J
      ENDIF
      IF(LLMEMBUF) THEN
      ...
      DO JK=1,YD_STRUCT%SLEV(JL)%IK
        ...
        IF(JL == 0) THEN
          ...
          DO JF=1,KF
            ...
            ENDDO
          ENDDO
          IF(YNODE%ICOLS > IRANK .AND. IRANK > 0) THEN
            IF (LLDOUBLE) THEN
        ...
        ELSE
          ...
          ENDIF
        ENDIF
        IF(JL == ILEVS .AND. YNODE%IRANK > 0) THEN
          IFR = YNODE%IFROW
          ILR = YNODE%ILROW
          IROWS = YNODE%IROWS
          IF (LLDOUBLE) THEN 
...
IF(YDNODE%ICOLS > IRANK) THEN
  ...
  ELSE
    CALL SGEMM('N','N',IRANK,KF,IN,1.0_JPRB,&
     & YDNODE%PNONIM(1),IRANK,ZVECIN(IRANK+1,1),YDNODE%ICOLS,1.0_JPRB,&
     & PVECOUT,IRANK)
!     PVECOUT = PVECOUT + matmul(YDNODE%PNONIM(1),ZVECIN(IRANK+1,1))
  ENDIF
ENDIF
...
IF(IN>0) THEN
  ...
  ELSE
    CALL SGEMM('T','N',IN,KF,IM,1.0_JPRB,&
     & YDNODE%PNONIM(1),IM,PVECIN,IM,0.0_JPRB,&
     & ZVECOUT(YDNODE%IRANK+1,1),YDNODE%ICOLS)
!     ZVECOUT(YDNODE%IRANK+1,1) = matmul(transpose(YDNODE%PNONIM(1)),PVECIN)
  ENDIF
ENDIF
DO JF=1,KF
