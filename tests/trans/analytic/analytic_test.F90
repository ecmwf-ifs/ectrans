...
USE ANALYTIC_UTILS, ONLY: ANALYTIC_GENERATOR, COMPUTE_MAX_ERROR
...
CLASS(ANALYTIC_GENERATOR), ALLOCATABLE :: ANALYGEN
...
ANALYGEN = ANALYTIC_GENERATOR(NPROMA, NGPBLKS, NDGL, N_REGIONS_NS, N_REGIONS_EW, NLOEN)
...
DO M = 0, NSMAX
  ...
  CALL ANALYGEN%PREPARE_LEGENDRE_POLYNOMIALS(M, NSMAX)

  DO N = M, NSMAX
    CALL INITIALIZE_SPECTRAL_ARRAY(NSMAX, M, N, ZSPSCALAR)

    ! Compute analytic solutions
    ZSPH_ANALYTIC = ANALYGEN%COMPUTE_ANALYTIC_SOLUTION(NPROMA, NGPBLKS, NGPTOT, M, N)
