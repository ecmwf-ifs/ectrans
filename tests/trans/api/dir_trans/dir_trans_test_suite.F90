...
! Setup fixture
SUBROUTINE SETUP_TEST(KSPEC2, KGPTOTG, KGPTOT, KGPBLKS, KMY_PROC)
...
INTEGER FUNCTION ECTRANS_TEST_TRANS_API_DIR_TRANS_CALL_MODE_1_SCALAR_1() RESULT(KRET) BIND(C)
  ...
  INTEGER(KIND=JPIM) :: ISPEC2, IGPTOTG, IGPTOT, IMY_PROC, IGPBLKS

  ! Set up everything
  CALL SETUP_TEST(ISPEC2, IGPTOTG, IGPTOT, IGPBLKS, IMY_PROC)
...
INTEGER FUNCTION ECTRANS_TEST_TRANS_API_DIR_TRANS_CALL_MODE_1_WIND_1() RESULT(KRET) BIND(C)
  ...
  INTEGER(KIND=JPIM) :: ISPEC2, IGPTOTG, IGPTOT, IMY_PROC, IGPBLKS
  REAL(KIND=JPRB) :: ZTEMP_TOLERANCE

  ! Set up everything
  CALL SETUP_TEST(ISPEC2, IGPTOTG, IGPTOT, IGPBLKS, IMY_PROC)
  ZGP = GET_INPUT_FIELD(IMY_PROC, IGPTOTG, IGPBLKS, 2)
  ...
  IF (JPRB == JPRD) THEN
    ZTEMP_TOLERANCE = 1E7_JPRD * PPTOLERANCE
  ELSE
    ZTEMP_TOLERANCE = PPTOLERANCE
  END IF
  KRET = MERGE(0, 1, ALL(APPROX_EQ(ZSPVOR, 0.0_JPRB, PTOL=ZTEMP_TOLERANCE)) .AND. &
    & ALL(APPROX_EQ(ZSPDIV, 0.0_JPRB, PTOL=ZTEMP_TOLERANCE)))
...
INTEGER FUNCTION ECTRANS_TEST_TRANS_API_DIR_TRANS_CALL_MODE_1_WIND_1_SCALAR_1() RESULT(KRET) BIND(C)
  ...
  ! Set up everything
  CALL SETUP_TEST(ISPEC2, IGPTOTG, IGPTOT, IGPBLKS, IMY_PROC)
  ZGP = GET_INPUT_FIELD(IMY_PROC, IGPTOTG, IGPBLKS, 3)
  ALLOCATE(ZSPVOR(1,ISPEC2))
  ALLOCATE(ZSPDIV(1,ISPEC2))
  ALLOCATE(ZSPSCALAR(1,ISPEC2))
  ...
  END IF
  IF (HAVE_M0_MODE()) THEN
    IF (ALL(APPROX_EQ(ZSPVOR, 0.0_JPRB, PTOL=ZTEMP_TOLERANCE)) .AND. &
      & ALL(APPROX_EQ(ZSPDIV, 0.0_JPRB, PTOL=ZTEMP_TOLERANCE)) .AND. &
...
INTEGER FUNCTION ECTRANS_TEST_TRANS_API_DIR_TRANS_CALL_MODE_2_WIND_1() RESULT(KRET) BIND(C)
  ...
  ZGPUV(:,1,:,:) = GET_INPUT_FIELD(IMY_PROC, IGPTOTG, IGPBLKS, 2)
  ALLOCATE(ZSPVOR(1,ISPEC2))
  ALLOCATE(ZSPDIV(1,ISPEC2))
