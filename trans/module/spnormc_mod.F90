...
IF (NPROC > 1.AND.MYPROC /= KMASTER) THEN
  CALL MPE_SEND(PSM,ISTOTAL,MREALT,NPRCIDS(KMASTER),ITAG,0,0,0,IERR)
  IF (IERR < 0) CALL ABOR1(' SPNORMC : ERROR IN MPE_SEND')
ENDIF
...
IF (MYPROC == KMASTER) THEN
  DO JROC=1,NPROC
    IF (JROC == KMASTER) THEN
      ...
      IMSGLEN = ISTOTAL
    ELSE
      CALL MPE_RECV(ZRECVBUF,IBUFLENR,MREALT,-1,ITAG,0,0,0,&
       &IMSGLEN,IRECVID,ITAGR,IERR)
      IF (IERR < 0) CALL ABOR1(' SPNORMC : ERROR IN MPE_RECV')
    ENDIF
...
IF( NPROC > 1 )THEN
  ...
  IF( IERR /= 0 )THEN
    CALL ABOR1('SPNORMC: ERROR IN MPE_BARRIER')
  ENDIF
ENDIF
!     ------------------------------------------------------------------
