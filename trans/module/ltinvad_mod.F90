MODULE LTINVAD_MOD
CONTAINS
SUBROUTINE LTINVAD(KM,KMLOC,PSPVOR,PSPDIV,PSPSCALAR,FSPGL_PROC)
...
REAL_B :: ZSOA1(2*NF_OUT_LT,R%NLEI3),ZAOA1(2*NF_OUT_LT,R%NLEI3)
...
CALL ASRE1AD(KM,KMLOC,ZAOA1,ZSOA1)
...
CALL LEINVAD(KM,IFC,ZIA(:,ISTA:ISTA+IFC-1),ZAOA1,ZSOA1,ZLEPO)
...
IF (LUV) THEN
  ...
  IVL   = 6*NF_UV+1
  IVU   = 8*NF_UV
  CALL VDTUVAD(KM,NF_UV,ZEPSNM,ZIA(:,IVORL:IVORU),ZIA(:,IDIVL:IDIVU),&
...
IF (NF_SCDERS > 0) THEN
  ...
  CALL SPNSDEAD(KM,ZEPSNM,ZIA(:,ISL:ISU),ZIA(:,IDL:IDU))
ENDIF
...
CALL PRFI1AD(KM,ZIA,PSPVOR,PSPDIV,PSPSCALAR)
