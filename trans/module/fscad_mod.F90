MODULE FSCAD_MOD
CONTAINS
SUBROUTINE FSCAD(PUV,PSCALAR,PNSDERS,PEWDERS,PUVDERS)

!**** *FSCAD - Division by a*cos(theta), east-west derivatives - adjoint

!     Purpose.
!     --------
!        In Fourier space divide u and v and all north-south
!        derivatives by a*cos(theta). Also compute east-west derivatives
!        of u,v,thermodynamic, passiv scalar variables and surface
!        pressure.

!**   Interface.
!     ----------
!        CALL FSCAD(..)
!        Explicit arguments :  PUV     - u and v
!        --------------------  PSCALAR - scalar valued varaibles
!                              PNSDERS - N-S derivative of S.V.V.
!                              PEWDERS - E-W derivative of S.V.V.
!                              PUVDERS - E-W derivative of u and v
!     Method.
!     -------

!     Externals.   None.
!     ----------

!     Author.
!     -------
!        Mats Hamrud *ECMWF*

!     Modifications.
!     --------------
!        Original : 00-03-03 (From SC2FSC)

!     ------------------------------------------------------------------

#include "tsmbkind.h"

USE TPM_DISTR
USE TPM_FIELDS
USE TPM_TRANS
USE TPM_GEOMETRY

IMPLICIT NONE
 
REAL_B , INTENT(INOUT) :: PUV(:,:)
REAL_B , INTENT(INOUT) :: PSCALAR(:,:)
REAL_B , INTENT(INOUT) :: PNSDERS(:,:)
REAL_B , INTENT(INOUT) :: PEWDERS(:,:)
REAL_B , INTENT(INOUT) :: PUVDERS(:,:)

REAL_B :: ZACHTE(D%NDGL_FS)
INTEGER_M :: IMEN(D%NDGL_FS),ISTAGTF(D%NDGL_FS)


INTEGER_M :: JGL,JLON,JF,IGLG,II,IR,JM

!     ------------------------------------------------------------------

DO JGL=1,D%NDGL_FS
  IGLG = D%NPTRLS(MYSETW)+JGL-1
  ZACHTE(JGL)  = F%RACTHE(IGLG)
  IMEN(JGL)    = G%NMEN(IGLG)
  ISTAGTF(JGL) = D%NSTAGTF(JGL)
ENDDO

!$OMP PARALLEL DO SCHEDULE(STATIC,1) PRIVATE(JGL,JLON,JF,JM,II,IR)
DO JGL=1,D%NDGL_FS

!     ------------------------------------------------------------------

!*       2.    EAST-WEST DERIVATIVES
!              ---------------------

!*       2.1      U AND V.

  IF(LUVDER)THEN
    DO JM=0,IMEN(JGL)
      IR = ISTAGTF(JGL)+2*JM+1
      II = IR+1
      DO JF=1,2*NF_UV
          PUV(JF,II) = PUV(JF,II) - JM*PUVDERS(JF,IR)*ZACHTE(JGL)
          PUV(JF,IR) = PUV(JF,IR) + JM*PUVDERS(JF,II)*ZACHTE(JGL)
          PUVDERS(JF,IR) = _ZERO_
          PUVDERS(JF,II) = _ZERO_
      ENDDO
    ENDDO
  ENDIF

!*       2.2     SCALAR VARIABLES

  IF(NF_SCDERS > 0)THEN
    DO JM=0,IMEN(JGL)
      IR = ISTAGTF(JGL)+2*JM+1
      II = IR+1
      DO JF=1,NF_SCALARS
          PSCALAR(JF,II) = PSCALAR(JF,II) - JM*PEWDERS(JF,IR)*ZACHTE(JGL)
          PSCALAR(JF,IR) = PSCALAR(JF,IR) + JM*PEWDERS(JF,II)*ZACHTE(JGL)
          PEWDERS(JF,IR) = _ZERO_
          PEWDERS(JF,II) = _ZERO_
      ENDDO
    ENDDO
  ENDIF

!     ------------------------------------------------------------------

!*       1.    DIVIDE U V AND N-S DERIVATIVES BY A*COS(THETA)
!              ----------------------------------------------

  
!*       1.1      U AND V.

  IF(NF_UV > 0) THEN
    DO JLON=ISTAGTF(JGL)+1,ISTAGTF(JGL)+2*(IMEN(JGL)+1)
      DO JF=1,2*NF_UV
        PUV(JF,JLON) = PUV(JF,JLON)*ZACHTE(JGL)
      ENDDO
    ENDDO
  ENDIF

!*      1.2      N-S DERIVATIVES

  IF(NF_SCDERS > 0)THEN
    DO JLON=ISTAGTF(JGL)+1,ISTAGTF(JGL)+2*(IMEN(JGL)+1)
      DO JF=1,NF_SCALARS
        PNSDERS(JF,JLON) = PNSDERS(JF,JLON)*ZACHTE(JGL)
      ENDDO
    ENDDO
  ENDIF


ENDDO
!$OMP END PARALLEL DO

!     ------------------------------------------------------------------

END SUBROUTINE FSCAD
END MODULE FSCAD_MOD
