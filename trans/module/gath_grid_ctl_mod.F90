...
INTEGER(KIND=JPIM) :: ISENDREQ(KFGATHG),IOUNT,ITO
LOGICAL :: LLSAME
!     ------------------------------------------------------------------
...
! test if values in KTO are all the same
  ...
  IF(IMYFIELDS > 0) ALLOCATE(ZBUF(D%NGPTOTMX,IMYFIELDS,NPROC))
  IFLDR = 0
  ...
  IF( LLSAME )THEN
    ...
    CALL MPL_SEND(ZFLD(1:D%NGPTOTMX,1:KFGATHG),KDEST=NPRCIDS(ISND),KTAG=ITAG,&
  ...
  IF( LLSAME )THEN
    ...
    IF(KTO(1) == MYPROC) THEN
      ...
      DO JROC=1,NPROC
        ...
        IRCV  = JROC
        ILENB = D%NGPTOTMX
        CALL MPL_RECV(ZBUF(1:ILENB,1:KFGATHG,IRCV),KSOURCE=NPRCIDS(IRCV),&
    ...
    ! RECIEVE
      IF(KTO(JFLD) == MYPROC) THEN
        ...
        DO JROC=1,NPROC
          ...
          IRCV  = JROC
          ILENB = D%NGPTOTMX
          CALL MPL_RECV(ZBUF(1:ILENB,IFLDR,IRCV),KSOURCE=NPRCIDS(IRCV),&
           &KMP_TYPE=JP_BLOCKING_STANDARD,KOUNT=ILRECV,&
           &KTAG=ITAG,CDSTRING='GATH_GRID_CTL:')
        ENDDO
      ENDIF
  ...
  IF( LLSAME )THEN
    CALL MPL_WAIT(ZDUM,KREQUEST=ISENDREQ(1), &
