...
REAL(KIND=JPRB)   ,INTENT(OUT) :: DDPOL(0:KNSMAX)

INTEGER(KIND=JPIM),INTENT(IN), OPTIONAL :: KCHEAP

REAL(KIND=JPRB) :: DLX,DLX1,DLSITA,DLSITA2,DL1SITA,DLK,DL1, DLKM1, DLKM2
...
REAL(KIND=JPRB) :: ZFAC, ZLSITA, ZFAC0, ZFAC1, ZMULT, ZEPS

INTEGER(KIND=JPIM) :: JCORR, ICORR3, ICORR(KNSMAX)
REAL(KIND=JPRB) :: ZSCALE, ZISCALE
...
IF( ABS(REAL(DLSITA,JPRB)) <= ZEPS ) THEN
  DLX=1._JPRB
  DLSITA=0._JPRB
  DL1SITA=0._JPRB
  DLSITA2=0._JPRB
...
!  ZSCALE=1._JPRB/ZEPS
  ZSCALE=1.0E+100_JPRB
  ZISCALE=1.0E-100_JPRB
...
!  WRITE(*,*) 'SUPOLF: ZSCALE=',ZSCALE,' ICORR3=',ICORR3,' KM=',KM,' ZLSITA=',ZLSITA
  ...
  ZFAC=1._JPRB
  DO JN=1,KM-1
    ZFAC=ZFAC*SQRT(REAL(2*JN-1,JPRB))
    ZFAC=ZFAC/SQRT(REAL(2*JN,JPRB))
  ENDDO
  ZFAC=ZFAC*SQRT(REAL(2*KM-1,JPRB))
!  WRITE(*,*) 'SUPOLF: ZSCALE=',ZSCALE,' ICORR3=',ICORR3,' ZFAC=',ZFAC
  ...
  DO IC=0,MIN(KNSMAX-KM,3)
    ...
    ! (2m+i)!
    ZFAC0 = ZFAC0 * REAL(2*KM+IC,JPRB)   

    SELECT CASE (IC)
    CASE (0)
      ZMULT=ZFAC
    CASE (1)
      ZFAC=ZFAC*REAL(2*KM+IC,JPRB)
      ZMULT=ZFAC*DLX
    CASE (2)
      ZMULT=0.5_JPRB*ZFAC*(REAL(2*KM+3,JPRB)*DLX*DLX-1._JPRB)
    CASE (3)
      ZFAC=ZFAC*REAL(2*KM+IC,JPRB)
      ZMULT=(1._JPRB/6._JPRB)*DLX*ZFAC*(REAL(2*KM+5,JPRB)*DLX*DLX-3._JPRB)
    END SELECT

    DDPOL(KM+IC) = ZLSITA*ZMULT*SQRT(2._JPRB*(REAL(KM+IC,JPRB)+0.5_JPRB)*ZFAC1/ZFAC0)

    ZFAC1=ZFAC1*REAL(IC+1,JPRB)
