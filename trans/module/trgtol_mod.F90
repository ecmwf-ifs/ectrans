...
USE MPL_MODULE  ,ONLY : MPL_RECV, MPL_SEND, MPL_WAIT, JP_NON_BLOCKING_STANDARD, MPL_WAITANY, &
     & JP_BLOCKING_STANDARD, MPL_BARRIER
...
USE YOMCT0          ,ONLY : LTRANS_WAITANY
!
...
IF (LTRANS_WAITANY) THEN
   ...
   DO INR=1,KNRECV
      ...
      CALL MPL_RECV(ZCOMBUFR(-1:KRECVTOT(IRECV),INR), &
           ...
           & KTAG=ITAG,CDSTRING='TRLTOG: NON-BLOCKING IRECV' )
   ENDDO
ENDIF
...
DO JNR=1,KNRECV
  ...
  ELSE
     ...
     CALL MPL_RECV(ZCOMBUFR(-1:KRECVTOT(IRECV),INR), &
          ...
          & KTAG=ITAG,CDSTRING='TRLTOG: BLOCKING RECV' )
  ENDIF
...
IF(KNSEND > 0) THEN
  CALL MPL_WAIT(KREQUEST=IREQ_SEND(1:KNSEND),CDSTRING='TRGTOL_COMM: WAIT FOR SENDS')
ENDIF

IF (.not.LTRANS_WAITANY) CALL MPL_BARRIER(CDSTRING='TRGTOL_COMM: BARRIER AT END')
