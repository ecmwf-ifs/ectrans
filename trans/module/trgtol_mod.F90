...
INTEGER_M :: JPOS(NGPBLKS),JFLDA(KF_GP),JJ,JI

!     ------------------------------------------------------------------

!*       0.    Some initializations
!              --------------------
IF (.NOT.LSYNCSTATS) CALL GSTATS(1852,0)
...
ENDIF
IF (.NOT.LSYNCSTATS) CALL GSTATS(1852,1)
...
!....Send loop.........................................................
  ...
  IF (.NOT.LSYNCSTATS) CALL GSTATS(691,0)
  DO INS=1,INSEND
    ISEND=JSEND(INS)
  ...
  ENDDO
  IF (.NOT.LSYNCSTATS) CALL GSTATS(691,1)

!  Receive loop.........................................................

  IF (.NOT.LSYNCSTATS) CALL GSTATS(692,0)
  DO INR=INRECV,1,-1
    IRECV=JRECV(INR)
  ...
  ENDIF
  IF (.NOT.LSYNCSTATS) CALL GSTATS(692,1)
...
! completed communication
  ...
  IF( NPROC > 1.AND.(D%LSPLIT.OR.NPRTRV > 1))THEN
    IF (.NOT.LSYNCSTATS) CALL GSTATS(731,0)
    CALL MPL_BARRIER(CDSTRING='TRGTOL:')
    IF (.NOT.LSYNCSTATS) CALL GSTATS(731,1)
  ENDIF
ENDIF
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,JBLK,IFIRST,ILAST,JK,JJ,JI)
DO JJ=ISEND_FLD_START(ISEND),ISEND_FLD_END
      ...
      DO JBLK=1,NGPBLKS
        ...
        IF(IFIRST > 0) THEN
          ...
          IF(LLINDER) THEN
            DO JK=IFIRST,ILAST
              JI=(JJ-ISEND_FLD_START(ISEND))*IPOS+JPOS(JBLK)+JK-IFIRST+1
              ZCOMBUFS(JI,INS) = PGCOL(JK,KPTRGP(JFLD),JBLK)
            ENDDO
          ELSE
            DO JK=IFIRST,ILAST
              JI=(JJ-ISEND_FLD_START(ISEND))*IPOS+JPOS(JBLK)+JK-IFIRST+1
              ZCOMBUFS(JI,INS) = PGCOL(JK,JFLD,JBLK)
            ENDDO
          ENDIF
        ENDIF
      ENDDO
