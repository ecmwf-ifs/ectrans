...
INTEGER(KIND=JPIM) :: IERR, IFIRST, IFIRSTLAT, IGL, IGLL, ILAST,&
...
INTEGER(KIND=JPIM) :: IJPOS(NGPBLKS),IFLDA(KF_GP),JJ,JI
...
! Copy local data
    ...
    IF(ISENDTOT(MYPROC)-ISENT(MYPROC) > 0 )THEN
      ...
      DO JBLK=1,NGPBLKS
        ...
        ENDIF
      ENDDO
      IF (.NOT.LSYNCSTATS) CALL GSTATS(1610,0)
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,JBLK,JK,IFLD,IPOS,IFIRST,ILAST)
      DO JBLK=1,NGPBLKS
        IFIRST = IGPTRSEND(1,JBLK,MYSETW)
        IF(IFIRST > 0) THEN
      ...
      ENDDO
!$OMP END PARALLEL DO
      IF (.NOT.LSYNCSTATS) CALL GSTATS(1610,1)
