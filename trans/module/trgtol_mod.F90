...
!$OMP END PARALLEL DO
  ...
  CALL MPL_SEND(ZCOMBUFS(-1:KSENDTOT(ISEND),INS),KDEST=NPRCIDS(ISEND), &
   ...
   & KTAG=ITAG,CDSTRING='TRGTOL_COMM:' )
...
DO JNR=1,KNRECV
  ...
  CALL MPL_WAITANY(KREQUEST=IREQ_RECV(1:KNRECV),KINDEX=INR,CDSTRING='TRGTOL_COMM: WAIT FOR ANY RECEIVES')
...
IF(KNSEND > 0) THEN
  CALL MPL_WAIT(ZDUM,KREQUEST=IREQ_SEND(1:KNSEND),CDSTRING='TRGTOL_COMM: WAIT FOR SENDS')
ENDIF
