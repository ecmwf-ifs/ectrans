...
IF (.NOT.LSYNCSTATS) CALL GSTATS(107,0)
...
IF (.NOT.LSYNCSTATS) CALL GSTATS(1639,0)
...
ENDDO
!$OMP END PARALLEL DO
IF (.NOT.LSYNCSTATS) CALL GSTATS(1639,1)
...
IF(KF_UV > 0 .OR. KF_SCDERS > 0) THEN
  ...
  NULLIFY(ZEWDERS)
ENDIF
IF (.NOT.LSYNCSTATS) CALL GSTATS(107,1)
...
IF (.NOT.LSYNCSTATS) CALL GSTATS(157,0)
CALL TRLTOG(ZGTF,KF_FS,KF_GP,KF_SCALARS_G,IVSET,KPTRGP,&
 &PGP,PGPUV,PGP3A,PGP3B,PGP2)
IF (.NOT.LSYNCSTATS) CALL GSTATS(157,1)
