MODULE PRFI1_MOD
CONTAINS
SUBROUTINE PRFI1(KM,PIA,PSPVOR,PSPDIV,PSPSCALAR)
...
IF(NF_UV > 0)THEN
  IVOR = 1
  IDIV = 2*NF_UV+1
  CALL PRFI1B(KM,PIA(:,IVOR:IDIV-1),PSPVOR,NF_UV)
  CALL PRFI1B(KM,PIA(:,IDIV:ILAST) ,PSPDIV,NF_UV)
  ILAST = ILAST+4*NF_UV
ENDIF
...
IF(NF_SCALARS > 0)THEN
  ...
  CALL PRFI1B(KM,PIA(:,IFIRST:ILAST),PSPSCALAR(:,:),NF_SCALARS)
ENDIF
