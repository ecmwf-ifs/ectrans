...
REAL(KIND=JPRB) :: PNM(R%NSPOLEG,D%NLEI3D)

REAL(KIND=JPRB), ALLOCATABLE :: ZCOMBUF(:)
REAL(KIND=JPRB), POINTER     :: ZPNM(:,:)
!     LOCAL INTEGER SCALARS
INTEGER(KIND=JPIM) :: IERR, IGLLOC, ILREC, IM, INENTR, IPOS, IRECD,&
...
IF( NPROC > 1 )THEN
  IF (.NOT.LSYNCSTATS) CALL GSTATS(808,0)
  CALL MPL_BARRIER(CDSTRING='SUTRLE:')
  IF (.NOT.LSYNCSTATS) CALL GSTATS(808,1)
ENDIF
...
IF (.NOT.LSYNCSTATS) CALL GSTATS(1803,0)
...
IF (.NOT.LSYNCSTATS) CALL GSTATS(1803,1)
