...
IF(.NOT.D%LGRIDONLY) THEN
  ...
  IF (G%LREDUCED_GRID) THEN
    ...
    IF( R%NDGL == 2*(R%NSMAX+1) ) THEN
      ...
      !ZSQM2(:) = 0.0_JPRB
      G%NMEN(1) = MIN(R%NSMAX,INT(REAL(G%NLOEN(1)-1,JPRB)/(2.0_JPRB+ZSQM2(1)))-1)
      DO JGL=2,R%NDGNH
        G%NMEN(JGL) = MIN(R%NSMAX,MAX(G%NMEN(JGL-1),&
         &INT(REAL(G%NLOEN(JGL)-1,JPRB)/(2.0_JPRB+ ZSQM2(JGL)))-1))
      ENDDO
      !       * SOUTHERN HEMISPHERE :
      G%NMEN(R%NDGL) = MIN(R%NSMAX,INT(REAL(G%NLOEN(R%NDGL)-1,JPRB)/(2.0_JPRB+ZSQM2(R%NDGL)))-1)
