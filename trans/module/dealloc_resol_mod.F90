...
ELSE
  ...
  NULLIFY(D%NFRSTLAT,D%NLSTLAT,D%NPTRLAT,D%NPTRFRSTLAT,D%NPTRLSTLAT)
  DEALLOCATE(D%LSPLITLAT,D%NSTA,D%NONL,D%NGPTOTL,D%NPROCA_GP)
  NULLIFY(D%LSPLITLAT,D%NSTA,D%NONL,D%NGPTOTL,D%NPROCA_GP)
  ...
  IF(.NOT.D%LGRIDONLY) THEN
    ...
    NULLIFY(D%MYMS,D%NUMPP,D%NPOSSP,D%NPROCM,D%NDIM0G,D%NASM0,D%NATM0)
    DEALLOCATE(D%NLATLS,D%NLATLE,D%NPMT,D%NPMS,D%NPMG,D%NULTPP,D%NPROCL)
    NULLIFY(D%NLATLS,D%NLATLE,D%NPMT,D%NPMS,D%NPMG,D%NULTPP,D%NPROCL)
    DEALLOCATE(D%NPTRLS,D%NALLMS,D%NPTRMS,D%NSTAGT0B,D%NSTAGT1B,D%NPNTGTB0)
    NULLIFY(D%NPTRLS,D%NALLMS,D%NPTRMS,D%NSTAGT0B,D%NSTAGT1B,D%NPNTGTB0)
    DEALLOCATE(D%NPNTGTB1,D%NLTSFTB,D%NLTSGTB,D%MSTABF)
    NULLIFY(D%NPNTGTB1,D%NLTSFTB,D%NLTSGTB,D%MSTABF)
    ...
    IF (.NOT.D%LCPNMONLY) THEN
      ...
      NULLIFY(T%TRIGS,T%NFAX)
    ENDIF

    !TPM_FIELDS
    DEALLOCATE(F%RMU,F%RW,F%R1MU2,F%RACTHE)
    NULLIFY(F%RMU,F%RW,F%R1MU2,F%RACTHE)
    DEALLOCATE(F%REPSNM,F%RN,F%RLAPIN,F%NLTN)
    NULLIFY(F%REPSNM,F%RN,F%RLAPIN,F%NLTN)
    IF( S%LKEEPRPNM ) THEN
      DEALLOCATE(F%RPNM)
      NULLIFY(F%RPNM)
    ENDIF
