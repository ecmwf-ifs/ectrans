...
!     Purpose.
!     --------
!           Initialize nsta and D%NONL in yommp.
  ...
  DO JNPTSRE=1,IPTSRE
    ...
    DO JGL=1,ILEN
      IF (IXPTLAT(JGL)  <=  ILSTPTLAT(JGL)) THEN
        ZLAT1 = REAL(IXPTLAT(JGL)-1,JPRB)/&
         &REAL(G%NLOEN(D%NFRSTLAT(MYSETNS)+JGL-1),JPRB)
        IF (MYSETNS <= D%NAPSETS .AND.&
         &(IPART /= 2.OR.LLALLAT)) THEN
          ZLAT=MIN(ZLAT1,ZLAT)
          INXLAT=JGL
          IPART=1
          EXIT
        ELSEIF (MYSETNS > NPRGPNS-D%NAPSETS .AND.&
         &(IPART /= 1.OR.LLALLAT)) THEN
          ZLAT=MIN(ZLAT1,ZLAT)
          INXLAT=JGL
          IPART=2
          EXIT
        ELSEIF (ZLAT1 < ZLAT) THEN
          ZLAT = ZLAT1
          INXLAT = JGL
        ENDIF
      ENDIF
...
IF( NPROC > 1 )THEN
  ...
  DO JA=1,NPRGPNS
    ...
    IF(ISEND /= MYPROC) THEN
      ITAG=MTAGPART+ISEND
      CALL MPE_SEND(ICOMBUF,ILSEND,MINTET,NPRCIDS(ISEND),ITAG,0,0,0,IERR)
      IF (IERR < 0) THEN
        CALL ABOR1(' SUSTAONL: ERROR IN MPE_SEND GLOBAL INFO')
      ENDIF
    ENDIF
  ENDDO
  ...
  DO JA=1,NPRGPNS
    ...
    IF(IRECV /= MYPROC) THEN
      ITAG=MTAGPART+MYPROC
      CALL MPE_RECV(ICOMBUF,ILEN,MINTET,NPRCIDS(IRECV),&
       &ITAG,0,0,0,ILRECV,IRCVID,IRCVTG,IERR)
      IF (IERR < 0.OR. ILRECV /= ILEN ) THEN
        CALL ABOR1(' SUSTAONL: ERROR IN MPE_RECV GLOBAL INFO')
      ENDIF
      IGL1 = D%NFRSTLAT(JA)
      IGL2 = D%NLSTLAT(JA)
      IPOS=0
