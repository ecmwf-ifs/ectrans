...
INTEGER(KIND=JPIM) :: IDGNH,IDGL,IFLEVL
...
IF(ALLOCATED(D%NMYLEVS)) THEN
  IFLEVL=SIZE(D%NMYLEVS)
  CALL FSPGL_PROC(KM,KMLOC,ISL,IDGL,IFLEVL,KF_OUT_LT,F%R1MU2,D%NMYLEVS,ZFIELD,&
 &   IPTRVOR,IPTRDIV,IPTRU,IPTRV,IPTRSC,IPTRNSD,KF_UV,KF_SCALARS,KF_SCDERS,&
 &   IFLDPTRUV,IFLDPTRSC)
ELSE
  CALL ABORT_TRANS('D%NMYLEVS is not allocated, call to FSPGLH cannot be made. This should NOT happen') 
END IF
