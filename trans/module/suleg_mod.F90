...
!     ENDIF
    ...
    ELSE    
       ...
       DO JSETV=1,IPRTRV
          ...
          IF( S%LUSEFLT .AND. ILA > ITHRESHOLD ) THEN
             ...
             DO J=1,ILA
                DO I=1,IDGLU
                   S%FA(IMLOC)%RPNMDA(I,J) = F%RPNM(ISL+I-1,D%NPMS(IM)+IA+(J-1)*2)
                ENDDO
             ENDDO
          ELSE
             DO J=1,ILA
    ...
    IF( S%LUSEFLT) THEN
      ...
      IF(.NOT.LMPOFF) THEN
        ...
        ENDIF
        IF(IRREQ > 0) THEN
          CALL MPL_WAIT(ZRCVBUTFV(:,1),KOUNT=IKOUNT(1:IRREQ),KREQUEST=IRECVREQ(1:IRREQ), &
           & CDSTRING='SUTRLE: SULEG')
        ENDIF
        CALL GSTATS(852,1)
        CALL GSTATS(1252,0)
...
!              ZCLONEA(IMLOC)%COMMSBUF=>NULL()
            ENDIF
            IF( ASSOCIATED(S%FA(IMLOC)%RPNMA) ) DEALLOCATE(S%FA(IMLOC)%RPNMA)
    ...
    ELSE    
      ...
      DO JSETV=1,IPRTRV
        ...
        IF( S%LUSEFLT .AND. ILS > ITHRESHOLD ) THEN
           ...
           DO J=1,ILS
              DO I=1,IDGLU
                 S%FA(IMLOC)%RPNMDS(I,J) = F%RPNM(ISL+I-1,D%NPMS(IM)+IS+(J-1)*2)
              ENDDO
           ENDDO
        ELSE
           DO J=1,ILS
...
!              ZCLONES(IMLOC)%COMMSBUF=>NULL()
            ENDIF
            IF( ASSOCIATED(S%FA(IMLOC)%RPNMS) ) DEALLOCATE(S%FA(IMLOC)%RPNMS)
