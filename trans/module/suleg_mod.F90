...
INTEGER(KIND=JPIM) :: INM, IM, ICOUNT, IRECVSET, IRECV, ISEND, ISENDSET, ISREQ, IRREQ, &
             &JGL,  JM, JMLOC, IMLOC, IILOC, JN, JNM, IODD, INN, INMAX, JI, IMAXN, ITAG, ITAG1, &
             &INX, ISL, ISTART, ITHRESHOLD, INSMAX, IMAXCOLS,ILATSMAX,JW,JV,J, &
             &IDGLU, ILA, ILS, IA, IS, I, ILATS,ILOOP, JROC, IPRTRV, JSETV, JH, &
...
! and the parallelism is more limited ? Nils
  ...
  DO JMLOC=1,D%NUMP,NPRTRV  ! +++++++++++++++++++++ JMLOC LOOP +++++++++++++++++++++++
    ...
    IF( S%LUSEFLT) THEN
      ...
      IF( MYPROC == 1 )THEN
        IF( IRECVLENMAX > 2 )THEN
          WRITE(0,'("SULEG: ANTI-SYM MAX BUTTERFLY CLONE LEN=",I8)')IRECVLENMAX
        ENDIF
      ENDIF
    ...
    IF( S%LUSEFLT) THEN
      ...
      IF( MYPROC == 1 )THEN
        IF( IRECVLENMAX > 2 )THEN
          WRITE(0,'("SULEG: SYM      MAX BUTTERFLY CLONE LEN=",I8)')IRECVLENMAX
        ENDIF
      ENDIF
      ALLOCATE (ZRCVBUFV(IRECVLENMAX,IPRTRV))
      DO JSETV=1,IPRTRV
