...
REAL(KIND=JPRB),ALLOCATABLE :: ZLRMUZ2(:)
...
REAL(KIND=JPRD) :: ZANM
REAL(KIND=JPRD) :: ZFNN
REAL(KIND=JPRB) :: ZPI, ZINC, ZOFF, ZTEMP, ZORIG

REAL(KIND=JPRB), ALLOCATABLE :: ZSNDBUFV(:),ZRCVBUFV(:,:)
REAL(KIND=JPRB), ALLOCATABLE :: ZPNMCDO(:,:),ZPNMCDD(:,:)
...
IF(NPROC == 1 .OR. R%NDGNH < 1024) THEN
  ITHRESHOLD = R%NDGNH/2
  DO
...
IF( S%LDLL ) THEN
  ...
  ZORIG = ASIN(F%RMU(1))
  IF( S%LSHIFTLL )  THEN
    ZINC = ZPI/REAL(INN,JPRB)
    ZOFF = 0.5_JPRB*ZINC
    ZTEMP = ZOFF + ZINC*REAL(S%NDGNHD-1,JPRB)
    ZLRMUZ2(1) =  SIN(MIN(ZTEMP,ZORIG) - 0.5_JPRB*MAX(0._JPRB,ZTEMP - ZORIG))
    ZLRMUZ2(S%NDGNHD) = SIN(ZOFF)
  ELSE
    ZINC = ZPI/REAL(INN-2,JPRB)
    ZOFF=-0.5_JPRB*ZINC
    ZTEMP = ZOFF + ZINC*REAL(S%NDGNHD-1,JPRB)
    ZLRMUZ2(1) =  SIN(MIN(ZTEMP,ZORIG) - 0.5_JPRB*MAX(0._JPRB,ZTEMP - ZORIG))
    ZOFF=0.01_JPRB*ZINC
  ...
  ENDIF
  DO JGL=2, S%NDGNHD-1
    ZLRMUZ2(JGL) = SIN(ZOFF + ZINC*REAL(S%NDGNHD-JGL,JPRB))
  ENDDO
  DO JGL=1, S%NDGNHD
    ISYM = INN-JGL+1
  ...
  IF( LLP2 ) THEN
    ...
    DO JGL= 1, INN
      WRITE(NOUT,*)  'dual  JGL=',JGL,(180._JPRB/ZPI)*ZINC,(180._JPRB/ZPI)*ASIN(ZLRMUZ2(JGL)),(180._JPRB/ZPI)*ASIN(F%RMU(JGL))
    ENDDO
  ENDIF
  ...
  DO JGL=1,INN
    ...
    F%RACTHE2(JGL) = 1.0_JPRB/(SQRT(1.0_JPRB-ZLRMUZ2(JGL)*ZLRMUZ2(JGL))+ZEPS)/REAL(RA,JPRB)
  ENDDO
  DEALLOCATE(ZLRMUZ2)
...
IF(.NOT.D%LGRIDONLY) THEN
  ...
  IF( S%LUSE_BELUSOV .OR. S%LKEEPRPNM ) THEN
  ...
  DO JMLOC=1,D%NUMP
    ...
    IF( S%LDLL ) THEN
      ...
      !$OMP DO SCHEDULE(DYNAMIC,1)
        DO JGL=1,2*IDGLU2
          CALL SUPOLF(IM,INMAX,F%RMU2(S%FA(JMLOC)%ISLD+JGL-1),ZLPOL(0:INMAX))
        ...
        !$OMP DO SCHEDULE(DYNAMIC,1)
        DO JGL=1,2*IDGLU
          CALL SUPOLF(IM,INMAX,F%RMU(ISL+JGL-1),ZLPOL(0:INMAX))
...
!!$  ENDIF
  ...
  DO JMLOC=1,D%NUMP,NPRTRV  ! +++++++++++++++++++++ JMLOC LOOP +++++++++++++++++++++++
    ...
    IF( .NOT. S%LUSE_BELUSOV ) THEN 
      ...
      CALL GSTATS(851,0)
      IF(IRREQ > 0) THEN
        CALL MPL_WAIT(ZRCVBUFV(:,1),KREQUEST=IRECVREQ(1:IRREQ), &
         & CDSTRING='SUTRLE: SULEG')
      ENDIF

      IF(ISREQ > 0) THEN
        CALL MPL_WAIT(ZSNDBUFV(:),KREQUEST=ISENDREQ(1:ISREQ), &
         & CDSTRING='SUTRLE: SULEG')
      ENDIF
      ...
      IF( S%LKEEPRPNM) THEN
        CALL GSTATS(1251,0)
        !$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(JSETV,IMLOC,IM,ISL,IA,ILA,IDGLU,J,I)
        DO JSETV=1,IPRTRV
          IMLOC=JMLOC+JSETV-1
          IM = D%MYMS(IMLOC)
          ISL = MAX(R%NDGNH-G%NDGLU(IM)+1,1)
          ...
          DO J=1,ILA
            DO I=1,IDGLU
              F%RPNM(ISL+I-1,D%NPMS(IM)+IA+(J-1)*2) = S%FA(IMLOC)%RPNMA(I,J)
            ENDDO
          ENDDO
        ENDDO
        !$OMP END PARALLEL DO
    ...
    ELSE    
      ...
      DO JSETV=1,IPRTRV
        ...
        DO J=1,ILA
          DO I=1,IDGLU
            S%FA(IMLOC)%RPNMA(I,J) = F%RPNM(ISL+I-1,D%NPMS(IM)+IA+(J-1)*2)
          ENDDO
        ENDDO
      ENDDO
      !$OMP END PARALLEL DO
    ...
    IF( S%LUSEFLT) THEN
      ...
      IF(.NOT.LMPOFF) THEN
        ...
        IF( MYPROC == 1 )THEN
          ...
          ENDIF
        ENDIF
        ALLOCATE (ZRCVBUFV(IRECVLENMAX,IPRTRV))
        DO JSETV=1,IPRTRV
          CALL SET2PE(IRECV,0,0,MYSETW,JSETV)
          IRREQ = IRREQ+1
          CALL MPL_RECV(ZRCVBUFV(:,JSETV),KSOURCE=NPRCIDS(IRECV), &
        ...
        ENDDO
        IF(ISREQ > 0) THEN
          CALL MPL_WAIT(ZDUM(:),KREQUEST=ISENDREQ(1:ISREQ), &
           & CDSTRING='SUTRLE: SULEG')
        ENDIF
        IF(IRREQ > 0) THEN
          CALL MPL_WAIT(ZRCVBUFV(:,1),KOUNT=IKOUNT(1:IRREQ),KREQUEST=IRECVREQ(1:IRREQ), &
           & CDSTRING='SUTRLE: SULEG')
        ENDIF
        CALL GSTATS(852,1)
        CALL GSTATS(1252,0)
    ...
    IF( .NOT. S%LUSE_BELUSOV ) THEN 
      ...
      IF( S%LKEEPRPNM) THEN
        CALL GSTATS(1251,0)
        !$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(JSETV,IMLOC,IM,ISL,IS,ILS,IDGLU,J,I)
        DO JSETV=1,IPRTRV
          IMLOC=JMLOC+JSETV-1
          IM = D%MYMS(IMLOC)
          ISL = MAX(R%NDGNH-G%NDGLU(IM)+1,1)
          ...
          DO J=1,ILS
            DO I=1,IDGLU
              F%RPNM(ISL+I-1,D%NPMS(IM)+IS+(J-1)*2) = S%FA(IMLOC)%RPNMS(I,J)
            ENDDO
          ENDDO
        ENDDO
        !$OMP END PARALLEL DO
    ...
    ELSE    
      ...
      DO JSETV=1,IPRTRV
        ...
        DO J=1,ILS
          DO I=1,IDGLU
            S%FA(IMLOC)%RPNMS(I,J) = F%RPNM(ISL+I-1,D%NPMS(IM)+IS+(J-1)*2)
          ENDDO
        ENDDO
      ENDDO
      !$OMP END PARALLEL DO
      ...
      IF(.NOT. LMPOFF) THEN
        ...
        DO JSETV=1,IPRTRV
          ...
          IF( ILS > ITHRESHOLD ) THEN
            IDGLU = MIN(R%NDGNH,G%NDGLU(IM))
            INX=IDGLU
            IF( .NOT.ASSOCIATED(ZCLONES(IMLOC)%COMMSBUF) )THEN
              ICLONELEN=IKOUNT(JSETV)
              ALLOCATE(ZCLONES(IMLOC)%COMMSBUF(ICLONELEN))
              ZCLONES(IMLOC)%COMMSBUF(:)=ZRCVBUFV(1:ICLONELEN,JSETV)
              CALL UNPACK_BUTTERFLY_STRUCT( S%FA(IMLOC)%YBUT_STRUCT_S,ZCLONES(IMLOC))
            ENDIF
            DEALLOCATE(ZCLONES(IMLOC)%COMMSBUF)
            ZCLONES(IMLOC)%COMMSBUF=>NULL()
            DEALLOCATE(S%FA(IMLOC)%RPNMS)
          ENDIF
        ENDDO
        !$OMP END PARALLEL DO
        CALL GSTATS(1252,1)
