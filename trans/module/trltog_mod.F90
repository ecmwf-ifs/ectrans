...
DO INR=1,KNRECV
  IRECV=KRECV(INR)
  CALL PE2SET(IRECV,ISETA,ISETB,ISETW(INR),ISETV)
  IFLD = 0
  DO JFLD=1,KF_GP
...
IF(KNSEND > 0) THEN
  CALL MPL_WAIT(KREQUEST=IREQ_SEND(1:KNSEND),CDSTRING='TRLTOG_COMM: WAIT FOR SENDS')
ENDIF

IF (.not.LTRANS_WAITANY) CALL MPL_BARRIER(CDSTRING='TRLTOG_COMM: BARRIER AT END')
