...
DO INR=1,KNRECV
  ...
  CALL MPL_RECV(ZCOMBUFR(-1:KRECVTOT(IRECV),INR), &
    ...
    & KTAG=ITAG,CDSTRING='TRLTOG_COMM:' )
ENDDO
...
DO JNR=1,KNRECV
  ...
  CALL MPL_WAITANY(KREQUEST=IREQ_RECV(1:KNRECV),KINDEX=INR,CDSTRING='TRLTOG_COMM: WAIT FOR ANY RECEIVES')
...
IF(KNSEND > 0) THEN
  CALL MPL_WAIT(ZDUM,KREQUEST=IREQ_SEND(1:KNSEND),CDSTRING='TRLTOG_COMM: WAIT FOR SENDS')
ENDIF
CALL GSTATS(805,1)
