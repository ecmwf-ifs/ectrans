...
IF (.NOT.LSYNCSTATS) CALL GSTATS(133,0)
ALLOCATE(ZGTF(KF_FS,D%NLENGTF))

! Save Fourier data in FOUBUF_IN

IF (.NOT.LSYNCSTATS) CALL GSTATS(1642,0)
...
ENDDO
!$OMP END PARALLEL DO
IF (.NOT.LSYNCSTATS) CALL GSTATS(1642,1)
IF (.NOT.LSYNCSTATS) CALL GSTATS(133,1)

DEALLOCATE(FOUBUF_IN)

! Transposition

IF (.NOT.LSYNCSTATS) CALL GSTATS(183,0)
...
DEALLOCATE(ZGTF)
IF (.NOT.LSYNCSTATS) CALL GSTATS(183,1)
