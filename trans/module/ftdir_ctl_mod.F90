...
IF (.NOT.LSYNCSTATS) CALL GSTATS(158,0)
CALL TRGTOL(ZGTF,PGP,KF_FS,KF_GP,IVSET,KPTRGP)
IF (.NOT.LSYNCSTATS) CALL GSTATS(158,1)
IF (.NOT.LSYNCSTATS) CALL GSTATS(106,0)
...
IF (.NOT.LSYNCSTATS) CALL GSTATS(1640,0)
...
ENDDO
!$OMP END PARALLEL DO
IF (.NOT.LSYNCSTATS) CALL GSTATS(1640,1)
IF(.NOT.LALLOPERM) THEN
  DEALLOCATE(ZGTF)
ENDIF
IF (.NOT.LSYNCSTATS) CALL GSTATS(106,1)
