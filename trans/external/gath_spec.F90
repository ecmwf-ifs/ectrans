...
IF(IFRECV > 0) THEN
  ...
  IF(UBOUND(PSPECG,1) < IFRECV) THEN
    CALL ABORT_TRANS('GATH_SPEC:FIRST DIMENSION OF PSPECG TOO SMALL')
  ENDIF 
...
IF(IFSEND > 0 ) THEN
  ...
  IF(UBOUND(PSPEC,1) < IFSEND) THEN
    CALL ABORT_TRANS('GATH_SPEC: FIRST DIMENSION OF PSPEC TOO SMALL')
  ENDIF
  IF(UBOUND(PSPEC,2) < D%NSPEC2 ) THEN
    CALL ABORT_TRANS('GATH_SPEC: SECOND DIMENSION OF PSPEC TOO SMALL')
  ENDIF
ENDIF
