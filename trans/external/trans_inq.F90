...
IF(PRESENT(KGPTOTL)) THEN
  IF(UBOUND(KGPTOTL,1) < NPRGPNS) THEN
    CALL ABOR1('TRANS_INQ: KGPTOTL DIM 1 TOO SMALL')
  ELSEIF(UBOUND(KGPTOTL,2) <  NPRGPEW) THEN
    CALL ABOR1('TRANS_INQ: KGPTOTL DIM 2 TOO SMALL')
  ELSE
    KGPTOTL(1:NPRGPNS,1:NPRGPEW) = D%NGPTOTL(:,:)
  ENDIF
ENDIF
