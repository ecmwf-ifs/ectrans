...
IF (NF_UV > 0) THEN
  IF(.NOT. PRESENT(PSPVOR) ) THEN
    CALL ABOR1('DIR_TRANSAD : NF_UV > 0 BUT PSPVOR MISSING')
  ENDIF
  IF(UBOUND(PSPVOR,1) < NF_UV) THEN
    WRITE(NERR,*)'DIR_TRANSAD : UBOUND(PSPVOR,1) < NF_UV ',&
     & UBOUND(PSPVOR,1),NF_UV
    CALL ABOR1('DIR_TRANSAD : PSPVOR TOO SHORT')
  ENDIF
  IF(.NOT. PRESENT(PSPDIV) ) THEN
    CALL ABOR1('DIR_TRANSAD : PSPVOR PRESENT BUT PSPDIV MISSING')
  ENDIF
  IF(UBOUND(PSPDIV,1) /= NF_UV) THEN
    WRITE(NERR,*)'DIR_TRANSAD : UBOUND(PSPDIV,1) < NF_UV ',&
...
IF (NF_SCALARS > 0) THEN
  IF(.NOT. PRESENT(PSPSCALAR) ) THEN
    CALL ABOR1('DIR_TRANSAD : NF_SCALARS > 0 BUT PSPSCALAR MISSING')
  ENDIF
  IF(UBOUND(PSPSCALAR,1) < NF_SCALARS) THEN
    WRITE(NERR,*)'DIR_TRANSAD : UBOUND(PSPSCALAR,1) < NF_SCALARS) ',&
     & UBOUND(PSPSCALAR,1),NF_SCALARS
    CALL ABOR1('DIR_TRANSAD : PSPSCALAR TOO SHORT')
  ENDIF
ENDIF
