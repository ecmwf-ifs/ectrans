...
IF (LUV) THEN
  IF(.NOT. PRESENT(PSPDIV) ) THEN
    CALL ABOR1("INV_TRANS : PSPVOR PRESENT BUT PSPDIV MISSING")
  ENDIF
  IF(UBOUND(PSPDIV,1) /= NF_UV) THEN
    CALL ABOR1("INV_TRANS : INCONSISTENT FIRST DIM. OF PSPVOR AND PSPDIV")
  ENDIF
ENDIF
IF(NF_UV_G < NF_UV ) THEN
  WRITE(NERR,*)'INV_TRANS:NF_UV_G < NF_UV',NF_UV_G,NF_UV
  CALL ABOR1('INV_TRANS:NF_UV_G < NF_UV')
ENDIF
IF(NF_SCALARS_G < NF_SCALARS ) THEN
  WRITE(NERR,*)'INV_TRANS:NF_SCALARS_G < NF_SCALARS',NF_SCALARS_G,NF_SCALARS
  CALL ABOR1('INV_TRANS:NF_SCALARS_G < NF_SCALARS')
ENDIF
IF(NF_UV_G == 0) THEN
  LUVDER = .FALSE.
ENDIF
