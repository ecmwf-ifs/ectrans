...
USE TPM_GEN,             ONLY: NERR, NOUT, LMPOFF, LSYNC_TRANS, NTRANS_SYNC_LEVEL, MSETUP0, &
  ...
  &                            LENABLED
USE TPM_DISTR,           ONLY: LEQ_REGIONS, NPRGPEW, NPRGPNS, NPRTRW, DISTR_RESOL
USE TPM_CONSTANTS,       ONLY: RA
USE TPM_DIM,             ONLY: DIM_RESOL
USE TPM_FIELDS,          ONLY: FIELDS_RESOL
USE TPM_GEOMETRY,        ONLY: GEOM_RESOL
...
! Allocate arrays for storing structures for each resolution
...
ALLOCATE(Y_RESOL_DEALLOCATORS(NMAX_RESOL))
