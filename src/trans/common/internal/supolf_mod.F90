...
INTEGER(KIND=JPIM) :: JN, K, ICHEAP, IC
...
DCL(K) = SQRT(REAL((K - KM + 1) * (K - KM + 2) * (K + KM + 1) * (K + KM + 2), JPRD) / &
 &  REAL((2 * K + 1) * (2 * K + 3) * (2 * K + 3) * (2 * K + 5), JPRD))
DDL(K) = REAL(2 * K * (K + 1) - 2 * KM**2 - 1, JPRD) / REAL((2 * K - 1) * (2 * K + 3), JPRD)
