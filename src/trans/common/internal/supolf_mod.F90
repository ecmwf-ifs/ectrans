...
REAL(KIND=JPRD) :: DLX, COS_THETA, COS2_THETA, COS_THETA_R, DLK, DL1, DLKM1, DLKM2
...
IF (ABS(COS_THETA) <= ZEPS) THEN
  ...
  COS2_THETA = 0.0_JPRD
ELSE
  COS_THETA_R = 1.0_JPRD/COS_THETA
ENDIF
...
ELSEIF( KM == 1 ) THEN
  DDPOL(0)=0
  DDPOL(1)=COS_THETA*DFB(1)
  DO JN=2,KNSMAX
    DLK=DFF(JN)*DLX*DLKM1-DFG(JN)*DLKM2
    DL1=DFI(JN)*(DLKM1-DLX*DLK)*COS_THETA_R
    DDPOL(JN)=DL1*DFB(JN)
    DLKM2=DLKM1
    DLKM1=DLK
  ENDDO
