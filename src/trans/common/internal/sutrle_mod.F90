...
MODULE SUTRLE_MOD
CONTAINS
SUBROUTINE SUTRLE(PNM,KGL,KLOOP,PPNM)
...
USE TPM_DIM,         ONLY: R
USE TPM_DISTR,       ONLY: D, MTAGLETR, NPRCIDS, NPRTRW, NPRTRV, MYSETV, MYSETW, NPROC
...
REAL(KIND=JPRD),INTENT(INOUT) :: PPNM(:,;)
  ...
  IF( IGL > 0 )THEN
    DO JMLOC=1,D%NUMP
      ...
      DO JN=1,R%NTMAX-IM+2
        PPNM(IGL,IOFFT+JN) = ZRCVBUFV(1+IOFFG+JN,IRECVSET)
      ENDDO
    ENDDO
  ENDIF
ENDDO
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JMLOC,IM,IOFFT,IOFFG,JN)
  DO JMLOC=1,D%NUMP
    ...
    IOFFG = D%NPMG(IM)
    DO JN=1,R%NTMAX-IM+2
      PPNM(KGL,IOFFT+JN) = PNM(IOFFG+JN)
    ENDDO
  ENDDO
!$OMP END PARALLEL DO
ENDIF
  ...
  DO JV=1,NPRTRV
    ...
    IF( IGL > 0 )THEN
      DO JMLOC=1,D%NUMP
        ...
        DO JN=1,R%NTMAX-IM+2
          IPOS = IPOS + 1
          PPNM(IGL,IOFFT+JN) = ZRCVBUFW(IPOS,IRECVSET)
        ENDDO
      ENDDO
    ENDIF
  ENDDO
