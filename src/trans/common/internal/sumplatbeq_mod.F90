...
! for the first latitude
    IF( LDSPLIT )THEN
      ...
      IF( KMEDIAP < (KLOENG(KDGSA)-1)/I2REGIONS+1 )THEN
        WRITE(NERR,'("SUMPLATBEQ: KMEDIAP=",I6," I2REGIONS=",I3," KLOENG(KDGSA)=",I4)')&
...
! test for number asets too large for the number of latitudes
      IF( KPROCA > KDGL )THEN
        WRITE(NERR,'("SUMPLATBEQ: KMEDIAP=",I6," KPROCA=",I4," KDGL=",I4)')&
        &KMEDIAP,KPROCA,KDGL
...
IF (LDSPLIT) THEN
  ...
  IF( LDWEIGHTED_DISTR )THEN
    IF( KLAST(KPROCA) /= KDGL )THEN
      DO JA=1,KPROCA
        IF( MYPROC == 1 )THEN
          WRITE(NOUT,'("SUMPLATBEQ_MOD: JA=",I3," KLAST=",I3," KINDIC=",I3)')&
          &JA,KLAST(JA),KINDIC(JA)
        ENDIF
      ENDDO
      WRITE(NOUT,'("SUMPLATBEQ: LWEIGHTED_DISTR=T FAILED TO PARTITION GRID, REVERTING TO ",&
  ...
  IF( SUM(KPROCAGP(:)) /= SUM(KLOENG(KDGSA:KDGL)) )THEN
    IF( MYPROC == 1 )THEN
      WRITE(NERR,'("SUM(KPROCAGP(:))=",I12)')SUM(KPROCAGP(:))
      WRITE(NERR,'("SUM(KLOENG(:))=",I12)')SUM(KLOENG(KDGSA:KDGL))
    ENDIF
...
ELSE
  ...
  IMEDIAP=KMEDIAP
  IF( MYPROC == 1 )THEN
    WRITE(NOUT,'("SUMPLATBEQ: IMEDIAP=",I6)')IMEDIAP
  ENDIF
  DO WHILE(.NOT.LLDONE)
!   loop until a satisfactory distribution can be found
    ...
    DO JGL=1,KDGL
      ...
      IF( IA == KPROCA .AND. JGL == KDGL )THEN
        IF( MYPROC == 1 )THEN
          WRITE(NOUT,'("SUMPLATBEQ: EXIT 1")')
        ENDIF
        EXIT
      ENDIF
      IF( IA == KPROCA .AND. JGL < KDGL )THEN
        IF( MYPROC == 1 )THEN
          WRITE(NOUT,'("SUMPLATBEQ: EXIT 2")')
        ENDIF
        KLAST(KPROCA)=KDGL
        EXIT
      ENDIF
    ...
    IF( KPROCA > 1 .AND. KLAST(KPROCA) == KLAST(KPROCA-1) )THEN
      IMEDIAP=IMEDIAP-1
      IF( MYPROC == 1 )THEN
        WRITE(NOUT,'("SUMPLATBEQ: REDUCING IMEDIAP=",I6)')IMEDIAP
      ENDIF
      IF( IMEDIAP <= 0 )THEN
