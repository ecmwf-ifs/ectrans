...
TYPE :: FPTR_DEALLOCATOR
  INTEGER(KIND=JPIM) :: BACKEND
  PROCEDURE(DEALLOC_RESOL), POINTER, NOPASS :: F => NULL()
END TYPE

TYPE(FPTR_DEALLOCATOR), ALLOCATABLE :: Y_RESOL_DEALLOCATORS(:)
...
TYPE :: FPTR_TEARDOWN
  ...
  PROCEDURE(BACKEND_TEARDOWN), POINTER, NOPASS :: F => NULL()
END TYPE

TYPE(FPTR_TEARDOWN) :: Y_BACKEND_TEARDOWNS(JP_N_BACKENDS)
