...
IF (LHOOK) CALL DR_HOOK('GATH_GRID',0,ZHOOK_HANDLE)
...
IF(IUBOUND(1) /= IPROMA) THEN
  WRITE(NOUT,*)'GATH_GRID:FIRST DIMENSION OF PGP /= IPROMA',IUBOUND(1),IPROMA
  CALL ABORT_TRANS('GATH_GRID:FIRST DIMENSION OF PGP /= IPROMA')
ENDIF
IF(IUBOUND(2) /= KFGATHG) THEN
  WRITE(NOUT,*)'GATH_GRID:SEC. DIMENSION OF PGP /= KFGATHG',IUBOUND(2),KFGATHG
  CALL ABORT_TRANS('GATH_GRID:SECOND DIMENSION OF PGP /= KFGATHG')
ENDIF
IF(IUBOUND(3) /= IGPBLKS) THEN
  WRITE(NOUT,*)'GATH_GRID:THIRD DIMENSION OF PGP /= IGPBLKS',IUBOUND(3),IGPBLKS
  CALL ABORT_TRANS('GATH_GRID:THIRD DIMENSION OF PGP /= IGPBLKS')
ENDIF
...
IF(IFRECV > 0) THEN
  ...
  IF(UBOUND(PGPG,1) /= D%NGPTOTG) THEN
    WRITE(NOUT,*)'GATH_GRID:FIRST DIMENSION OF PGPG /= D%NGPTOTG',UBOUND(PGPG,1),D%NGPTOTG
    CALL ABORT_TRANS('GATH_GRID:FIRST DIMENSION OF PGPG /= D%NGPTOTG')
  ENDIF
  IF(UBOUND(PGPG,2) /= IFRECV) THEN
    WRITE(NOUT,*)'GATH_GRID:SECOND DIMENSION OF PGPG /= IFRECV',UBOUND(PGPG,2),IFRECV
    CALL ABORT_TRANS('GATH_GRID:SECOND DIMENSION OF PGPG /= IFRECV')
  ENDIF
ENDIF

CALL GATH_GRID_CTL(PGPG,KFGATHG,IPROMA,KTO,PGP)

IF (LHOOK) CALL DR_HOOK('GATH_GRID',1,ZHOOK_HANDLE)
!endif INTERFACE
