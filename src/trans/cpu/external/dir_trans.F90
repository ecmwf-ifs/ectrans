...
IF(PRESENT(KVSETUV)) THEN
  ...
  DO J=1,IF_UV_G
    IF(KVSETUV(J) > NPRTRV .OR. KVSETUV(J) < 1) THEN
      WRITE(NERR,*) 'DIR_TRANS:KVSETUV(J) OUT OF RANGE ',J,KVSETUV(J),NPRTRV
...
IF(PRESENT(KVSETSC)) THEN
  ...
  DO J=1,IF_SCALARS_G
    IF(KVSETSC(J) > NPRTRV .OR. KVSETSC(J) < 1) THEN
      WRITE(NERR,*) 'DIR_TRANS:KVSETSC(J) OUT OF RANGE ',J,KVSETSC(J),NPRTRV
...
IF(PRESENT(KVSETSC2)) THEN
  IF(.NOT.PRESENT(PSPSC2)) THEN
    CALL ABORT_TRANS('DIR_TRANS:KVSETSC2 PRESENT BUT NOT PSPSC2')
  ENDIF
  ...
  DO J=1,UBOUND(KVSETSC2,1)
    IF(KVSETSC2(J) > NPRTRV .OR. KVSETSC2(J) < 1) THEN
      WRITE(NERR,*) 'DIR_TRANS:KVSETSC2(J) OUT OF RANGE ',J,KVSETSC2(J),NPRTRV
...
IF(PRESENT(KVSETSC3A)) THEN
  IF(.NOT.PRESENT(PSPSC3A)) THEN
    CALL ABORT_TRANS('DIR_TRANS:KVSETSC3A PRESENT BUT NOT PSPSC3A')
  ENDIF
  ...
  DO J=1,UBOUND(KVSETSC3A,1)
    IF(KVSETSC3A(J) > NPRTRV .OR. KVSETSC3A(J) < 1) THEN
      WRITE(NERR,*) 'DIR_TRANS:KVSETSC3A(J) OUT OF RANGE ',J,KVSETSC3A(J),NPRTRV
...
IF(PRESENT(KVSETSC3B)) THEN
  IF(.NOT.PRESENT(PSPSC3B)) THEN
    CALL ABORT_TRANS('DIR_TRANS:KVSETSC3B PRESENT BUT NOT PSPSC3B')
  ENDIF
  ...
  DO J=1,UBOUND(KVSETSC3B,1)
    IF(KVSETSC3B(J) > NPRTRV .OR. KVSETSC3B(J) < 1) THEN
      WRITE(NERR,*) 'DIR_TRANS:KVSETSC3B(J) OUT OF RANGE ',J,KVSETSC3B(J),NPRTRV
...
IF (IF_UV > 0) THEN
  ...
  IF(UBOUND(PSPVOR,1) /= IF_UV) THEN
    ...
    CALL ABORT_TRANS('DIR_TRANS : FIRST DIMENSION OF PSPVOR /= IF_UV')
  ENDIF
  ...
  IF(UBOUND(PSPDIV,1) /= IF_UV) THEN
    ...
    CALL ABORT_TRANS('DIR_TRANS : INCONSISTENT FIRST DIMENSION OF PSPVOR AND PSPDIV')
  ENDIF
ENDIF
...
IF (IF_SCALARS > 0) THEN
  IF(PRESENT(PSPSCALAR)) THEN
    IF(UBOUND(PSPSCALAR,1) /= IF_SCALARS) THEN
      ...
      CALL ABORT_TRANS('DIR_TRANS : FIRST DIMENSION OF PSPSCALAR /= IF_SCALARS')
    ENDIF
...
IF(PRESENT(PGP)) THEN
  ...
  IF(IUBOUND(1) /= NPROMA) THEN
    WRITE(NOUT,*)'DIR_TRANS:FIRST DIMENSION OF PGP /= NPROMA',IUBOUND(1),NPROMA
    CALL ABORT_TRANS('DIR_TRANS:FIRST DIMENSION OF PGP /= NPROMA')
  ENDIF
  IF(IUBOUND(2) /= IF_GP) THEN
    WRITE(NOUT,*)'DIR_TRANS:SECOND DIMENSION OF PGP /= IF_GP',IUBOUND(2),IF_GP
    CALL ABORT_TRANS('DIR_TRANS:SECOND DIMENSION OF PGP /= IF_GP')
  ENDIF
  IF(IUBOUND(3) /= NGPBLKS) THEN
    WRITE(NOUT,*)'DIR_TRANS:THIRD DIMENSION OF PGP /= NGPBLKS ',IUBOUND(3),NGPBLKS
    CALL ABORT_TRANS('DIR_TRANS:THIRD DIMENSION OF PGP /= NGPBLKS')
  ENDIF
ENDIF
...
IF(PRESENT(PGPUV)) THEN
  ...
  IF(IUBOUND(1) /= NPROMA) THEN
    WRITE(NOUT,*)'DIR_TRANS:FIRST DIMENSION OF PGPUV /= NPROMA ',IUBOUND(1),NPROMA
    CALL ABORT_TRANS('DIR_TRANS:FIRST DIMENSION OF PGPUV /= NPROMA')
  ENDIF
  IF(IUBOUND(2) /= IF_UV_G) THEN
    WRITE(NOUT,*)'DIR_TRANS:SECOND DIMENSION OF PGPUV /= IF_UV_G',IUBOUND(2),IF_UV_G
    CALL ABORT_TRANS('DIR_TRANS:SECOND DIMENSION OF PGPUV /= IF_UV_G')
  ENDIF
  IF(IUBOUND(3) /= 2) THEN
    WRITE(NOUT,*)'DIR_TRANS:THIRD DIMENSION OF PGPUV MUST BE 2',IUBOUND(3),2
    CALL ABORT_TRANS('DIR_TRANS:THIRD DIMENSION OF PGPUV MUST BE 2')
  ENDIF
  IF(IUBOUND(4) /= NGPBLKS) THEN
    WRITE(NOUT,*)'DIR_TRANS:FOURTH DIMENSION OF PGPUV /= NGPBLKS ',IUBOUND(4),NGPBLKS
    CALL ABORT_TRANS('DIR_TRANS:FOURTH DIMENSION OF PGPUV /= NGPBLKS')
  ENDIF
ENDIF
...
IF(IF_SC2_G > 0) THEN
  IF(PRESENT(PGP2)) THEN
    ...
    IF(IUBOUND(1) /= NPROMA) THEN
      WRITE(NOUT,*)'DIR_TRANS:FIRST DIMENSION OF PGP2 /= NPROMA',IUBOUND(1),NPROMA
      CALL ABORT_TRANS('DIR_TRANS:FIRST DIMENSION OF PGP2 /= NPROMA')
    ENDIF
    IF(IUBOUND(2) /= IF_SC2_G) THEN
      WRITE(NOUT,*)'DIR_TRANS:SECOND DIMENSION OF PGP2 /= IF_SC2_G',IUBOUND(2),IF_SC2_G
      CALL ABORT_TRANS('DIR_TRANS:SECOND DIMENSION OF PGP2 /= IF_SC2_G')
    ENDIF
    IF(IUBOUND(3) /= NGPBLKS) THEN
      WRITE(NOUT,*)'DIR_TRANS:THIRD DIMENSION OF PGP2 /= NGPBLKS',IUBOUND(3),NGPBLKS
      CALL ABORT_TRANS('DIR_TRANS:THIRD DIMENSION OF PGP2 /= NGPBLKS')
    ENDIF
  ELSE
...
IF(IF_SC3A_G > 0) THEN
  IF(PRESENT(PGP3A)) THEN
    ...
    IF(IUBOUND(1) /= NPROMA) THEN
      WRITE(NOUT,*)'DIR_TRANS:FIRST DIMENSION OF PGP3A /= NPROMA',IUBOUND(1),NPROMA
      CALL ABORT_TRANS('DIR_TRANS:FIRST DIMENSION OF PGP3A /= NPROMA')
    ENDIF
    IF(IUBOUND(2) /= IF_SC3A_G) THEN
      WRITE(NOUT,*)'DIR_TRANS:SECOND DIMENSION OF PGP3A /= IF_SC3A_G ',IUBOUND(2),IF_SC3A_G
      CALL ABORT_TRANS('DIR_TRANS:SECOND DIMENSION OF PGP3A /= IF_SC3A_G')
    ENDIF
    IF(IUBOUND(3) /= UBOUND(PSPSC3A,3) ) THEN
      WRITE(NOUT,*)'DIR_TRANS:THIRD DIMENSION OF PGP3A /= THIRD DIMENSION OF PSPSC3A ',&
       & IUBOUND(3),UBOUND(PSPSC3A,3)
      CALL ABORT_TRANS('DIR_TRANS:THIRD DIMENSION OF PGP3A /= THIRD DIMENSION OF PSPSC3A')
    ENDIF
    IF(IUBOUND(4) /= NGPBLKS) THEN
      WRITE(NOUT,*)'DIR_TRANS:FOURTH DIMENSION OF PGP3A /= NGPBLKS',IUBOUND(4),NGPBLKS
      CALL ABORT_TRANS('DIR_TRANS:FOURTH DIMENSION OF PGP3A /= NGPBLKS')
    ENDIF
  ELSE
...
IF(IF_SC3B_G > 0) THEN
  IF(PRESENT(PGP3B)) THEN
    ...
    IF(IUBOUND(1) /= NPROMA) THEN
      WRITE(NOUT,*)'DIR_TRANS:FIRST DIMENSION OF PGP3B /= NPROMA',IUBOUND(1),NPROMA
      CALL ABORT_TRANS('DIR_TRANS:FIRST DIMENSION OF PGP3B /= NPROMA')
    ENDIF
    IF(IUBOUND(2) /= IF_SC3B_G) THEN
      WRITE(NOUT,*)'DIR_TRANS:SECOND DIMENSION OF PGP3B /= IF_SC3B_G',IUBOUND(2),IF_SC3B_G
      CALL ABORT_TRANS('DIR_TRANS:SECOND DIMENSION OF PGP3B /= IF_SC3B_G')
    ENDIF
    IF(IUBOUND(3) /= UBOUND(PSPSC3B,3) ) THEN
      WRITE(NOUT,*)'DIR_TRANS:THIRD DIMENSION OF PGP3B /= THIRD DIMENSION OF PSPSC3B',&
       & IUBOUND(3),UBOUND(PSPSC3B,3)
      CALL ABORT_TRANS('DIR_TRANS:THIRD DIMENSION OF PGP3B /= THIRD DIMENSION OF PSPSC3B')
    ENDIF
    IF(IUBOUND(4) /= NGPBLKS) THEN
      WRITE(NOUT,*)'DIR_TRANS:FOURTH DIMENSION OF PGP3B /= NGPBLKS',IUBOUND(4),NGPBLKS
      CALL ABORT_TRANS('DIR_TRANS:FOURTH DIMENSION OF PGP3B /= NGPBLKS')
    ENDIF
  ELSE
