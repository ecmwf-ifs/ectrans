...
IF (.NOT. SPLIT_CALLS() .OR. (PRESENT(PSPECG) .AND. PRESENT(PSPEC))) THEN
  CALL DIST_SPEC_CONTROL(PSPECG=PSPECG, PSPEC=PSPEC, &
   & KFDISTG=KFDISTG,KFROM=KFROM,KVSET=IVSET,LDIM1_IS_FLD=LLDIM1_IS_FLD,&
   & KSMAX=ISMAX,KSPEC2=ISPEC2,KSPEC2MX=ISPEC2MX,KSPEC2G=ISPEC2_G,KPOSSP=IPOSSP,KDIM0G=IDIM0G,&
   & KUMPP=IUMPP,KALLMS=IALLMS,KPTRMS=IPTRMS,KN=IKN,KSORT=KSORT)
ELSE IF (PRESENT(PSPECG) .AND. .NOT. PRESENT(PSPEC)) THEN
  CALL DIST_SPEC_CONTROL(PSPECG=PSPECG, &
   & KFDISTG=KFDISTG,KFROM=KFROM,KVSET=IVSET,LDIM1_IS_FLD=LLDIM1_IS_FLD,&
   & KSMAX=ISMAX,KSPEC2=ISPEC2,KSPEC2MX=ISPEC2MX,KSPEC2G=ISPEC2_G,KPOSSP=IPOSSP,KDIM0G=IDIM0G,&
   & KUMPP=IUMPP,KALLMS=IALLMS,KPTRMS=IPTRMS,KN=IKN,KSORT=KSORT)
ELSE IF (.NOT. PRESENT(PSPECG) .AND. PRESENT(PSPEC)) THEN
  CALL DIST_SPEC_CONTROL(PSPEC=PSPEC, &
   & KFDISTG=KFDISTG,KFROM=KFROM,KVSET=IVSET,LDIM1_IS_FLD=LLDIM1_IS_FLD,&
   & KSMAX=ISMAX,KSPEC2=ISPEC2,KSPEC2MX=ISPEC2MX,KSPEC2G=ISPEC2_G,KPOSSP=IPOSSP,KDIM0G=IDIM0G,&
   & KUMPP=IUMPP,KALLMS=IALLMS,KPTRMS=IPTRMS,KN=IKN,KSORT=KSORT)
ELSE
...
FUNCTION SPLIT_CALLS()
! Boolean function to determine if calls to DIST_SPEC_CONTROL should be split based on the presence of PSPECG and PSPEC.
! Return value:
!  - .TRUE. (DEFAULT) : The calls to DIST_SPEC_CONTROL should be split based on the presence of PSPECG and PSPEC.
!  - .FALSE.          : A single call to DIST_SPEC_CONTROL is made regardless of the presence of PSPECG and PSPEC.
!                       In that case the CONTIGUOUS attribute in DIST_SPEC_CONTROL may cause runtime issues on some compilers.
!                       That is probably a compiler bug.
!                       A workaround in DIST_SPEC_CONTROL exists to avoid using the CONTIGUOUS attribute, when
!                       the preprocessor defines "CONTIG_BUGGY_COMPILER".
! The preprocessor macro SPLIT_CALLS_ON_DIST_SPEC_CONTROL={0,1} can be defined to control this behavior.

  LOGICAL :: SPLIT_CALLS
  SPLIT_CALLS = .TRUE.

#ifdef SPLIT_CALLS_ON_DIST_SPEC_CONTROL
#if SPLIT_CALLS_ON_DIST_SPEC_CONTROL == 0
  SPLIT_CALLS = .FALSE.
#endif
#endif
END FUNCTION SPLIT_CALLS
