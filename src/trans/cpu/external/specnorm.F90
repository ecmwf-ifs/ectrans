...
IF(PRESENT(KVSET)) THEN
  ...
  DO J=1,IFLD_G
    ...
    ENDIF
  ENDDO
ELSEIF(PRESENT(PSPEC)) THEN
  IFLD = UBOUND(PSPEC,1)
  IFLD_G = IFLD
ELSE
  CALL ABORT_TRANS('SPECNORM:PSPEC AND/OR KVSET NEEDS TO BE PRESENT')
ENDIF
...
IF(PRESENT(PSPEC)) THEN
  CALL SPNORM_CTL(PNORM=PNORM,KFLD=IFLD,KFLD_G=IFLD_G,KVSET=KVSET,KMASTER=IMASTER,PMET=PMET,PSPEC=PSPEC)
ELSE
  CALL SPNORM_CTL(PNORM=PNORM,KFLD=IFLD,KFLD_G=IFLD_G,KVSET=KVSET,KMASTER=IMASTER,PMET=PMET)
ENDIF
