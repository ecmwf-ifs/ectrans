...
INTEGER(KIND=JPIM) :: JM, IM, IBLEN, IF_FS_CMPLX

!     ------------------------------------------------------------------

IF_FS_CMPLX = 2 * KF_FS ! KF_FS complex values is 2 * KF_FS real values

! Transposition from Fourier space distribution to spectral space distribution

IBLEN = D%NLENGT0B * IF_FS_CMPLX
...
CALL GSTATS(153,0)
CALL TRLTOM(FOUBUF_IN, FOUBUF, IF_FS_CMPLX)
CALL GSTATS(153,1)
...
!$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(JM,IM)
  DO JM=1,D%NUMP
    IM = D%MYMS(JM)
    CALL LTDIR(IM, JM, KF_UV, KF_SCALARS, IF_FS_CMPLX, PSPVOR, PSPDIV, PSPSCALAR, PSPSC3A, &
