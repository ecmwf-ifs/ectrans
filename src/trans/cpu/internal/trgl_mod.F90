...
SUBROUTINE ALLOCATE_CTX_CST(SELF)
...
END SUBROUTINE ALLOCATE_CTX_CST


SUBROUTINE ALLOCATE_CTX_SR(SELF, KF_GP)
  USE TPM_TRANS       ,ONLY : NGPBLKS
  ...
  ENDIF
END SUBROUTINE ALLOCATE_CTX_SR
...
SUBROUTINE TRGL_INIT_OFF_VARS(YDCTX,YLVARS,KVSET,KPTRGP,KF_GP)
  ...
  ASSOCIATE(IFLDOFF=>YLVARS%IFLDOFF, IGPTROFF=>YLVARS%IGPTROFF, LLINDER=>YDCTX%LLINDER)
  IFLDS = 0
  DO JFLD=1,KF_GP
...
SUBROUTINE TGRL_INIT_PACKING_VARS(YDCTX,YLVARS, KVSET, KF_GP, PCOMBUFS)
  USE PARKIND1  ,ONLY : JPIM, JPRB
  USE TPM_TRANS       ,ONLY : NGPBLKS
  TYPE(TRGL_CTX),  INTENT(INOUT) :: YDCTX
  TYPE(TRGL_VARS), INTENT(INOUT) :: YLVARS
...
SUBROUTINE TGRL_COPY_ZCOMBUF(YDCTX, YLVARS, INRS,&
                      & ZCOMBUF, &
                      & KPTRGP,&
                      & PGP, PGPUV, PGP3A, PGP3B, PGP2)
  ...
  ASSOCIATE (KSENDCOUNT=>YDCTX%ISENDCOUNT, KRECVCOUNT=>YDCTX%IRECVCOUNT, KNSEND=>YDCTX%INSEND, KNRECV=>YDCTX%INRECV, &
          ...
          & KNDOFF=>YDCTX%INDOFF, KGPTRSEND =>YDCTX%IGPTRSEND,&
          & IFLDA=>YDCTX%IFLDA,IPOSPLUS=>YDCTX%IPOSPLUS, JPOS=>YDCTX%IJPOS, ISETW=>YDCTX%ISETW,LLPGPONLY=>YDCTX%LLPGPONLY, &
          & LLINDER=>YDCTX%LLINDER)
...
SUBROUTINE TGRL_COPY_PGLAT(PGLAT, YDCTX, YLVARS, &
                    & PGP, PGPUV, PGP3A, PGP3B, PGP2)
  ...
  ASSOCIATE (KSENDCOUNT=>YDCTX%ISENDCOUNT, KRECVCOUNT=>YDCTX%IRECVCOUNT, KNSEND=>YDCTX%INSEND, KNRECV=>YDCTX%INRECV, &
          & KSENDTOT=>YDCTX%ISENDTOT, KRECVTOT=>YDCTX%IRECVTOT, KSEND=>YDCTX%ISEND, KRECV=>YDCTX%IRECV, KINDEX=>YDCTX%IINDEX, &
          & KNDOFF=>YDCTX%INDOFF, KGPTRSEND =>YDCTX%IGPTRSEND,IFLDS=>YDCTX%IFLDS,LLPGPONLY=>YDCTX%LLPGPONLY, LLINDER=>YDCTX%LLINDER)
...
SUBROUTINE TRGL_PROLOG(KF_FS,KF_GP,KVSET,YDCTX)
...
DO JROC=1,NPROC
  ...
  CALL PE2SET(JROC,ISETA,ISETB,YDCTX%ISETWL(JROC),YDCTX%ISETVL(JROC))
