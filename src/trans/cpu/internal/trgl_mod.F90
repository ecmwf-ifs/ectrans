...
SUBROUTINE TRGL_INIT_VARS(YLVARS, KF_SCALARS_G, &
  ...
  ASSOCIATE(IUVLEVS=>YLVARS%IUVLEVS, IUVPARS=>YLVARS%IUVPARS, &
            IGP2PARS=>YLVARS%IGP2PARS, LLUV=>YLVARS%LLUV, &
            LLGP2=>YLVARS%LLGP2, LLGP3A=>YLVARS%LLGP3A, &
            LLGP3B=>YLVARS%LLGP3B, IGP3APARS=>YLVARS%IGP3APARS, &
            IGP3ALEVS=>YLVARS%IGP3ALEVS, IGP3BPARS=>YLVARS%IGP3BPARS, &
...
SUBROUTINE TRGL_INIT_OFF_VARS(YDBUFS,YLVARS,KVSET,KPTRGP,KF_GP)
  ...
  ASSOCIATE(KGPTRSEND =>YDBUFS%IGPTRSEND,IFLDS=>YDBUFS%IFLDS)
  ASSOCIATE(IFLDOFF=>YLVARS%IFLDOFF, IGPTROFF=>YLVARS%IGPTROFF, LLINDER=>YDBUFS%LLINDER)
  IFLDS = 0
  DO JFLD=1,KF_GP
...
SUBROUTINE TGRL_INIT_PACKING_VARS(YDBUFS,YLVARS, KVSET, KF_GP, PCOMBUFS)
  ...
  ASSOCIATE (KGPTRSEND =>YDBUFS%IGPTRSEND,IPOSPLUS=>YDBUFS%IPOSPLUS, IJPOS=>YDBUFS%IJPOS, IFLDA=>YDBUFS%IFLDA, ISETW=>YDBUFS%ISETW)
  ...
  !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(INRS,IV,ISETV,JBLK,IFIRST,ILAST,&
  !$OMP&                                     IFLD,IPOS,JFLD)
  DO INRS=1,KINRS
    IF (YDBUFS%LLTRGTOL) THEN
...
SUBROUTINE TGRL_COPY_ZCOMBUF(YDBUFS, YLVARS, INRS,&
                      & ZCOMBUF, &
                      & KPTRGP,&
                      & PGP, PGPUV, PGP3A, PGP3B, PGP2)
  ...
  ASSOCIATE (KSENDCOUNT=>YDBUFS%ISENDCOUNT, KRECVCOUNT=>YDBUFS%IRECVCOUNT, KNSEND=>YDBUFS%INSEND, KNRECV=>YDBUFS%INRECV, &
          ...
          & KNDOFF=>YDBUFS%INDOFF, KGPTRSEND =>YDBUFS%IGPTRSEND,&
          & IFLDA=>YDBUFS%IFLDA,IPOSPLUS=>YDBUFS%IPOSPLUS, JPOS=>YDBUFS%IJPOS, ISETW=>YDBUFS%ISETW,LLPGPONLY=>YDBUFS%LLPGPONLY, &
          & LLINDER=>YDBUFS%LLINDER)
...
SUBROUTINE TGRL_COPY_PGLAT(PGLAT, YDBUFS, YLVARS, &
                    & PGP, PGPUV, PGP3A, PGP3B, PGP2)
  ...
  ASSOCIATE(IUVLEVS=>YLVARS%IUVLEVS,IFLDOFF=>YLVARS%IFLDOFF,&
            IGPTROFF=>YLVARS%IGPTROFF,&
            IUVPARS=>YLVARS%IUVPARS, &
            IGP2PARS=>YLVARS%IGP2PARS, LLUV=>YLVARS%LLUV, &
            LLGP2=>YLVARS%LLGP2, LLGP3A=>YLVARS%LLGP3A, &
            LLGP3B=>YLVARS%LLGP3B, IGP3APARS=>YLVARS%IGP3APARS, &
            IGP3ALEVS=>YLVARS%IGP3ALEVS, IGP3BPARS=>YLVARS%IGP3BPARS, &
            IGP3BLEVS=>YLVARS%IGP3BLEVS)
  ASSOCIATE (KSENDCOUNT=>YDBUFS%ISENDCOUNT, KRECVCOUNT=>YDBUFS%IRECVCOUNT, KNSEND=>YDBUFS%INSEND, &
            KNRECV=>YDBUFS%INRECV, KSENDTOT=>YDBUFS%ISENDTOT, KRECVTOT=>YDBUFS%IRECVTOT, &
            KSEND=>YDBUFS%ISEND, KRECV=>YDBUFS%IRECV, KINDEX=>YDBUFS%IINDEX, KNDOFF=>YDBUFS%INDOFF, &
            KGPTRSEND =>YDBUFS%IGPTRSEND,IFLDS=>YDBUFS%IFLDS,LLPGPONLY=>YDBUFS%LLPGPONLY, &
            LLINDER=>YDBUFS%LLINDER)
...
YDBUFS%IRECVCOUNT=0
DO J=1,NPROC
  YDBUFS%ISENDCOUNT=MAX(YDBUFS%ISENDCOUNT,YDBUFS%ISENDTOT(J))
  YDBUFS%IRECVCOUNT=MAX(YDBUFS%IRECVCOUNT,YDBUFS%IRECVTOT(J))
ENDDO
