...
IF (KIFC > 0 .AND. KDGLU > 0 ) THEN
  ...
  IF (ILA <= S%ITHRESHOLD .OR. .NOT. S%LUSEFLT) THEN
    IF (LLDOUBLE) THEN
      DO IFLD = 1, KIFC
        PAIA(:, IFLD) = PAIA(:, IFLD) * PW(KSL:KSL + KDGLU - 1)
      ENDDO
    ...
    ELSE
      ...
      ELSE
        BLOCK
          ...
          DO IFLD = 1, KIFC
            ZAIA_D(:, IFLD) = REAL(PAIA(:, IFLD), JPRD) * PW(KSL:KSL + KDGLU - 1)
          ENDDO
  ...
  IF (ILS <= S%ITHRESHOLD .OR. .NOT. S%LUSEFLT) THEN
    IF (LLDOUBLE) THEN
      DO IFLD = 1, KIFC
        PSIA(:, IFLD) = PSIA(:, IFLD) * PW(KSL:KSL + KDGLU - 1)
      ENDDO
    ...
    ELSE
      ...
      ELSE
        BLOCK
          ...
          DO IFLD = 1, KIFC
            ZSIA_D(:, IFLD) = REAL(PSIA(:, IFLD), JPRD) * PW(KSL:KSL + KDGLU - 1)
          ENDDO
