...
IF (KIFC > 0 .AND. KDGLU > 0 ) THEN
  ...
  IF (ILA <= S%ITHRESHOLD .OR. .NOT. S%LUSEFLT) THEN
    IF (LLDOUBLE) THEN
      CALL PACK_FOR_GEMM(KIFC, KDGLU, KSL, PAIA, PW, ZB)
    ...
    ELSE
      ...
      ELSE
        BLOCK
          ...
          CALL PACK_FOR_GEMM_JPRD(KIFC, KDGLU, KSL, PAIA, PW, ZB_D)
  ...
  IF (ILS <= S%ITHRESHOLD .OR. .NOT. S%LUSEFLT) THEN
    IF (LLDOUBLE) THEN
      CALL PACK_FOR_GEMM(KIFC, KDGLU, KSL, PSIA, PW, ZB)
    ...
    ELSE
      ...
      ELSE
        BLOCK
          ...
          CALL PACK_FOR_GEMM_JPRD(KIFC, KDGLU, KSL, PSIA, PW, ZB_D)
...
SUBROUTINE PACK_FOR_GEMM(KIFC, KDGLU, KSL, PIN, PW, POUT)
...
DO IFLD = 1, KIFC
  DO J = 1, KDGLU
    POUT(J, IFLD) = REAL(REAL(PIN(J, IFLD), JPRD) * PW(KSL + J - 1), JPRB)
  ENDDO
ENDDO
