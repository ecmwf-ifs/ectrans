...
IF (KIFC > 0 .AND. KDGLU > 0 ) THEN
  ...
  IF (ILA <= S%ITHRESHOLD .OR. .NOT. S%LUSEFLT) THEN
    IF (LLDOUBLE) THEN
      CALL PACK_FOR_GEMM(KIFC, KDGLU, KSL, PAIA, PW)
    ...
    ELSE
      ...
      ELSE
        BLOCK
          ...
          CALL PACK_FOR_GEMM_JPRD(KIFC, KDGLU, KSL, PAIA, PW, ZAIA_D)
  ...
  IF (ILS <= S%ITHRESHOLD .OR. .NOT. S%LUSEFLT) THEN
    IF (LLDOUBLE) THEN
      CALL PACK_FOR_GEMM(KIFC, KDGLU, KSL, PSIA, PW)
    ...
    ELSE
      ...
      ELSE
        BLOCK
          ...
          CALL PACK_FOR_GEMM_JPRD(KIFC, KDGLU, KSL, PSIA, PW, ZSIA_D)
  ...
  ELSE
    ...
    CALL MULT_BUTM('T', S%FA(KMLOC)%YBUT_STRUCT_S, KIFC, ZSIA, ZCS, KM)
...
SUBROUTINE PACK_FOR_GEMM(KIFC, KDGLU, KSL, PIN, PW)
...
INTEGER(KIND=JPIM) :: IFLD, J

DO IFLD = 1, KIFC
  DO J = 1, KDGLU
    PIN(J, IFLD) = REAL(REAL(PIN(J, IFLD), JPRD) * PW(KSL + J - 1), JPRB)
  ENDDO
ENDDO

END SUBROUTINE PACK_FOR_GEMM

SUBROUTINE PACK_FOR_GEMM_JPRD(KIFC, KDGLU, KSL, PIN, PW, POUT)
...
DO IFLD = 1, KIFC
  DO J = 1, KDGLU
    POUT(J, IFLD) = REAL(PIN(J, IFLD), JPRD) * PW(KSL + J - 1)
  ENDDO
ENDDO
