...
USE TRXTOY_CTX_MOD  ,ONLY : TRXTOY_CTX, ALLOCATE_CTX, TRXTOY_PROLOG
...
CALL GSTATS(1806,0)
CALL TRXTOY_PROLOG(KF_FS,KF_GP,KVSET,YDCTX, .TRUE.)  ! TRUE. for LSEND
CALL GSTATS(1806,1)
...
CALL ALLOCATE_TOG_VARS(YDTOGLVARS, KF_GP)
...
IF(KSENDTOT(MYPROC) > 0 )THEN
  ...
  CALL INIT_TOG_OFF_VARS(YDCTX,YDTOGLVARS,KVSET,KPTRGP,KF_GP)

  CALL GSTATS(1604,0)

  CALL COPY_PGLAT(PGLAT, YDCTX, YDTOGLVARS, &
                 & PGP, PGPUV,PGP3A, PGP3B,PGP2)
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(INR,IRECV,ISETV)
DO INR=1,KNRECV
  
  IRECV=KRECV(INR)
  
  YDCTX%ISETW(INR)=KSETWL(IRECV)
  ISETV=KSETVL(IRECV)
  CALL INIT_TOG_PACKING_VARS(YDCTX,YDTOGLVARS, INR, ISETV, KVSET, KF_GP)
ENDDO
!$OMP END PARALLEL DO
...
DO JNR=1,KNRECV
  ...
  CALL COPY_ZCOMBUF(YDCTX, YDTOGLVARS, INR, &
                      &  PCOMBUFR, &
                      & KPTRGP, &
                      & PGP, PGPUV, PGP3A, PGP3B, PGP2) 
ENDDO
