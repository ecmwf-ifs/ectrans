...
CALL ALLOCATE_CTX(YDCTX, NPROC,D%NLENGTF,NGPBLKS,NPRTRNS,KF_GP)
...
IF(KSENDTOT(MYPROC) > 0 )THEN
  ...
  CALL COPY_PGLAT(YDCTX, YDTOGLVARS, NGPBLKS,IFLDS, LLPGPONLY, LLINDER,&                    
...
DO INR=1,KNRECV
  ...
  ISETV=KSETVL(IRECV)
  CALL INIT_TOG_PACKING_VARS(YDCTX,YDTOGLVARS, INR, ISETV, KVSET, NGPBLKS,KF_GP)
ENDDO
!$OMP END PARALLEL DO
