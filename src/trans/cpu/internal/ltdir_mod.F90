...
MODULE LTDIR_MOD
CONTAINS
SUBROUTINE LTDIR(KM, KMLOC, KF_UV, KF_SCALARS, KF_FS_CMPLX, PSPVOR, PSPDIV, PSPSCALAR, PSPSC3A, &
  &              PSPSC3B, PSPSC2, KFLDPTRUV,KFLDPTRSC)
...
INTEGER(KIND=JPIM),INTENT(IN)   :: KF_FS_CMPLX, KF_UV, KF_SCALARS
...
REAL(KIND=JPRB) :: ZOA1(R%NLED4,KF_FS_CMPLX),         ZOA2(R%NLED4,MAX(4*KF_UV,1))
...
! Zeroth mode is all real
IIFC = MERGE(KF_FS_CMPLX / 2, KF_FS_CMPLX, KM == 0)

IDGLU = MIN(R%NDGNH,G%NDGLU(KM))
ISL = MAX(R%NDGNH-G%NDGLU(KM)+1,1)

ALLOCATE(ZSIA(IIFC,IDGLU))
ALLOCATE(ZAIA(IIFC,IDGLU))
...
IF( LATLON.AND.S%LDLL ) THEN
  ...
  IF( (S%LSHIFTLL .AND. KM < 2*IDGLU) .OR.&
   & (.NOT.S%LSHIFTLL .AND. KM < 2*(IDGLU-1)) ) THEN
    ...
    CALL CDMAP(KM, KMLOC, ISL, ISLO, ZEPSNM(R%NTMAX+1), 1_JPIM, R%NDGNH, S%NDGNHD, IIFC, &
      &        ZAIA, ZSIA)
...
ELSE
  ...
  CALL PRFI2(KM, KMLOC, KF_FS_CMPLX, ZAIA, ZSIA)

ENDIF

CALL LDFOU2(KM, MERGE(2 * KF_UV, 4 * KF_UV, KM == 0), ZAIA, ZSIA)

CALL LEDIR(KM, KMLOC, KF_FS_CMPLX, IIFC, ISL, IDGLU, ZAIA, ZSIA, ZOA1, F%RW(1:R%NDGNH))
