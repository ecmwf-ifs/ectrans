...
IF(NPROMATR > 0 .AND. KF_GP > NPROMATR) THEN
 ...
 & KVSETUV,KVSETSC)
  ...
  DO WHILE (NDONE < IBLKS)
    WRITE(NOUT,*) "Processing block", JBLK
    ...
    WRITE(NOUT,*) "IF_GP = ", IF_GP
    WRITE(NOUT,*) "IF_FS = ", IF_FS
    ...
    IF(IF_UV_G > 0 .AND. IF_SCALARS_G > 0) THEN
      CALL FTDIR_CTL_COMM_SEND(IF_UV_G,IF_SCALARS_G,IF_GP,IF_FS,&
