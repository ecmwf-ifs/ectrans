...
REAL(KIND=JPRB), ALLOCATABLE :: PGTF(:,:)
...
IF (NPROMATR > 0 .AND. KF_GP > NPROMATR) THEN
  ...
  IF (ALLOCATED(PGTF) .AND. SIZE(PGTF, 1) /= KF_FS .AND. SIZE(PGTF, 2) /= (D%NLENGTF)) THEN
    DEALLOCATE(PGTF)
  ENDIF
  IF (.NOT. ALLOCATED(PGTF)) THEN
    ALLOCATE(PGTF(KF_FS,D%NLENGTF))
  ENDIF
  ...
  CALL ACTIVATE(JBLK, KF_GP, KF_SCALARS_G, KF_UV_G, KVSETUV, KVSETSC, PGP, PGTF)
    ...
    DO WHILE (ASSOCIATED(IB))
      ...
      TYPE IS (BATCH)
        IF (THISBATCH%COMM_COMPLETE()) THEN
          CALL THISBATCH%FINISH_COMM(PGTF, PSPVOR, PSPDIV, PSPSCALAR)
