  ...
  DO WHILE (NDONE < IBLKS)
    ...
    IF (NEW_BATCH%NF_UV_G > 0 .AND. NEW_BATCH%NF_SCALARS_G > 0) THEN
      CALL FTDIR_CTL_COMM_SEND(NEW_BATCH%NF_UV_G, NEW_BATCH%NF_SCALARS_G, NEW_BATCH%NF_GP, NEW_BATCH%NF_FS, &
        &                      KVSETUV=NEW_BATCH%NVSETUV(NEW_BATCH%NSTUV_G:NEW_BATCH%NENUV_G), KVSETSC=NEW_BATCH%NVSETSC(NEW_BATCH%NSTSC_G:NEW_BATCH%NENSC_G), &
        &                      KPTRGP=NEW_BATCH%NPTRGP, PGP=PGP)
    ELSEIF (NEW_BATCH%NF_UV_G > 0) THEN
      CALL FTDIR_CTL_COMM_SEND(NEW_BATCH%NF_UV_G, NEW_BATCH%NF_SCALARS_G, NEW_BATCH%NF_GP, NEW_BATCH%NF_FS, &
        &                      KVSETUV=NEW_BATCH%NVSETUV(NEW_BATCH%NSTUV_G:NEW_BATCH%NENUV_G), KPTRGP=NEW_BATCH%NPTRGP, PGP=PGP)
    ELSEIF (NEW_BATCH%NF_SCALARS_G > 0) THEN
      CALL FTDIR_CTL_COMM_SEND(NEW_BATCH%NF_UV_G, NEW_BATCH%NF_SCALARS_G, NEW_BATCH%NF_GP, NEW_BATCH%NF_FS, &
        &                      KVSETSC=NEW_BATCH%NVSETSC(NEW_BATCH%NSTSC_G:NEW_BATCH%NENSC_G), KPTRGP=NEW_BATCH%NPTRGP, PGP=PGP)
    ENDIF
    ...
    CALL LTDIR_CTL(NEW_BATCH%NF_FS, NEW_BATCH%NF_UV, NEW_BATCH%NF_SCALARS, PSPVOR=PSPVOR, PSPDIV=PSPDIV, PSPSCALAR=PSPSCALAR, &
      &            KFLDPTRUV=NEW_BATCH%NPTRSPUV, KFLDPTRSC=NEW_BATCH%NPTRSPSC)
