...
MODULE LEDIRAD_MOD
CONTAINS
SUBROUTINE LEDIRAD(KM,KMLOC,KFC,KIFC,KDGLU,PAIA,PSIA,POA1)
...
!*       1. ANTISYMMETRIC PART.
  ...
  DO JK=1,KFC,ISKIP
    IFLD=IFLD+1
    DO J=1,KDGLU
      PAIA(JK,J) = ZB(J,IFLD)*REAL(F%RW(ISL+J-1),JPRB)
    ENDDO
  ENDDO
...
!*       1.3      SYMMETRIC PART.
  ...
  IF(ILS <= ITHRESHOLD .OR. .NOT.S%LUSEFLT) THEN
    ...
    CALL GEMM('N','N',KDGLU,KIFC,ILS,1.0_JPRB,S%FA(KMLOC)%RPNMS,KDGLU,ZCS,ILS,0._JPRB,ZB,KDGLU)
  ...
  DO JK=1,KFC,ISKIP
    IFLD=IFLD+1
    DO J=1,KDGLU
      PSIA(JK,J) = ZB(J,IFLD)*REAL(F%RW(ISL+J-1),JPRB)
    ENDDO
  ENDDO
