...
DO JMLOC = 1, D%NUMP, NPRTRV
  ...
  DO JSETV = 1, IPRTRV
    ...
    IF (S%LUSEFLT .AND. ILA > S%ITHRESHOLD) THEN
      IBYTES_TO_WRITE = IBYTES_TO_WRITE + JPIBUFL * IIBYTES + SIZE(YLCLONE%COMMSBUF) * IRBYTES
    ELSE
      IBYTES_TO_WRITE = IBYTES_TO_WRITE + IDGLU * ILA * IRBYTES
    ENDIF
    IF (S%LUSEFLT .AND. ILS > S%ITHRESHOLD) THEN
      IBYTES_TO_WRITE = IBYTES_TO_WRITE + JPIBUFL * IIBYTES + SIZE(YLCLONE%COMMSBUF) * IRBYTES
    ELSE
      IBYTES_TO_WRITE = IBYTES_TO_WRITE + IDGLU * ILS * IRBYTES
    ENDIF
  ENDDO
ENDDO
...
IF (S%LDLL) THEN
  DO JMLOC=1,D%NUMP
    IBYTES_TO_WRITE = IBYTES_TO_WRITE + JPIBUFL * IIBYTES
    IBYTES_TO_WRITE = IBYTES_TO_WRITE + 4 * IDGLU * IRBYTES + 4 * IDGLU2 * IRBYTES
  ENDDO
ENDIF

IBYTES_TO_WRITE = IBYTES_TO_WRITE + 2 * R%NDGNH * IIBYTES
