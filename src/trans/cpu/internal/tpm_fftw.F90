...
IF( TW%FFTW_PLANS(KN)%NPLAN_ID /= NPLAN_ID_UNINITIALISED )THEN
  ...
  CALL ABOR1('CREATE_PLAN_FFTW.1: NPLAN_ID /= NPLAN_ID_UNINITIALISED')
ENDIF
...
IF( CURR_FFTW_PLAN%NPLAN_ID /= NPLAN_ID_UNINITIALISED )THEN
  ...
  CALL ABOR1('CREATE_PLAN_FFTW.2: NPLAN_ID /= NPLAN_ID_UNINITIALISED')
ENDIF
...
DO JL=1,TW%N_PLANS(KN)
  ...
  ELSEIF( JL /= TW%N_PLANS(KN) )THEN
    ...
    IF( CURR_FFTW_PLAN%NPLAN_ID /= NPLAN_ID_UNINITIALISED )THEN
      ...
      CALL ABOR1('CREATE_PLAN_FFTW.3: NPLAN_ID /= NPLAN_ID_UNINITIALISED')
    ENDIF
  ENDIF
ENDDO
IF( .NOT.LLFOUND )THEN
...
IF ( (KTYPE /= NTYPE_R2C) .AND. (KTYPE /= NTYPE_C2R) ) THEN
  CALL ABOR1('TPM_FFTW:'//TRIM(CDNAME)//' : WRONG VALUE KTYPE')
ENDIF
