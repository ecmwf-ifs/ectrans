...
!     W. Deconinck  17-Jun-2024 Replace legacy FFTW interface with the FFTW3 interface.
...
!     WRITE(*,'("CREATE_PLAN_FFTW: END: DESTROYING A PLAN AT THE START OF THE LIST")')
  ...
  ELSEIF( KTYPE==-1 )THEN
    ...
    IPLAN=TPM_FFTW_PLAN_MANY_DFT_R2C(IRANK,IN,KLOT,ZDUM,IEMBED,ISTRIDE,IRDIST,&
         & CDUM,CEMBED,ISTRIDE,ICDIST,FFTW_ESTIMATE+FFTW_NO_SIMD)
...
COMPLEX(KIND=JPCB), POINTER :: CFFT1(:)
TYPE(C_PTR) :: ZFFTP, ZFFT1P

INTEGER(KIND=JPIM) :: JJ,JF

TYPE(C_PTR) :: IPLAN_C2R, IPLAN_C2R1
REAL(KIND=JPHOOK) :: ZHOOK_HANDLE, ZHOOK_HANDLE2
...
IF( LD_ALL )THEN
  ...
  ZFFTP=TPM_FFTW_ALLOC_COMPLEX(INT(KCLEN/2*KFIELDS,C_SIZE_T))
  CALL C_F_POINTER(ZFFTP,ZFFT,[KCLEN,KFIELDS])
  CALL C_F_POINTER(ZFFTP,CFFT,[KCLEN/2,KFIELDS])
  IF (KTYPE==1) THEN
    DO JF=1,KFIELDS
      DO JJ=1,KCLEN
    ...
    CALL TPM_FFTW_EXECUTE_DFT_C2R(IPLAN_C2R,CFFT,ZFFT)
  ...
  ELSE
    ...
    CALL TPM_FFTW_EXECUTE_DFT_R2C(IPLAN_C2R,ZFFT,CFFT)
...
ELSE
  ...
  ZFFT1P=TPM_FFTW_ALLOC_COMPLEX(INT(KCLEN/2,C_SIZE_T))
  CALL C_F_POINTER(ZFFT1P,ZFFT1,[KCLEN])
  CALL C_F_POINTER(ZFFT1P,CFFT1,[KCLEN/2])
  IF (KTYPE==1) THEN
    DO JF=1,KFIELDS
      DO JJ=1,KCLEN
      ...
      ENDDO
      IF (LHOOK) CALL DR_HOOK('FFTW_EXECUTE_DFT_C2R',0,ZHOOK_HANDLE2)
      CALL TPM_FFTW_EXECUTE_DFT_C2R(IPLAN_C2R1,CFFT1,ZFFT1)
      IF (LHOOK) CALL DR_HOOK('FFTW_EXECUTE_DFT_C2R',1,ZHOOK_HANDLE2)
      DO JJ=1,KRLEN
  ...
  ELSE
    DO JF=1,KFIELDS
      ...
      ENDDO
      IF (LHOOK) CALL DR_HOOK('FFTW_EXECUTE_DFT_R2C',0,ZHOOK_HANDLE2)
      CALL TPM_FFTW_EXECUTE_DFT_R2C(IPLAN_C2R1,ZFFT1,CFFT1)
      IF (LHOOK) CALL DR_HOOK('FFTW_EXECUTE_DFT_R2C',1,ZHOOK_HANDLE2)
      DO JJ=1,KCLEN
