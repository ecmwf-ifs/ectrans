...
INTEGER(KIND=JPIM) :: IGLS,  ISL, ISTAN, ISTAS, JF, JGL, IFLD, ISKIP
...
!OCL    NOVREC
  IFLD = 1
  DO JF = 1, KF_FS_CMPLX, ISKIP
    PSIA(IFLD,JGL - ISL + 1) = FOUBUF(ISTAN+JF)+FOUBUF(ISTAS+JF)
    PAIA(IFLD,JGL - ISL + 1) = FOUBUF(ISTAN+JF)-FOUBUF(ISTAS+JF)
    IFLD = IFLD + 1
  ENDDO
ENDDO
