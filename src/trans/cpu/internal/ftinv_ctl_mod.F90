...
USE TPM_GEN         ,ONLY : NERR   ,NSTACK_MEMORY_TR, LSPOOF_COMMS
...
IF (LSPOOF_COMMS) THEN
  IF (PRESENT(PGP)) PGP(:,:,:) = 0.0_JPRB
  IF (PRESENT(PGPUV)) PGPUV(:,:,:,:) = 0.0_JPRB
  IF (PRESENT(PGP3A)) PGP3A(:,:,:,:) = 0.0_JPRB
  IF (PRESENT(PGP3B)) PGP3B(:,:,:,:) = 0.0_JPRB
  IF (PRESENT(PGP2)) PGP2(:,:,:) = 0.0_JPRB
ELSE
