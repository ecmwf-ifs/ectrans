...
SUBROUTINE ALLOCATE_TOG_VARS(SELF, KF_GP, NGPBLKS, INSEND)
  CLASS(TRXTOY_VARS), INTENT(INOUT) :: SELF
  INTEGER(KIND=JPIM), INTENT(IN) :: KF_GP
  INTEGER(KIND=JPIM), INTENT(IN) :: NGPBLKS
  INTEGER(KIND=JPIM), INTENT(IN) :: INSEND
  ...
  ALLOCATE(SELF%IJPOS(NGPBLKS,INSEND))
  ALLOCATE(SELF%IPOSPLUS(INSEND))
  ALLOCATE(SELF%IFLDA(KF_GP,INSEND))
...
SUBROUTINE INIT_TOG_PACKING_VARS(YDCTX,YDTOGLVARS, INRS, ISETV, KVSET, NGPBLKS,KF_GP, ISETW, PCOMBUFS)
  USE PARKIND1  ,ONLY : JPIM, JPRB
  ...
  INTEGER(KIND=JPIM), INTENT(IN) :: ISETV
  INTEGER(KIND=JPIM), INTENT(IN) :: KVSET(:)
  INTEGER(KIND=JPIM), INTENT(IN) :: NGPBLKS
  INTEGER(KIND=JPIM), INTENT(IN) :: KF_GP
  ...
  ASSOCIATE (KGPTRSEND =>YDCTX%IGPTRSEND)
  ASSOCIATE(IPOSPLUS=>YDTOGLVARS%IPOSPLUS, IJPOS=>YDTOGLVARS%IJPOS, IFLDA=>YDTOGLVARS%IFLDA)
  IFLD = 0
  IPOS = 0
  IPOSPLUS(INRS)=0
  DO JFLD=1,KF_GP
...
SUBROUTINE COPY_ZCOMBUF(LSEND, YDCTX, YDTOGLVARS, INRS,&
  ...
  ASSOCIATE(IUVLEVS=>YDTOGLVARS%IUVLEVS, IUVPARS=>YDTOGLVARS%IUVPARS, &
            ...
            LLGP3B=>YDTOGLVARS%LLGP3B, IGP3APARS=>YDTOGLVARS%IGP3APARS, &
            IGP3ALEVS=>YDTOGLVARS%IGP3ALEVS, IGP3BPARS=>YDTOGLVARS%IGP3BPARS, &
            IGP3BLEVS=>YDTOGLVARS%IGP3BLEVS, IFLDA=>YDTOGLVARS%IFLDA,IPOSPLUS=>YDTOGLVARS%IPOSPLUS, JPOS=>YDTOGLVARS%IJPOS)
