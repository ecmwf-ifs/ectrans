...
IF (NSTACK_MEMORY_TR == 0) THEN
  CALL TRGL_ALLOCATE_HEAP_BUFFER(PCOMBUFS_HEAP, YDCTX%ISENDCOUNT, YDCTX%INSEND)
  CALL TRGL_ALLOCATE_HEAP_BUFFER(PCOMBUFR_HEAP, YDCTX%IRECVCOUNT, YDCTX%INRECV)
...
CALL TRGL_ALLOCATE_VARS(YDTOGLVARS, KF_GP)
CALL TRGL_INIT_VARS(YDTOGLVARS, KF_SCALARS_G, &
                   & PGP, PGPUV, PGP3A, PGP3B, PGP2)
...
IF(KSENDTOT(MYPROC) > 0 )THEN
  ...
  CALL TRGL_INIT_OFF_VARS(YDCTX,YDTOGLVARS,KVSET,KPTRGP,KF_GP)
  
  CALL GSTATS(1601,0)
  
  CALL TGRL_COPY_PGLAT(PGLAT, YDCTX, YDTOGLVARS, &
                    & PGP, PGPUV, PGP3A, PGP3B, PGP2)
...
CALL TGRL_INIT_PACKING_VARS(.TRUE., YDCTX,YDTOGLVARS, KVSET, KF_GP, PCOMBUFS) !.TRUE. for LSEND

DO INS=1,KNSEND
  
  CALL TGRL_COPY_ZCOMBUF(YDCTX, YDTOGLVARS, INS, &
                      &  PCOMBUFS, & 
                      & KPTRGP, &
                      PGP, PGPUV, PGP3A, PGP3B, PGP2)
