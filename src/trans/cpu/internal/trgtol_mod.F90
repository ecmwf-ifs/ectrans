...
CALL TRXTOY_PROLOG(KF_FS,KF_GP,KVSET,YDCTX, .FALSE.)  ! .FALSE. for LSEND
...
DO INS=1,KNSEND
  ...
  ISEND=KSEND(INS)  
  YDCTX%ISETW(INS)=KSETWL(ISEND)
  ISETV=KSETVL(ISEND)

  CALL INIT_TOG_PACKING_VARS(YDCTX,YDTOGLVARS, INS, ISETV, KVSET, KF_GP, PCOMBUFS)

ENDDO
!$OMP END DO
DO INS=1,KNSEND
  
  CALL COPY_ZCOMBUF(YDCTX, YDTOGLVARS, INS, &
                      &  PCOMBUFS, & 
                      & KPTRGP, &
                      PGP, PGPUV, PGP3A, PGP3B, PGP2)
