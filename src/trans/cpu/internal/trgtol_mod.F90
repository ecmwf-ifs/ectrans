...
PUBLIC TRGTOL
PRIVATE TRGTOL_PROLOG, TRGTOL_COMM, TRGTOL_COMM_SEND, TRGTOL_COMM_RECV
...
REAL(KIND=JPRB), ALLOCATABLE, SAVE :: ZCOMBUFS(:,:)
REAL(KIND=JPRB), ALLOCATABLE, SAVE :: ZCOMBUFR(:,:)
...
CALL TRGTOL_PROLOG(KF_FS,KF_GP,KVSET,&
 ...
 IF ( .NOT. ALLOCATED(ZCOMBUFS) ) THEN
  ALLOCATE(ZCOMBUFS(-1:ISENDCOUNT,INSEND))
  ISENDCOUNT_PREV = ISENDCOUNT
  INSEND_PREV = INSEND
ELSEIF ( ISENDCOUNT .NE. ISENDCOUNT_PREV .OR. INSEND .NE. INSEND_PREV ) THEN
  DEALLOCATE(ZCOMBUFS)
  ALLOCATE(ZCOMBUFS(-1:ISENDCOUNT,INSEND))
  ISENDCOUNT_PREV = ISENDCOUNT
  INSEND_PREV = INSEND
...
IF (INSEND > 0 .AND. ISENDCOUNT >= -1) ZCOMBUFS(-1,1) = HUGE(1._JPRB)

IF ( .NOT. ALLOCATED(ZCOMBUFR) ) THEN
  ALLOCATE(ZCOMBUFR(-1:IRECVCOUNT,INRECV))
  IRECVCOUNT_PREV = IRECVCOUNT
  INRECV_PREV = INRECV
ELSEIF ( IRECVCOUNT .NE. IRECVCOUNT_PREV .OR. INRECV .NE. INRECV_PREV ) THEN
  DEALLOCATE(ZCOMBUFR)
  ALLOCATE(ZCOMBUFR(-1:IRECVCOUNT,INRECV))
  IRECVCOUNT_PREV = IRECVCOUNT
  INRECV_PREV = INRECV
...
CALL TRGTOL_COMM_SEND(PGLAT, KF_FS, KF_GP, KF_SCALARS_G, KVSET, ISENDCOUNT, IRECVCOUNT, INSEND, &
  &                   INRECV, ISENDTOT, IRECVTOT, ISEND, IRECV, IINDEX, INDOFF, IGPTRSEND, &
  &                   ZCOMBUFS, ZCOMBUFR, KPTRGP, PGP, PGPUV, PGP3A, PGP3B, PGP2)

CALL TRGTOL_COMM_RECV(PGLAT, KF_FS, IRECVCOUNT, INRECV, IRECVTOT, IRECV, IINDEX, INDOFF, &
  &                   ZCOMBUFR)
