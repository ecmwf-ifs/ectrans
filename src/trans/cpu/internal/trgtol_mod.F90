...
CALL TRGTOL_PROLOG(KF_FS,KF_GP,KVSET,&
 ...
 IF ( .NOT. ALLOCATED(ZCOMBUFS_HEAP) ) THEN
  ALLOCATE(ZCOMBUFS_HEAP(-1:ISENDCOUNT,INSEND))
...
IF (INSEND > 0 .AND. ISENDCOUNT >= -1) ZCOMBUFS_HEAP(-1,1) = HUGE(1._JPRB)

IF ( .NOT. ALLOCATED(ZCOMBUFR_HEAP) ) THEN
  ALLOCATE(ZCOMBUFR_HEAP(-1:IRECVCOUNT,INRECV))
...
CALL TRGTOL_COMM_SEND(PGLAT, KF_FS, KF_GP, KF_SCALARS_G, KVSET, ISENDCOUNT, IRECVCOUNT, INSEND, &
  ...
  &                   ZCOMBUFS_HEAP, ZCOMBUFR_HEAP, KPTRGP, PGP, PGPUV, PGP3A, PGP3B, PGP2)
