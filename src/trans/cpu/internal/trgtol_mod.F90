...
INTEGER(KIND=JPIM) :: ISEND, ITAG, JBLK, JFLD, JK, JL, IFLD, II, IFLDS, INS, INR, JNR
INTEGER(KIND=JPIM) :: JJ,JI,IFLDT
...
DO INS=1,KNSEND
  ...
  IF (NTRANS_SYNC_LEVEL <= 1) THEN
    CALL MPL_SEND(PCOMBUFS(-1:YDCTX%ISENDTOT(ISEND),INS),KDEST=NPRCIDS(ISEND), &
...
DO JNR=1,KNRECV
  ...
  ELSE
    INR = JNR
    IRECV=KRECV(INR)
    CALL MPL_RECV(PCOMBUFR(-1:YDCTX%IRECVTOT(IRECV),INR), &
          & KSOURCE=NPRCIDS(IRECV), &
