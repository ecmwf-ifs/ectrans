...
! Anti-symmetric
    ...
    ELSE
      IF(C%CIO_TYPE == 'file') THEN
        ...
        IF (ISIZE > 0) S%FA(IMLOC)%RPNMA(:,:) = RESHAPE(ZBUF,(/IDGLU,ILA/))
        DEALLOCATE(ZBUF)
      ELSE
...
! Symmetric
    ...
    ELSE
      IF(C%CIO_TYPE == 'file') THEN
        ...
        IF (ISIZE > 0) S%FA(IMLOC)%RPNMS(:,:) = RESHAPE(ZBUF,(/IDGLU,ILS/))
        DEALLOCATE(ZBUF)
      ELSE
  ...
  IF(CLABEL_16 /= 'LATLON---BEG-BEG')CALL ABORT_TRANS('READ_LEGPOL:WRONG LAT/LON LABEL')
   ...
   DO JMLOC=1,D%NUMP
    ...
    IF(C%CIO_TYPE == 'file') THEN
      ...
      IF (ISIZE > 0) S%FA(JMLOC)%RPNMWI(:,:) = RESHAPE(ZBUF,(/2*IDGLU,2/))
      DEALLOCATE(ZBUF)
      ...
      IF (ISIZE > 0) S%FA(JMLOC)%RPNMWO(:,:) = RESHAPE(ZBUF,(/2*IDGLU2,2/))
      DEALLOCATE(ZBUF)
