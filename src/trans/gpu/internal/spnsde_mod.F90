...
DO KMLOC=1,D%NUMP
  ...
  !$ACC& FIRSTPRIVATE(KM,ISMAX,KMLOC)
#endif
  DO JN=KM-1,ISMAX+2
   IJ = ISMAX+3-JN
   ZN(IJ) = F_RN(JN)
   ...
   ENDIF
  ENDDO
  !$ACC KERNELS DEFAULT(NONE) COPYIN(ISMAX)
  ZN(0) = F_RN(ISMAX+3)
  !$ACC END KERNELS
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IR) FIRSTPRIVATE(KF_SCALARS,KMLOC,ISMAX)
#endif
      DO J=1,KF_SCALARS
        IR = 2*J-1
        DO JI=2,ISMAX+3
 ...
 &           ZN(JI-2)*ZZEPSNM(JI-1)*PF(IR,JI-1,KMLOC)
  ...
  ELSE  
    ...
    !$ACC& FIRSTPRIVATE(KF_SCALARS,KM,KMLOC,ISMAX)
#endif
    DO J=1,KF_SCALARS
      DO JI=2,ISMAX+3-KM
        IR = 2*J-1
