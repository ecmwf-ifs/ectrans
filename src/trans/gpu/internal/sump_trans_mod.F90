...
INTEGER(KIND=JPIB) :: OFFSET1, OFFSET_LEINV_IN, OFFSET_LEDIR_OUT
INTEGER(KIND=JPIB) :: OFFSET_LEINV_LEGPOL, OFFSET_LEDIR_LEGPOL
...
ALLOCATE(D%OFFSETS_LEINV_IN(D%NUMP+1))
ALLOCATE(D%OFFSETS_LEDIR_OUT(D%NUMP+1))
ALLOCATE(D%OFFSETS_LEINV_LEGPOL(D%NUMP+1))
ALLOCATE(D%OFFSETS_LEDIR_LEGPOL(D%NUMP+1))
...
OFFSET_LEINV_IN = 0
OFFSET_LEDIR_OUT = 0
OFFSET_LEINV_LEGPOL = 0
OFFSET_LEDIR_LEGPOL = 0
DO KMLOC=1,D%NUMP
  KM = D%MYMS(KMLOC)
  D%OFFSETS_GEMM1(KMLOC) = OFFSET1
  D%OFFSETS_LEINV_IN(KMLOC) = OFFSET_LEINV_IN
  D%OFFSETS_LEDIR_OUT(KMLOC) = OFFSET_LEDIR_OUT
  D%OFFSETS_LEINV_LEGPOL(KMLOC) = OFFSET_LEINV_LEGPOL
  D%OFFSETS_LEDIR_LEGPOL(KMLOC) = OFFSET_LEDIR_LEGPOL
  ...
  IF (KM /= 0) THEN
    ...
    OFFSET_LEINV_IN = OFFSET_LEINV_IN + ALIGN((R%NSMAX-KM+3)/2,8)
    OFFSET_LEDIR_OUT = OFFSET_LEDIR_OUT + ALIGN((R%NSMAX-KM+3)/2,8)

    D%LEGENDRE_MATRIX_STRIDES(KMLOC) = ALIGN(G%NDGLU(KM),8)
    ! Note that both sizes have to be aligned because we make the GEMMs
    ! multiples of 8
    OFFSET_LEINV_LEGPOL = OFFSET_LEINV_LEGPOL + ALIGN((R%NSMAX-KM+3)/2,8) * D%LEGENDRE_MATRIX_STRIDES(KMLOC)
    OFFSET_LEDIR_LEGPOL = OFFSET_LEDIR_LEGPOL + ALIGN((R%NSMAX-KM+3)/2,8) * D%LEGENDRE_MATRIX_STRIDES(KMLOC)
  ENDIF
ENDDO
D%OFFSETS_GEMM1(D%NUMP+1) = OFFSET1
D%OFFSETS_LEINV_IN(D%NUMP+1) = OFFSET_LEINV_IN
D%OFFSETS_LEDIR_OUT(D%NUMP+1) = OFFSET_LEDIR_OUT
D%OFFSETS_LEINV_LEGPOL(D%NUMP+1) = OFFSET_LEINV_LEGPOL
D%OFFSETS_LEDIR_LEGPOL(D%NUMP+1) = OFFSET_LEDIR_LEGPOL
