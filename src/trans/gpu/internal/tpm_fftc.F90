...
PUBLIC CREATE_PLAN_FFT, DESTROY_PLAN_FFT, DESTROY_ALL_PLANS_FFT, INIT_PLANS_FFT, &
      & FFTC_RESOL, TC, EXECUTE_DIR_FFT, EXECUTE_INV_FFT
...
INTEGER(KIND=JPIM) :: IPLAN, IPLAN_SIZE
...
INTERFACE
  SUBROUTINE CREATE_PLAN_FFTC(KPLAN,KTYPE,KN,KLOT,KSTRIDE,PLAN_SIZE) BIND(C,NAME="create_plan_fftc_")
   USE, INTRINSIC :: ISO_C_BINDING
   INTEGER(C_INT) :: KPLAN
   INTEGER(C_INT) :: KTYPE,KN,KLOT,KSTRIDE,PLAN_SIZE
...
!     WRITE(*,'("CREATE_PLAN_FFT: END: DESTROYING A PLAN AT THE START OF THE LIST")')
    ENDIF
  ENDIF
  CALL CREATE_PLAN_FFTC(IPLAN,KTYPE,KN,KLOT,KSTRIDE,IPLAN_SIZE)
  KPLAN=IPLAN
  TC%N_PLANS(KN)=TC%N_PLANS(KN)+1
