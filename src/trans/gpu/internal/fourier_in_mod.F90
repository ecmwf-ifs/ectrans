...
INTEGER(KIND=JPIM) :: JM,JF,IGLG,ISTA,OFFSET_VAR,IOFF_LAT,KGL,D_NDGL_FS
...
#ifdef ACCGPU
!$ACC DATA PRESENT(G_NLOEN,G_NLOEN_MAX,G_NMEN,D_NPNTGTB0,FOUBUF,PREEL_COMPLEX,D_NSTAGTF) ASYNC(1)
#endif
#ifdef OMPGPU
!$OMP TARGET DATA MAP(PRESENT,ALLOC:G_NMEN,D_MSTABF,D_NPNTGTB0,FOUBUF,PREEL_COMPLEX,D_NSTAGTF)
!$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) PRIVATE(IGLG,ISTA) DEFAULT(NONE) &
!$OMP&       SHARED(IBEG,IEND,IINC,D_NPTRLS,MYSETW,G_NMEN, &
!$OMP&              D_MSTABF,D_NPNTGTB0,PREEL_COMPLEX,D_NSTAGTF,FOUBUF)
#endif
...
#endif
#ifdef ACCGPU
!$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IGLG,IOFF_LAT,ISTA,RET_REAL,RET_COMPLEX) &
!$ACC&              FIRSTPRIVATE(KF_CURRENT,OFFSET_VAR,KF_TOTAL,D_NDGL_FS) ASYNC(1) TILE(32,16,1)
#endif
DO KGL=1,D_NDGL_FS
  DO JF=1,KF_CURRENT
