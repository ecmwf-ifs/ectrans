...
CONTAINS
  FUNCTION PREPARE_TRMTOL(ALLOCATOR, KF_LEG) RESULT(HTRMTOL)
    ...
    HTRMTOL%HPFBUF = RESERVE(ALLOCATOR, int(D%NLENGT0B*2*KF_LEG*STORAGE_SIZE(DUMMY)/8,kind=c_size_t))
  END FUNCTION
...
#define TRMTOL_DTYPE MPI_DOUBLE
    ...
    CALL ASSIGN_PTR(PFBUF, GET_ALLOCATION(ALLOCATOR, HTRMTOL%HPFBUF),&
        & 1_C_SIZE_T, int(D%NLENGT0B*2*KF_LEG*STORAGE_SIZE(PFBUF(1))/8,kind=c_size_t))
