...
IF (KF_UV > 0) THEN
  !stop 'Error: code path not (yet) supported in GPU version'
   ...
   IDIVS = 2*KF_UV+1
   IDIVE = 4*KF_UV
   CALL UPDSPB(KF_UV,POA2,IVORS,PSPVOR,KFLDPTRUV)
   CALL UPDSPB(KF_UV,POA2,IDIVS,PSPDIV,KFLDPTRUV)
ENDIF
...
IF (KF_SCALARS > 0) THEN
  IF(PRESENT(PSPSCALAR)) THEN
    ...
    CALL UPDSPB(KF_SCALARS,POA1,IST,PSPSCALAR,KFLDPTRSC)
  ELSE
    IF(PRESENT(PSPSC2) .AND. NF_SC2 > 0) THEN
      IDIM1 = NF_SC2
      IEND  = IST+2*IDIM1-1
      CALL UPDSPB(IDIM1,POA1,IST,PSPSC2)
      IST=IST+2*IDIM1
    ENDIF
    IF(PRESENT(PSPSC3A) .AND. NF_SC3A > 0) THEN
      ...
      DO J3=1,IDIM3
        IEND = IST+2*IDIM1-1
        CALL UPDSPB(IDIM1,POA1,IST,PSPSC3A(:,:,J3))
        IST=IST+2*IDIM1
      ENDDO
    ENDIF
    IF(PRESENT(PSPSC3B) .AND. NF_SC3B > 0) THEN
      ...
      DO J3=1,IDIM3
        IEND = IST+2*IDIM1-1
        CALL UPDSPB(IDIM1,POA1,IST,PSPSC3B(:,:,J3))
        IST=IST+2*IDIM1
      ENDDO
    ENDIF
  ENDIF
