...
CONTAINS
  FUNCTION PREPARE_LTDIR(ALLOCATOR, KF_FS, KF_UV) RESULT(HLEDIR)
    ...
    CALL LEDIR_STRIDES(KF_FS,IOUT_STRIDES0=IOUT_STRIDES0,IOUT_STRIDES1=IOUT_STRIDES1,&
    ...
    HLEDIR%HOUT_AND_POA = RESERVE(ALLOCATOR, IALLOC_SZ)

  END FUNCTION
  SUBROUTINE LTDIR(ALLOCATOR,HLEDIR,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV,KF_SCALARS,&
  ...
  TYPE(LEDIR_HANDLE), INTENT(IN) :: HLEDIR
  ...
  CALL ASSIGN_PTR(POA1_L, GET_ALLOCATION(ALLOCATOR, HLEDIR%HOUT_AND_POA),&
  ...
  CALL ASSIGN_PTR(POA2_L, GET_ALLOCATION(ALLOCATOR, HLEDIR%HOUT_AND_POA),&
      & IALLOC_POS, IALLOC_SZ)
  CALL C_F_POINTER(C_LOC(POA2_L), POA2, (/ 4*KF_UV, R%NTMAX+3, D%NUMP /))
  IALLOC_POS = IALLOC_POS + IALLOC_SZ
  
  ! ZOUT
  IALLOC_SZ = ALIGN(IOUT_STRIDES1*D%NUMP*SIZEOF(ZOUT(1)),128)
  CALL ASSIGN_PTR(ZOUT, GET_ALLOCATION(ALLOCATOR, HLEDIR%HOUT_AND_POA),&
      & IALLOC_POS, IALLOC_SZ)
  IALLOC_POS = IALLOC_POS + IALLOC_SZ
  
  ! ZOUT0
  IALLOC_SZ = ALIGN(IOUT0_STRIDES1*SIZEOF(ZOUT0(1)),128)
  CALL ASSIGN_PTR(ZOUT0, GET_ALLOCATION(ALLOCATOR, HLEDIR%HOUT_AND_POA),&
      & IALLOC_POS, IALLOC_SZ)
  IALLOC_POS = IALLOC_POS + IALLOC_SZ
