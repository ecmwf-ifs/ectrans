...
SUBROUTINE FSCAD(PREEL_COMPLEX, KF_FS, KF_UV, KF_SCALARS, KUV_OFFSET, &
...
IF (KUV_EWDER_OFFSET >= 0) THEN
  ...
  !DIR& !SHARED(IBEG,IEND,IINC,KF_UV,ILOEN_MAX,OFFSET_VAR,G,D,KF_FS,KUV_OFFSET,KUV_EWDER_OFFSET,F) &
  !DIR& !SHARED(PREEL_COMPLEX) &
  !DIR& !FIRSTPRIVATE(IBEG,IEND,IINC,KF_UV,ILOEN_MAX,OFFSET_VAR,KF_FS,KUV_OFFSET,KUV_EWDER_OFFSET) &
  !DIR& !ASYNC(1)
...
IF (KSCALARS_EWDER_OFFSET > 0) THEN
  !DIR !PARALLEL COLLAPSE(3) DEFAULT(NONE) &
  !DIR& PRIVATE(IGLG,IOFF_LAT,IOFF_SCALARS_EWDER,IOFF_SCALARS,ZACHTE2,KGL,JF,JM) &
  !DIR& !SHARED(IBEG,IEND,IINC,KF_SCALARS,R,ILOEN_MAX,OFFSET_VAR,KSCALARS_EWDER_OFFSET) &
  !DIR& !SHARED(KSCALARS_OFFSET,G,D,KF_FS,F,PREEL_COMPLEX) &
  !DIR& !FIRSTPRIVATE(IBEG,IEND,IINC,KF_SCALARS,OFFSET_VAR,KSCALARS_EWDER_OFFSET,KSCALARS_OFFSET) &
  !DIR& !FIRSTPRIVATE(KF_FS,ILOEN_MAX) &
  !DIR& !ASYNC(1)
  DO KGL=IBEG,IEND,IINC
    DO JF=1,KF_SCALARS
      DO JM=0,ILOEN_MAX/2
...
!DIR !PARALLEL COLLAPSE(3) DEFAULT(NONE) &
!DIR& PRIVATE(IGLG,IOFF_LAT,IOFF_UV,ZACHTE2,KGL,JF,JM) &
!DIR& !SHARED(IBEG,IEND,IINC,KF_UV,R,OFFSET_VAR,G,D,KF_FS,KUV_OFFSET,F,PREEL_COMPLEX) &
!DIR& !FIRSTPRIVATE(IBEG,IEND,IINC,KF_UV,OFFSET_VAR,KUV_OFFSET,KF_FS) &
!DIR& !ASYNC(1)
DO KGL=IBEG,IEND,IINC
  DO JF=1,2*KF_UV
...
IF (KSCALARS_NSDER_OFFSET >= 0) THEN
  ...
  !DIR& !SHARED(PREEL_COMPLEX) &
  !DIR& !FIRSTPRIVATE(IBEG,IEND,IINC,OFFSET_VAR,KF_SCALARS,KSCALARS_NSDER_OFFSET,KF_FS) &
  !DIR& !ASYNC(1)
  DO KGL=IBEG,IEND,IINC
    DO JF=1,KF_SCALARS
      DO JM=0,R_NSMAX !(note that R_NSMAX <= G_NMEN(IGLG) for all IGLG)
