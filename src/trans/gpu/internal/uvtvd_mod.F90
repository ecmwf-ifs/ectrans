...
#ifdef OMPGPU
!$OMP TARGET DATA MAP(ECTRANS_MAP_PRESENT_ALLOC:D,D_MYMS,D_NUMP,R,R_NTMAX,FG,ZEPSNM,PU,PV,PVOR,PDIV)
#endif
#ifdef ACCGPU
!$ACC DATA &
...
!$OMP& MAP(TO:KF_UV) DEFAULT(ECTRANS_OMP_DEFAULT) 
#endif
#ifdef ACCGPU
...
!$OMP& SHARED(D,R,KF_UV,FG,PVOR,PV,PU,PDIV) DEFAULT(ECTRANS_OMP_DEFAULT)
#endif
#ifdef ACCGPU
