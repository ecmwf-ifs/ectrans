...
#ifdef OMPGPU
!$OMP TARGET PARALLEL DO DEFAULT(NONE) SHARED(R_NTMAX,ZN,F)
#endif
#ifdef ACCGPU
...
#ifdef OMPGPU
!$OMP TARGET PARALLEL DO COLLAPSE(2) PRIVATE(KM,IN) DEFAULT(NONE) &
!$OMP& SHARED(D_NUMP,KFIELD,D_MYMS,F,PU,PV)
#endif
#ifdef ACCGPU
!$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(KM,IN) DEFAULT(NONE) &
!$ACC& PRESENT(D_NUMP,KFIELD,D_MYMS,F,PU,PV)
#endif
DO KMLOC=1,D_NUMP
  DO J=1,2*KFIELD
    KM = D_MYMS(KMLOC)
...
#ifdef OMPGPU
!$OMP TARGET PARALLEL DO COLLAPSE(3) PRIVATE(IR,II,IN,KM,ZKM) DEFAULT(NONE) &
...
!$ACC& PRESENT(D_NUMP,R_NTMAX,KFIELD,D_MYMS,PVOR,PV,PU,ZN,PDIV,ZEPSNM)
#endif
DO KMLOC=1,D_NUMP
  DO JN=0,R_NTMAX
    DO J=1,KFIELD
