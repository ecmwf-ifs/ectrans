...
CONTAINS
  FUNCTION PREPARE_TRLTOM(ALLOCATOR, KF_FS) RESULT(HTRLTOM)
    ...
    HTRLTOM%HPFBUF = RESERVE(ALLOCATOR, int(D%NLENGT1B*2*KF_FS*STORAGE_SIZE(DUMMY)/8,kind=c_size_t))
  END FUNCTION
...
#define TRLTOM_DTYPE MPI_DOUBLE
    ...
    CALL ASSIGN_PTR(PFBUF, GET_ALLOCATION(ALLOCATOR, HTRLTOM%HPFBUF),&
        & 1_C_SIZE_T, int(D%NLENGT1B*2*KF_FS*STORAGE_SIZE(PFBUF(1))/8,kind=c_size_t))
