...
#define TRLTOM_DTYPE MPI_REAL8
...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(ECTRANS_MAP_PRESENT_ALLOC:PFBUF,PFBUF_IN)
#endif
#ifdef ACCGPU
...
#ifdef OMPGPU
          !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(ECTRANS_OMP_DEFAULT) SHARED(PFBUF,PFBUF_IN,FROM_RECV,TO_RECV,FROM_SEND,TO_SEND)
    ...
    ELSE
      ...
      CALL GSTATS(1607,0)
#ifdef OMPGPU
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(ECTRANS_OMP_DEFAULT) SHARED(IEND,ISTA,PFBUF_IN,PFBUF)
#endif
#ifdef ACCGPU
