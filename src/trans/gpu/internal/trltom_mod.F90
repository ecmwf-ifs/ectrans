...
INTEGER::IGROUP_MS,IGROUP_WORLD,IRANKS1(NPRTRW),IRANKS2(NPRTRW)
...
IF(NPROC > 1) THEN
  ...
  IF (ILENS(IRANK) > 0) THEN
      ...
      !$ACC KERNELS DEFAULT(NONE) PRESENT(PFBUF,PFBUF_IN) ASYNC(1)
#elif defined(_CRAYFTN)
      !$ACC KERNELS DEFAULT(NONE) &
      !$ACC& COPYIN(FROM_RECV,TO_RECV,FROM_SEND,TO_SEND) &
      !$ACC& PRESENT(PFBUF,PFBUF_IN) ASYNC(1)
#endif
#endif
