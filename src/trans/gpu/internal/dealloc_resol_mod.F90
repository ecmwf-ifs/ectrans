...
ELSE
  ...
  !$DIR !EXIT_DATA !DELETE(R%NDGNH,R%NDGL,R%NTMAX,R%NSMAX,R) !ASYNC(1)
  !$DIR !EXIT_DATA !DELETE(F%RW,F%RACTHE,F%RLAPIN,F) !ASYNC(1)
  !$DIR !EXIT_DATA !DELETE(FG%ZAA0,FG%ZAS0) IF(ALLOCATED(FG%ZAA0)) !ASYNC(1)
  !$DIR !EXIT_DATA !DELETE(FG%ZEPSNM,FG%ZAS,FG%ZAA,FG) !ASYNC(1)
  !$DIR !EXIT_DATA !DELETE(D%NDGL_FS,D%OFFSETS_GEMM2,D%OFFSETS_GEMM1,D%NASM0,D%MSTABF) !ASYNC(1)
  !$DIR !EXIT_DATA !DELETE(D%NPTRLS,D%NPROCL,D%NPROCM,D%NSTAGTF,D%NSTAGT1B,D%NSTAGT0B) !ASYNC(1)
  !$DIR !EXIT_DATA !DELETE(D%NPNTGTB1,D%NPNTGTB0,D%MYMS,D%NUMP,D) !ASYNC(1)
  !$DIR !EXIT_DATA !DELETE(G%NLOEN,G%NMEN,G%NDGLU,G) !ASYNC(1)
  !$DIR !WAIT(1)
