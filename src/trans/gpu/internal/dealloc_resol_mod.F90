...
ELSE
  ...
  _DIR _EXIT_DATA _DELETE(R%NDGNH,R%NDGL,R%NTMAX,R%NSMAX,R) _ASYNC(1)
  _DIR _EXIT_DATA _DELETE(F%RW,F%RACTHE,F%RLAPIN,F) _ASYNC(1)
  _DIR _EXIT_DATA _DELETE(FG%ZAA0,FG%ZAS0) IF(ALLOCATED(FG%ZAA0)) _ASYNC(1)
  _DIR _EXIT_DATA _DELETE(FG%ZEPSNM,FG%ZAS,FG%ZAA,FG) _ASYNC(1)
  _DIR _EXIT_DATA _DELETE(D%NDGL_FS,D%OFFSETS_GEMM2,D%OFFSETS_GEMM1,D%NASM0,D%MSTABF) _ASYNC(1)
  _DIR _EXIT_DATA _DELETE(D%NPTRLS,D%NPROCL,D%NPROCM,D%NSTAGTF,D%NSTAGT1B,D%NSTAGT0B) _ASYNC(1)
  _DIR _EXIT_DATA _DELETE(D%NPNTGTB1,D%NPNTGTB0,D%MYMS,D%NUMP,D) _ASYNC(1)
  _DIR _EXIT_DATA _DELETE(G%NLOEN,G%NMEN,G%NDGLU,G) _ASYNC(1)
  _DIR _WAIT(1)
