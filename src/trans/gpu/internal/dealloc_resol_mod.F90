...
ELSE
  ...
  !$ACC EXIT DATA DELETE(D, D%MYMS, D%NPNTGTB0, D%NPNTGTB1, D%NSTAGT0B, D%NSTAGT1B, D%NSTAGTF) &
  !$ACC&          DELETE(D%NPROCM, D%NPROCL, D%NPTRLS, D%MSTABF, D%NASM0, D%OFFSETS_GEMM1) &
  !$ACC&          DELETE(D%OFFSETS_LEINV_IN, D%OFFSETS_LEDIR_OUT) ASYNC(1)
  !$ACC EXIT DATA DELETE(G, G%NDGLU, G%NMEN, G%NLOEN) ASYNC(1)
!$ACC WAIT(1)
#endif
#ifdef OMPGPU
  !$OMP TARGET EXIT DATA MAP(DELETE: R%NDGNH, R%NDGL, R%NTMAX, R%NSMAX, R)
  !$OMP TARGET EXIT DATA MAP(DELETE: F%RW, F%RACTHE, F%RLAPIN, F)
  !$OMP TARGET EXIT DATA MAP(DELETE: FG%ZAA0,FG%ZAS0) IF(ALLOCATED(FG%ZAA0))
  !$OMP TARGET EXIT DATA MAP(DELETE: FG%ZEPSNM, FG%ZAS, FG%ZAA,FG)
  !$OMP TARGET EXIT DATA MAP(DELETE: D%NDGL_FS, D%OFFSETS_LEINV_IN, D%OFFSETS_LEDIR_OUT)
  !$OMP TARGET EXIT DATA MAP(DELETE: D%OFFSETS_GEMM1, D%NASM0,D%MSTABF)
  !$OMP TARGET EXIT DATA MAP(DELETE: D%NPTRLS, D%NPROCL, D%NPROCM, D%NSTAGTF, D%NSTAGT1B, D%NSTAGT0B)
  !$OMP TARGET EXIT DATA MAP(DELETE: D%NPNTGTB1, D%NPNTGTB0, D%MYMS, D%NUMP,D)
