...
REAL(KIND=JPRBT), POINTER :: PREEL_COMPLEX_2D(:,:)
...
CALL C_F_POINTER(C_LOC(PREEL_COMPLEX), PREEL_COMPLEX_2D, [2*KF_FS,D%NLENGTF/2])
IFIRST = 0
IF (LVORGP) IFIRST = IFIRST + KF_UV ! Vorticity
IF (LDIVGP) IFIRST = IFIRST + KF_UV ! Divergence
PUV => PREEL_COMPLEX_2D(2*IFIRST+1:2*(IFIRST+2*KF_UV),:)
IFIRST = IFIRST + 2*KF_UV ! U and V
PSCALARS => PREEL_COMPLEX_2D(2*IFIRST+1:2*(IFIRST+KF_SCALARS),:)
IFIRST = IFIRST + KF_SCALARS ! Scalars
IF (LSCDERS) THEN
  PSCALARS_NSDER => PREEL_COMPLEX_2D(2*IFIRST+1:2*(IFIRST+KF_SCALARS),:)
  IFIRST = IFIRST + KF_SCALARS ! Scalars NS Derivatives
ENDIF
IF (LUVDER) THEN
  PUV_EWDER => PREEL_COMPLEX_2D(2*IFIRST+1:2*(IFIRST+2*KF_UV),:)
  IFIRST = IFIRST+2*KF_UV ! U and V derivatives
ENDIF
IF (LSCDERS) THEN
  PSCALARS_EWDER => PREEL_COMPLEX_2D(2*IFIRST+1:2*(IFIRST+KF_SCALARS),:)
