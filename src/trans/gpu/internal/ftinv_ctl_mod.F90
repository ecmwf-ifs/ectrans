...
USE TRLTOG_MOD      ,ONLY : TRLTOG_CUDAAWARE
#else
...
REAL(KIND=JPRB), ALLOCATABLE, INTENT(INOUT) :: FOUBUF(:)
...
REAL(KIND=JPRBT), POINTER :: ZGTF_REAL(:,:)
REAL(KIND=JPRBT), POINTER :: ZGTF_COMPLEX(:,:)
...
! Compute ZGTF_COMPLEX Domain decomposition
IFIRST = 0
...
ALLOCATE(ZGTF_COMPLEX(2*KF_FS, D%NLENGTF/2))
#ifdef OMPGPU
#endif
#ifdef ACCGPU
!$ACC ENTER DATA CREATE(ZGTF_COMPLEX)
#endif
...
IF (LVORGP) IFIRST = IFIRST + KF_UV ! Vorticity
IF (LDIVGP) IFIRST = IFIRST + KF_UV ! Divergence
PUV => ZGTF_COMPLEX(2*IFIRST+1:2*(IFIRST+2*KF_UV),:)
IFIRST = IFIRST + 2*KF_UV ! U and V
PSCALARS => ZGTF_COMPLEX(2*IFIRST+1:2*(IFIRST+KF_SCALARS),:)
IFIRST = IFIRST + KF_SCALARS ! Scalars
IF (LSCDERS) THEN
  PSCALARS_NSDER => ZGTF_COMPLEX(2*IFIRST+1:2*(IFIRST+KF_SCALARS),:)
  IFIRST = IFIRST + KF_SCALARS ! Scalars NS Derivatives
ENDIF
IF (LUVDER) THEN
  PUV_EWDER => ZGTF_COMPLEX(2*IFIRST+1:2*(IFIRST+2*KF_UV),:)
  IFIRST = IFIRST+2*KF_UV ! U and V derivatives
ENDIF
IF (LSCDERS) THEN
  PSCALARS_EWDER => ZGTF_COMPLEX(2*IFIRST+1:2*(IFIRST+KF_SCALARS),:)
  IFIRST = IFIRST + KF_SCALARS ! Scalars EW Derivatives
ENDIF

! from FOUBUF to ZGTF_COMPLEX. Divide by two because we consider this complex space now
CALL FOURIER_IN(FOUBUF,ZGTF_COMPLEX,KF_INPUT/2)

!    2.  Fourier space computations

! fill the rest of ZGTF_COMPLEX
CALL FSC(KF_UV, KF_SCALARS, PUV, PSCALARS, PSCALARS_NSDER, PUV_EWDER, PSCALARS_EWDER)

!   3.  Fourier transform
! inplace operation
IF(KF_FS > 0) THEN
  CALL FTINV(ZGTF_COMPLEX,ZGTF_REAL,KF_FS)
ELSE
  ZGTF_REAL => ZGTF_COMPLEX
ENDIF
...
#endif
CALL TRLTOG_CUDAAWARE(ZGTF_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,IVSET,KPTRGP,&
 &                    PGP,PGPUV,PGP3A,PGP3B,PGP2)
IF (LSYNC_TRANS) THEN
  CALL GSTATS(440,0)
...
CALL TRLTOG(ZGTF_REAL,KF_FS,KF_GP,KF_SCALARS_G,IVSET,KPTRGP,&
 &          PGP,PGPUV,PGP3A,PGP3B,PGP2)
#endif
CALL GSTATS(157,1)
!     ------------------------------------------------------------------
...
#endif
#ifdef ACCGPU
!$ACC EXIT DATA DELETE(ZGTF_REAL)
#endif
DEALLOCATE(ZGTF_REAL)
