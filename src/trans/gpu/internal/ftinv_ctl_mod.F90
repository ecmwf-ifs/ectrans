...
!USE TPM_DIM
!USE TPM_GEOMETRY
USE TPM_TRANS       ,ONLY : LDIVGP, LSCDERS, LUVDER, LVORGP,LATLON, ZGTF
USE TPM_DISTR       ,ONLY : D, MYPROC, NPROC
USE TPM_FLT         ,ONLY : S
USE TPM_FIELDS      ,ONLY : ZGTF_START, ZGTF_START_INDEX_NSDERS, ZGTF_START_INDEX_UVDERS
...
CALL GSTATS(1639,0)
! from FOUBUF to ZGTF
CALL FOURIER_IN(FOUBUF,ZGTF,ZGTF_START(ZGTF_START_INDEX_NSDERS+1)-1)

!    2.  Fourier space computations

! fills rest of data up to ZGTF_START_INDEX_UVDERS
CALL FSC
...
! from ZGTF to ZGTF
IF(KF_FS > 0) THEN
  CALL FTINV(ZGTF,size(zgtf,1),ZGTF_START(ZGTF_START_INDEX_UVDERS+1)-1)
ENDIF
