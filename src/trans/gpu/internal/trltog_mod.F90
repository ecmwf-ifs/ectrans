...
CONTAINS
  FUNCTION PREPARE_TRLTOG(ALLOCATOR,KF_FS,KF_GP) RESULT(HTRLTOG)
    ...
    NELEM = ALIGN(1_JPIB*KF_GP*D%NGPTOT*C_SIZEOF(DUMMY), 128) ! ZCOMBUFR

    HTRLTOG%COMBUFS_START = 1 + NELEM
    NELEM = NELEM + ALIGN(1_JPIB*KF_FS*D%NLENGTF*C_SIZEOF(DUMMY), 128) !ZCOMBUFS upper bound
    
    HTRLTOG%HCOMBUFR_COMBUFS = RESERVE_GAM(ALLOCATOR, NELEM, "HTRLTOG%HCOMBUFR_AND_COMBUFS")
...
#define TRLTOG_DTYPE MPI_REAL8
    ...
    IF (IRECV_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION_GAM(ALLOCATOR, HTRLTOG%HCOMBUFR_COMBUFS),&
          & HTRLTOG%COMBUFR_START, ICOMBUFR_OFFSET(IRECV_COUNTS+1)*C_SIZEOF(ZCOMBUFR(1)))
    ENDIF
    IF (ISEND_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION_GAM(ALLOCATOR, HTRLTOG%HCOMBUFR_COMBUFS),&
          & HTRLTOG%COMBUFS_START, ICOMBUFS_OFFSET(ISEND_COUNTS+1)*C_SIZEOF(ZCOMBUFS(1)))
    ENDIF
