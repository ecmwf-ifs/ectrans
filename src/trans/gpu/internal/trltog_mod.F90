...
MODULE TRLTOG_MOD
 ...
 &                            PGP,PGPUV,PGP3A,PGP3B,PGP2)
  ...
  USE TPM_DISTR       ,ONLY : D, MYSETV, MYSETW, MTAGLG,      &
       &                      NPRCIDS, MYPROC, NPROC,NPRTRW,NPRTRV
  ...
  INTEGER(KIND=JPIM) :: IOFF,IOFF1,IOFFNS,IOFFEW
  INTEGER(KIND=JPIM) :: NDERS
...
#define TRLTOG_DTYPE MPI_DOUBLE_PRECISION
  ...
  !$ACC DATA &
  !$ACC PRESENT(PGLAT) &
  !$ACC COPYIN(IIN_TO_SEND_BUFR,LLUV,LLGP2,LLGP3A,LLGP3B) ASYNC(1)
  !$ACC DATA IF(PRESENT(PGP))   COPYOUT(PGP) ASYNC(1)
  !$ACC DATA IF(PRESENT(PGPUV)) COPYOUT(PGPUV) COPYIN(IUVLEVS,IUVPARS) ASYNC(1)
  !$ACC DATA IF(PRESENT(PGP2))  COPYOUT(PGP2)  COPYIN(IGP2PARS) ASYNC(1)
  !$ACC DATA IF(PRESENT(PGP3A)) COPYOUT(PGP3A) COPYIN(IGP3APARS,IGP3ALEVS) ASYNC(1)
  !$ACC DATA IF(PRESENT(PGP3B)) COPYOUT(PGP3B) COPYIN(IGP3BPARS, IGP3BLEVS) ASYNC(1)
#endif
#ifdef OMPGPU
  !$OMP TARGET DATA &
  !$OMP& MAP(PRESENT,ALLOC:PGLAT) &
  !$OMP& MAP(TO:LLUV,LLGP2,LLGP3A,LLGP3B,KPTRGP)
  ...
  IF(ISENDTOT(MYPROC) > 0) THEN
    ...
    IF (PRESENT(PGP)) THEN
      ...
      !$OMP&     COLLAPSE(2) &
      !$OMP&     SHARED(NGPBLKS,IFLDS,MYSETW,MYPROC, &
      !$OMP&            LLINDER,KPTRGP,PGP,PGLAT)
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS,IOFF) &
      !$ACC&     COLLAPSE(2) &
      !$ACC&     PRESENT(NGPBLKS,IFLDS,MYSETW,MYPROC, &
      !$ACC&            LLINDER,KPTRGP,PGP,PGLAT) ASYNC(1) &
      !$ACC&     FIRSTPRIVATE(IRECV_WSET_SIZE_V,KF_FS,IRECV_WSET_OFFSET_V,NPROMA,IIN_TO_SEND_BUFR_V)
#endif
    ...
    ELSE
      ...
      !$OMP&     COLLAPSE(2) &
      !$OMP&     SHARED(NGPBLKS,IFLDS,MYSETW,MYPROC, &
      !$OMP&            LLUV,PGPUV,IUVLEVS,IUVPARS,LLGP2,PGP2,IGP2PARS, &
      !$OMP&            PGLAT,LLGP3A,PGP3A,IGP3ALEVS,IGP3APARS,LLGP3B,PGP3B,&
      !$OMP&            IGP3BLEVS,IGP3BPARS)
#endif
#ifdef ACCGPU
#if defined(__NVCOMPILER) || defined(__PGI)
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS,IOFF) &
      !$ACC&     COLLAPSE(2) &
      !$ACC&     PRESENT(LLUV,PGPUV,LLGP2,PGP2,IGP2PARS, &
      !$ACC&            PGLAT,LLGP3A,PGP3A,LLGP3B,PGP3B) ASYNC(1)
#elif defined(_CRAYFTN)
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS,IOFF) &
      !$ACC&     COPYIN(IRECV_WSET_SIZE_V,IRECV_WSET_OFFSET_V,KF_FS,NPROMA,IIN_TO_SEND_BUFR_V) COLLAPSE(2) &
      !$ACC&     PRESENT(LLUV,PGPUV,LLGP2,PGP2,IGP2PARS, &
      !$ACC&            PGLAT,LLGP3A,PGP3A,IGP3ALEVS,IGP3APARS,LLGP3B,PGP3B) !!,&
      !!!$ACC&            IUVLEVS,IUVPARS,IGP3BLEVS,IGP3BPARS) ASYNC(1)
#endif
#endif
      DO JL=1,IRECV_WSET_SIZE_V
        DO JFLD=1,KF_FS
          ...
          IF(LLUV(IFLD)) THEN
            PGPUV(JK,IUVLEVS(IFLD),IUVPARS(IFLD),JBLK) = PGLAT(JFLD,IPOS)
          ELSEIF(LLGP2(IFLD)) THEN
            PGP2(JK,IGP2PARS(IFLD),JBLK)=PGLAT(JFLD,IPOS)
          ELSEIF(LLGP3A(IFLD)) THEN
            PGP3A(JK,IGP3ALEVS(IFLD),IGP3APARS(IFLD),JBLK)=PGLAT(JFLD,IPOS)
          ELSEIF(LLGP3B(IFLD)) THEN
            PGP3B(JK,IGP3BLEVS(IFLD),IGP3BPARS(IFLD),JBLK)=PGLAT(JFLD,IPOS)
          ENDIF
        ENDDO
      ENDDO
    ENDIF
  ...
  DO INS=1,ISEND_COUNTS
    ...
    !$OMP&  SHARED(ILEN,KF_FS,ISEND,ZCOMBUFS,INS,PGLAT)
#endif
#ifdef ACCGPU
#if defined(__NVCOMPILER) || defined(__PGI)
    !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(II) COPYIN(ILEN) COLLAPSE(2) &
    !$ACC&  PRESENT(ZCOMBUFS,PGLAT) ASYNC(1)
#elif defined(_CRAYFTN)
    !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(II) COPYIN(ILEN,KF_FS,IIN_TO_SEND_BUFR_V,ICOMBUFS_OFFSET_V) COLLAPSE(2) &
    !$ACC&  PRESENT(ZCOMBUFS,PGLAT) ASYNC(1)
#endif
#endif
    DO JL=1,ILEN
  ...
  DO INR=1,IRECV_COUNTS
    ...
    IF (PRESENT(PGP)) THEN
      ...
      !$OMP&     MAP(TO:INR,KF_FS,IPOS,IFLD,IFLDA) COLLAPSE(3) &
      !$OMP&     SHARED(NGPBLKS,IFLDA,ISETW,IPOS, &
...
#ifdef ACCGPU
#if defined(__NVCOMPILER) || defined(__PGI)
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI) &
      !$ACC&     COPYIN(INR,KF_FS,IPOS,IFLD,IFLDA) COLLAPSE(2) &
      !$ACC&     PRESENT(PGP,KPTRGP,ZCOMBUFR,LLUV,PGPUV, &
      !$ACC&            LLGP2,PGP2,IGP2PARS,LLGP3A,PGP3A,LLGP3B,PGP3B)  ASYNC(1)
#elif defined(_CRAYFTN)
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI) &
      !$ACC&     COPYIN(IFLDA,IRECV_WSET_SIZE_V,IRECV_FIELD_COUNT_V,IRECV_WSET_OFFSET_V,ICOMBUFR_OFFSET_V,NPROMA) COLLAPSE(2) &
      !$ACC&     PRESENT(PGP,KPTRGP,ZCOMBUFR,LLUV,PGPUV, &
      !$ACC&            LLGP2,PGP2,IGP2PARS,LLGP3A,PGP3A,IGP3ALEVS,IGP3APARS, &
      !$ACC&            LLGP3B,PGP3B) ASYNC(1)
#endif
#endif
      DO JL=1,IRECV_WSET_SIZE_V
    ...
    ELSE
      ...
      !$ACC& FIRSTPRIVATE(IRECV_WSET_SIZE_V,IRECV_FIELD_COUNT_V,IRECV_WSET_OFFSET_V,NPROMA,ICOMBUFR_OFFSET_V)
      DO JL=1,IRECV_WSET_SIZE_V
        DO JFLD=1,IRECV_FIELD_COUNT_V
          JK = MOD(IRECV_WSET_OFFSET_V+JL-1,NPROMA)+1
          JBLK = (IRECV_WSET_OFFSET_V+JL-1)/NPROMA+1
          ...
          JI = ICOMBUFR_OFFSET_V+(JFLD-1)*IRECV_WSET_SIZE_V+JL
          IF(LLUV(IFLD)) THEN
            PGPUV(JK,IUVLEVS(IFLD),IUVPARS(IFLD),JBLK) = ZCOMBUFR(JI)
          ELSEIF(LLGP2(IFLD)) THEN
            PGP2(JK,IGP2PARS(IFLD),JBLK) = ZCOMBUFR(JI)
          ELSEIF(LLGP3A(IFLD)) THEN
            PGP3A(JK,IGP3ALEVS(IFLD),IGP3APARS(IFLD),JBLK) = ZCOMBUFR(JI)
          ELSEIF(LLGP3B(IFLD)) THEN
            PGP3B(JK,IGP3BLEVS(IFLD),IGP3BPARS(IFLD),JBLK) = ZCOMBUFR(JI)
          ENDIF
        ENDDO
      ENDDO
    ENDIF 
