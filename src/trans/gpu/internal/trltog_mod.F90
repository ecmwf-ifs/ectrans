...
#define TRLTOG_DTYPE MPI_DOUBLE_PRECISION
  ...
  IF( IRECVTOT(MYPROC) > 0 )THEN
    ...
    IF (LLPGPONLY) THEN
      ...
      !$OMP&     MAP(TO:IGPTROFF,IFLDOFF) COLLAPSE(3) &
      !$OMP&     SHARED(JK_MAX,NGPBLKS,IFLDS,IGPTRSEND,MYSETW,INDOFF,IGPTROFF,IFLDOFF,MYPROC, &
      !$OMP&            LLINDER,KPTRGP,PGP,PGLAT,KINDEX)
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) &
      !$ACC&     COPYIN(IGPTROFF,IFLDOFF,JK_MAX,IFLDS,LLINDER,MYPROC,MYSETW,NGPBLKS) COLLAPSE(3)
#endif
      DO JBLK=1,NGPBLKS
        DO JFLD=1,IFLDS
          DO JKL=1,JK_MAX
