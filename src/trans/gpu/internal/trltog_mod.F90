...
#endif
        DO JI = ICOMBUFR_OFFSET(INR)+1, ICOMBUFR_OFFSET(INR+1)
          ZCOMBUFR(JI) = 0.0_JPRB
        ENDDO
#else
        ZCOMBUFR(ICOMBUFR_OFFSET(INR)+1:ICOMBUFR_OFFSET(INR+1)) = 0.0_JPRB
#endif
      ELSE
#ifdef USE_RAW_MPI
        CALL MPI_IRECV(ZCOMBUFR(ICOMBUFR_OFFSET(INR)+1:ICOMBUFR_OFFSET(INR+1)), &
          & IRECVTOT_MPI(IRECV), TRLTOG_DTYPE, NPRCIDS(IRECV)-1, MTAGLG, LOCAL_COMM, IREQUEST(IR), &
          & IERROR)
...
#ifdef USE_RAW_MPI
        CALL MPI_ISEND(ZCOMBUFS(ICOMBUFS_OFFSET(INS)+1:ICOMBUFS_OFFSET(INS+1)), &
          &            ISENDTOT_MPI(ISEND), TRLTOG_DTYPE, NPRCIDS(ISEND)-1, MTAGLG, LOCAL_COMM, &
          &            IREQUEST(IR), IERROR)
        IREQ(IR) = IREQUEST(IR)%MPI_VAL
#else
