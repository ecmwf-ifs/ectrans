...
CONTAINS
  FUNCTION PREPARE_TRLTOG(ALLOCATOR,KF_FS,KF_GP) RESULT(HTRLTOG)
    ...
    NELEM = ALIGN(KF_GP*D%NGPTOT*STORAGE_SIZE(DUMMY)/8,128) ! ZCOMBUFR
    NELEM = ALIGN(NELEM + KF_FS*D%NLENGTF*STORAGE_SIZE(DUMMY)/8,128) !ZCOMBUFS upper obund
...
#define TRLTOG_DTYPE MPI_DOUBLE
    ...
    IF (ISEND_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION(ALLOCATOR, HTRLTOG%HCOMBUFR_AND_COMBUFS),&
          & int(ALIGN(KF_GP*D%NGPTOT*STORAGE_SIZE(ZCOMBUFR(1))/8,128)+1,kind=c_size_t), &
          & int(ICOMBUFS_OFFSET(ISEND_COUNTS+1)*STORAGE_SIZE(ZCOMBUFS(1))/8,kind=c_size_t))
    ENDIF
