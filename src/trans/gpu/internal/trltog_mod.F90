...
MODULE TRLTOG_MOD
  CONTAINS
  SUBROUTINE TRLTOG_CUDAAWARE(PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,KVSET,KPTRGP,&
...
#define TRLTOG_DTYPE MPI_DOUBLE_PRECISION
  ...
  IF(ISENDTOT(MYPROC) > 0) THEN
    ...
    IF (PRESENT(PGP)) THEN
      ...
      !$OMP&     COLLAPSE(2) &
      !$OMP&     SHARED(IFLDS,MYSETW,MYPROC, &
      !$OMP&            KPTRGP,PGP,PREEL_REAL)
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS) &
      !$ACC&     COLLAPSE(2) &
      !$ACC&     PRESENT(PGP,PREEL_REAL) ASYNC(1) &
      ...
      DO JL=1,IRECV_WSET_SIZE_V
        DO JFLD=1,KF_FS
          ...
          IFLD = IFLDA(JFLD)
          IPOS = KF_FS*(IIN_TO_SEND_BUFR(IIN_TO_SEND_BUFR_V+JL)-1)+JFLD
    ...
    ELSE
      ...
      !$OMP&     COLLAPSE(2) &
      !$OMP&     SHARED(IFLDS,MYSETW,MYPROC, &
      !$OMP&            PGPUV,IUVLEVS,IUVPARS,PGP2, &
      !$OMP&            PREEL_REAL,PGP3A,IGP3ALEVS,IGP3APARS,PGP3B,&
      ...
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS) &
      !$ACC&     COLLAPSE(2) &
      !$ACC&     PRESENT(PGPUV,PGP2, &
      !$ACC&            PREEL_REAL,PGP3A,PGP3B) ASYNC(1)
#elif defined(_CRAYFTN)
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS) &
      !$ACC&     COPYIN(IRECV_WSET_SIZE_V,IRECV_WSET_OFFSET_V,KF_FS,NPROMA,IIN_TO_SEND_BUFR_V) &
      !$ACC&     PRESENT(PGPUV,PGP2,PREEL_REAL,PGP3A,PGP3B) ASYNC(1)
#endif
#endif
      DO JL=1,IRECV_WSET_SIZE_V
        DO JFLD=1,KF_FS
  ...
  DO INS=1,ISEND_COUNTS
    ...
    !$OMP&  SHARED(ILEN,KF_FS,ISEND,ZCOMBUFS,INS,PREEL_REAL)
#endif
#ifdef ACCGPU
#if defined(__NVCOMPILER) || defined(__PGI)
    !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IPOS) COPYIN(ILEN) COLLAPSE(2) &
    !$ACC&  PRESENT(ZCOMBUFS,PREEL_REAL) ASYNC(1)
#elif defined(_CRAYFTN)
    !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IPOS) COPYIN(ILEN,KF_FS,IIN_TO_SEND_BUFR_V,ICOMBUFS_OFFSET_V) COLLAPSE(2) &
    !$ACC&  PRESENT(ZCOMBUFS,PREEL_REAL) ASYNC(1)
#endif
#endif
    DO JL=1,ILEN
  ...
  !$ACC EXIT DATA DELETE(PREEL_REAL) ASYNC(1)
#endif
  ...
  DO INR=1,IRECV_COUNTS
    ...
    IF (PRESENT(PGP)) THEN
      ...
      !$OMP&     SHARED(IFLDA,ISETW,IPOS, &
      !$OMP&            PGP,KPTRGP,ZCOMBUFR,INR,LLPGPONLY,PGPUV,IUVLEVS, &
      !$OMP&            IUVPARS,PGP2,PGP3A,IGP3ALEVS,IGP3APARS, &
      !$OMP&            PGP3B,IGP3BLEVS,IGP3BPARS)
#endif
#ifdef ACCGPU
#if defined(__NVCOMPILER) || defined(__PGI)
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI) &
      !$ACC&     COPYIN(INR,KF_FS,IPOS,IFLD,IFLDA) COLLAPSE(2) &
      !$ACC&     PRESENT(PGP,ZCOMBUFR) ASYNC(1)
#elif defined(_CRAYFTN)
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI) &
  ...
  SUBROUTINE TRLTOG(PREEL_REAL,KF_FS,KF_GP,KF_SCALARS_G,KVSET,KPTRGP,&
 &                  PGP,PGPUV,PGP3A,PGP3B,PGP2)
      ...
      IF(IFIRST > 0) THEN
         ...
         IF(LLINDER) THEN
          DO JFLD=1,IFLDS
            ...
            DO JK=IFIRST,ILAST
              ...
              PGP(JK,IFLD,JBLK) = PREEL_REAL((INDEX(IPOS)-1)*KF_FS+JFLD)
            ENDDO
          ENDDO
         ELSE
          DO JFLD=1,IFLDS
        ...
        ELSE
          DO JFLD=1,IFLDS
            ...
            IF(LLUV(IFLD)) THEN
              DO JK=IFIRST,ILAST
                IPOS = INDEX(INDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1)-1
                PGPUV(JK,IUVLEVS(IFLD),IUVPARS(IFLD),JBLK) = PREEL_REAL(JFLD+KF_FS*IPOS)
              ENDDO
            ELSEIF(LLGP2(IFLD)) THEN
              DO JK=IFIRST,ILAST
                IPOS = INDEX(INDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1)-1
                PGP2(JK,IGP2PARS(IFLD),JBLK)=PREEL_REAL(JFLD+KF_FS*IPOS)
              ENDDO
            ELSEIF(LLGP3A(IFLD)) THEN
              DO JK=IFIRST,ILAST
                IPOS = INDEX(INDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1)-1
                PGP3A(JK,IGP3ALEVS(IFLD),IGP3APARS(IFLD),JBLK)=PREEL_REAL(JFLD+KF_FS*IPOS)
              ENDDO
            ELSEIF(LLGP3B(IFLD)) THEN
              DO JK=IFIRST,ILAST
                IPOS = INDEX(INDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1)-1
                PGP3B(JK,IGP3BLEVS(IFLD),IGP3BPARS(IFLD),JBLK)=PREEL_REAL(JFLD+KF_FS*IPOS)
              ENDDO
            ELSE
  ...
  !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JL,II,JFLD,INS,ISEND,ILEN,ISEND_FLD_END)
    DO INS=1,INSEND
      ...
      DO JL=1,ILEN
        ...
        DO JFLD=ISEND_FLD_START(ISEND),ISEND_FLD_END
#endif
          ZCOMBUFS((JFLD-ISEND_FLD_START(ISEND))*ILEN+JL,INS) = PREEL_REAL((II-1)*KF_FS+JFLD)
        ENDDO
      ENDDO
      ZCOMBUFS(-1,INS) = 1
