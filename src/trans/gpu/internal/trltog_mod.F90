...
MODULE TRLTOG_MOD
  ...
  INTEGER(KIND=JPIM),allocatable:: ICOMR_KFFS(:), ICOMS_KFFS(:)
  ...
  IF (IBUFLENS > 0) ALLOCATE(ICOMS_KFFS(INSEND))
  IF (IBUFLENR > 0) ALLOCATE(ICOMR_KFFS(INRECV))

  !$ACC DATA &
  !$ACC PRESENT(PGLAT) &
  !$ACC COPYIN(IGPTRSEND,INDOFF,KINDEX, LLUV,LLGP2,LLGP3A,LLGP3B,KPTRGP)
  !$ACC DATA IF(PRESENT(PGP))   PRESENT_OR_COPYOUT(PGP)
  !$ACC DATA IF(PRESENT(PGPUV)) PRESENT_OR_COPYOUT(PGPUV) COPYIN(IUVLEVS,IUVPARS)
  !$ACC DATA IF(PRESENT(PGP2))  PRESENT_OR_COPYOUT(PGP2)  COPYIN(IGP2PARS)
  !$ACC DATA IF(PRESENT(PGP3A)) PRESENT_OR_COPYOUT(PGP3A) COPYIN(IGP3APARS,IGP3ALEVS)
  !$ACC DATA IF(PRESENT(PGP3B)) PRESENT_OR_COPYOUT(PGP3B) COPYIN(IGP3BPARS, IGP3BLEVS)
...
!      IRECV_FLD_START = 1 !! INT(ZCOMBUFR(-1,INR),KIND=JPIM) !! is this always 1 ?
      ...
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IFIRST,JI,ILAST,JK,IFLDT) &
      !$ACC     COPYIN(KF_FS,IPOS,JPOS,IFLD,IFLDA,JK_MAX,IRECV_FLD_END) COLLAPSE(3)
      DO JBLK=1,NGPBLKS
    ...
    !$ACC END DATA   !! CREATE ZCOMBUF[RS]
