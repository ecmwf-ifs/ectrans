...
REAL(KIND=JPRBT), ALLOCATABLE :: ZALL(:,:), ZQX(:,:)
REAL(KIND=JPRBT), ALLOCATABLE :: ZALL1(:,:), ZQY(:,:)
...
IF( KDIR == -1 ) THEN
  ...
  ZQX(:,1:KSL) = 0._JPRBT
  ZQX(:,IEND:2*KDGNH) = 0._JPRBT
  ZQY(:,1:KSL) = 0._JPRBT
  ZQY(:,IEND:2*KDGNH) = 0._JPRBT
  DO JGL=KSL, IEND
