...
IIN_STRIDES0 = ALIGN(KFIELDS,8)
IIN_STRIDES1 = IIN_STRIDES0 * ALIGN(MAX((R%NTMAX+2)/2,(R%NTMAX+3)/2),8)
IOUT_STRIDES0 = ALIGN(KFIELDS,8)
IOUT_STRIDES1 = IOUT_STRIDES0 * ALIGN(R_NDGNH,8)
IIN0_STRIDES0 = ALIGN(KFIELDS/2,8)
IIN0_STRIDES1 = IIN0_STRIDES0 * ALIGN(MAX((R%NTMAX+2)/2,(R%NTMAX+3)/2),8)
IOUT0_STRIDES0 = ALIGN(KFIELDS/2,8)
IOUT0_STRIDES1 = IOUT0_STRIDES0 * ALIGN(R_NDGNH,8)

ALLOCATE(ZINP(IIN_STRIDES1*D_NUMP))
ALLOCATE(ZOUTS(IOUT_STRIDES1*D_NUMP))
ALLOCATE(ZOUTA(IOUT_STRIDES1*D_NUMP))
...
CALL CUDA_GEMM_BATCHED( &
    ...
    & KFIELDS, NS(:), KS(:), &
    & 1.0_JPRBT, &
    & ZINP, IIN_STRIDES0, IIN_STRIDES1, &
    & ZAA, SIZE(ZAA,1), SIZE(ZAA,1)*SIZE(ZAA,2), &
    & 0.0_JPRBT, &
    & ZOUTA, IOUT_STRIDES0, IOUT_STRIDES1, &
    & D_NUMP)
  ...
  DO JGL=1,G_NDGLU(0)
    DO JK=1,KFIELDS,2
      ...
      ZOUTA((JK-1)/2+1+(JGL-1)*IOUT_STRIDES0+(KMLOC0-1)*IOUT_STRIDES1) &
        & = ZOUT0((JK-1)/2+1+(JGL-1)*IOUT0_STRIDES0)
    ENDDO
  ENDDO
...
CALL CUDA_GEMM_BATCHED( &
  ...
  & 0.0_JPRBT, &
  & ZOUTS, IOUT_STRIDES0, IOUT_STRIDES1, &
  & D_NUMP)
CALL cudaDeviceSynchronize()
IF (LSYNC_TRANS) THEN
  CALL GSTATS(444,0)
