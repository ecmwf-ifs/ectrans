...
!$ACC ENTER DATA CREATE(ZZBA,ZZCSTA,ZZBS,ZZCSTS)
ZZBA=0._JPRBT
ZZBS=0._JPRBT
ZZCSTA=0._JPRBT
ZZCSTS=0._JPRBT  
!$ACC update device(ZZBA,ZZBS,ZZCSTA,ZZCSTS)
...
CALL CUDA_DGEMM_BATCHED_OVERLOAD( &
  & 'N', 'T', &
  & ITDZCA, int(ILDZCA,kind=jpim), int(ILDZBA,kind=jpim), &
  & 1.0_JPRBT, &
  & ZZBA, ITDZBA, int(ILDZBA,kind=jpim),&
  & ZAA, LDZAA, int(TDZAA,kind=jpim), &
  & 0._JPRBT, &
  & ZZCSTA, ITDZCA, int(ILDZCA,kind=jpim), &
  & D_NUMP)
!$ACC END HOST_DATA
...
CALL CUDA_DGEMM_BATCHED_OVERLOAD( &
  ...
  & 1.0_JPRBT, &
  & ZZBS, ITDZBS, int(ILDZBS,jpim), &
  & ZAS, LDZAS, int(TDZAS,jpim), &
  & 0._JPRBT, &
  & ZZCSTS, ITDZCS, int(ILDZCS,jpim), &
  & D_NUMP)
!$ACC END HOST_DATA
...
!$ACC EXIT DATA DELETE(ZZBA,ZZCSTA,ZZBS,ZZCSTS)
DEALLOCATE(ZZBS)
DEALLOCATE(ZZBA)
DEALLOCATE(ZZCSTS)
DEALLOCATE(ZZCSTA)
