...
!$ACC&      PRESENT (D_MYMS,G_NDGLU,D_NUMP,R_NDGNH,R_NSMAX) &
!$ACC&      COPYIN (IF_FS_INV, KFC) &
!$ACC&      PRESENT (ZAA,ZAS) &
!$ACC&      PRESENT (IZCST) &
!$ACC&      COPYIN (PIA) &
...
#endif
#ifdef OMPGPU
!$OMP TARGET DATA MAP(TO:S,S%ITHRESHOLD,S%LUSEFLT,D,D_MYMS,R,G,G_NDGLU,D_NUMP,R_NDGNH,R_NSMAX) &
!$OMP&      MAP(ALLOC:ZAA,ZAS,IZCST,PIA,PSOA1,PAOA1,IZBS)
#endif
...
!$ACC&      PRESENT(D_MYMS,PSOA1,PAOA1,KFC,R_NDGNH,D_NUMP)
#endif
DO KMLOC=1,D_NUMP
   DO JGL=1,R_NDGNH
      DO J1=2,KFC,2
...
!$OMP&      SHARED(D_NUMP,R_NSMAX,KFC,D_MYMS,IZBS,TDZAA,IF_FS_INV,PIA) DEFAULT(NONE)
#endif
#ifdef ACCGPU
!$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(KM,ISKIP,ILA,ILS,IA) DEFAULT(NONE) &
!$ACC&      PRESENT(D_NUMP,R_NSMAX,KFC,D_MYMS,IZBS,IF_FS_INV,PIA,TDZAA)
#endif
DO KMLOC=1,D_NUMP
  DO J=1,(R_NSMAX+2)/2
    DO JK=1,KFC
...
CALL CUDA_SGEMM_STRIDED_BATCHED( &
  ...
  & 1.0_JPRBT, &
  & IZBS, ITDZBA, INT(ILDZBA, KIND=C_LONG_LONG),&
  & ZAA, LDZAA, INT(TDZAA, KIND=C_LONG_LONG), &
  & 0._JPRBT, &
  & IZCST, ITDZCA, INT(ILDZCA, KIND=C_LONG_LONG), &
  & D_NUMP)
#ifdef OMPGPU
!$OMP END TARGET DATA
#endif
...
!$OMP&      SHARED(D_NUMP,R_NSMAX,KFC,D_MYMS,IZBS,ITDZBS,ILDZBS,PIA) DEFAULT(NONE)
#endif
#ifdef ACCGPU
!$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(KM,ISKIP,ILS,IS) DEFAULT(NONE) &
!$ACC&      PRESENT(D_NUMP,R_NSMAX,KFC,D_MYMS,IZBS,ITDZBS,ILDZBS,PIA)
#endif
DO KMLOC=1,D_NUMP
  DO J=1,(R_NSMAX+3)/2
    DO JK=1,KFC
...
CALL CUDA_SGEMM_STRIDED_BATCHED( &
 ...
 & 1.0_JPRBT, &
 & IZBS, ITDZBS, INT(ILDZBS, KIND=C_LONG_LONG), &
 & ZAS, LDZAS, INT(TDZAS, KIND=C_LONG_LONG), &
 & 0._JPRBT, &
 & IZCST, ITDZCS, INT(ILDZCS, KIND=C_LONG_LONG), &
 & D_NUMP)
#ifdef OMPGPU
!$OMP END TARGET DATA
#endif
