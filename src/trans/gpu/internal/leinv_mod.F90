...
CONTAINS
  SUBROUTINE LEINV_STRIDES(KF_LEG, IOUT_STRIDES0, IOUT_SIZE, IIN_STRIDES0, IIN_SIZE,&
    ...
    INTEGER(KIND=JPIM), OPTIONAL, INTENT(OUT) :: IOUT_STRIDES0
    INTEGER(KIND=JPIB), OPTIONAL, INTENT(OUT) :: IOUT_SIZE
    INTEGER(KIND=JPIM), OPTIONAL, INTENT(OUT) :: IIN_STRIDES0
    INTEGER(KIND=JPIB), OPTIONAL, INTENT(OUT) :: IIN_SIZE
    INTEGER(KIND=JPIM), OPTIONAL, INTENT(OUT) :: IOUT0_STRIDES0
    INTEGER(KIND=JPIM), OPTIONAL, INTENT(OUT) :: IOUT0_SIZE
    INTEGER(KIND=JPIM), OPTIONAL, INTENT(OUT) :: IIN0_STRIDES0
    INTEGER(KIND=JPIM), OPTIONAL, INTENT(OUT) :: IIN0_SIZE

    ASSOCIATE(D_OFFSETS_GEMM1=>D%OFFSETS_GEMM1, D_OFFSETS_LEINV_IN=>D%OFFSETS_LEINV_IN)
    ...
    IF (PRESENT(IIN_SIZE)) &
      IIN_SIZE = IIN_STRIDES0*D_OFFSETS_LEINV_IN(D%NUMP+1)
...
#define HIP_GEMM HIP_DGEMM_GROUPED
    ...
    INTEGER(KIND=JPIM)  :: IOUT0_STRIDES0
    ...
    ASSOCIATE(D_NUMP=>D%NUMP, R_NSMAX=>R%NSMAX, G_NDGLU=>G%NDGLU, D_MYMS=>D%MYMS, &
      &       D_OFFSETS_GEMM1=>D%OFFSETS_GEMM1, D_OFFSETS_LEINV_IN=>D%OFFSETS_LEINV_IN, ZAA=>FG%ZAA, &
      &       ZAS=>FG%ZAS, ZAA0=>FG%ZAA0, ZAS0=>FG%ZAS0)
    ...
    CALL LEINV_STRIDES(KF_LEG, IOUT_STRIDES0=IOUT_STRIDES0, IIN_STRIDES0=IIN_STRIDES0, &
      &                IOUT0_STRIDES0=IOUT0_STRIDES0, IIN0_STRIDES0=IIN0_STRIDES0)
      ...
      DO JK=1,2*KF_LEG
        ...
        IF(KM /= 0)THEN
          ...
          DO J=1,(R_NSMAX-KM+2)/2
            ZINP(JK+(J-1)*IIN_STRIDES0+D_OFFSETS_LEINV_IN(KMLOC)*IIN_STRIDES0)=PIA(JK,IA+1+(J-1)*2,KMLOC)
          ENDDO
          ...
          DO J=(R_NSMAX-KM+2)/2+1,ALIGN((R_NSMAX-KM+2)/2,A)
            ZINP(JK+(J-1)*IIN_STRIDES0+D_OFFSETS_LEINV_IN(KMLOC)*IIN_STRIDES0)=0
          ENDDO
#endif
   ...
   ENDIF
    ...
    DO KMLOC=1,D_NUMP
      ...
      NS(KMLOC) = G_NDGLU(KM)
      AOFFSETS(KMLOC) = IIN_STRIDES0*D_OFFSETS_LEINV_IN(KMLOC)
      BOFFSETS(KMLOC) = D%OFFSETS_LEINV_LEGPOL(KMLOC)
      ...
      DO JK=1,2*KF_LEG
        ...
        IF(KM /= 0) THEN
          ...
          DO J=1,(R_NSMAX-KM+3)/2
            ZINP(JK+(J-1)*IIN_STRIDES0+D_OFFSETS_LEINV_IN(KMLOC)*IIN_STRIDES0)=PIA(JK,IS+1+(J-1)*2,KMLOC)
          ENDDO
#if defined(USE_CUTLASS) && defined(USE_CUTLASS_3XTF32)
          ! those are only needed with tensor cores (zinp might contain NaNs!)
          !$ACC LOOP SEQ
