...
CONTAINS
  ...
  SUBROUTINE LEINV(ALLOCATOR,PIA,ZINP,ZOUTS,ZOUTA,KF_LEG)
    ...
    USE TPM_DISTR,                   ONLY: D
    USE HICBLAS_MOD,                 ONLY: HIP_DGEMM_GROUPED, HIP_SGEMM_GROUPED
...
#define MULT_BUTM MULT_BUTM_DP
    ...
    INTEGER(KIND=JPIM)  :: KM, KMLOC, IA, IS, JK, J
    ...
    ASSOCIATE(D_NUMP=>D%NUMP, R_NSMAX=>R%NSMAX, G_NDGLU=>G%NDGLU, D_MYMS=>D%MYMS, D_OFFSETS_GEMM1=>D%OFFSETS_GEMM1,&
        D_OFFSETS_GEMM2=>D%OFFSETS_GEMM2, ZAA=>FG%ZAA, ZAS=>FG%ZAS, Y_BUT_FLAT_A=>FG%Y_BUT_FLAT_A)
    ...
    !$OMP& PRIVATE(KM,IA,J) &
    !$OMP& SHARED(D,R,KF_LEG,ZINP,IIN_STRIDES0) MAP(TO:KF_LEG)
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(KM,IA,J) &
    !$ACC& FIRSTPRIVATE(KF_LEG,IIN_STRIDES0) DEFAULT(NONE) &
#ifdef _CRAYFTN
    !$ACC&
#else
    !$ACC& ASYNC(1)
...
#endif
#ifdef ACCGPU
      !$ACC HOST_DATA USE_DEVICE(Y_BUT_FLAT_A%SLEV_NODE_PNONIM, ZINP, ZOUTA)
#endif
      CALL MULT_BUTM('N', Y_BUT_FLAT_A%N_ORDER, Y_BUT_FLAT_A%N_LEVELS, &
        &            Y_BUT_FLAT_A%IBETALEN_MAX, Y_BUT_FLAT_A%SLEV_OFFSET, Y_BUT_FLAT_A%SLEV_IJ, &
        &            Y_BUT_FLAT_A%SLEV_IK, Y_BUT_FLAT_A%SLEV_IBETALEN, &
        ...
        &            C_LOC(Y_BUT_FLAT_A%SLEV_NODE_B), C_LOC(ZINP), C_LOC(ZOUTA))
#ifdef ACCGPU
      !$ACC END HOST_DATA
#endif
#ifdef OMPGPU
