...
USE CUDA_GEMM_BATCHED_MOD, ONLY : CUDA_GEMM_BATCHED !!, ONLY: CUDA_TCGEMM_BATCHED
...
!$ACC&      PRESENT (ZAA,ZAS,IZBS,IZCST) &
!$ACC&      PRESENT (PIA) &
...
!!            IZBA((JK-1)/ISKIP+1,J,KMLOC)=PIA(JK,IA+1+(J-1)*2,KMLOC)*RRPELTMINV/ZAMAX((JK-1)/ISKIP+1,KMLOC)
  ...
  ENDDO
ENDDO
!!notinRAPS      !$ACC UPDATE SELF(IZBA)
!!notinRAPS      !$ACC UPDATE SELF(ZAMAX)
!!notinRAPS!$ACC UPDATE SELF(ZAA)
...
!!call CUDA_DGEMM_STRIDED_BATCHED_1D_OVERLOAD( &
  & 'N', 'T', &
  & ITDZCA, int(ILDZCA,kind=jpim), int(ILDZBA,kind=jpim), &
  & 1.0_JPRBT, &
  & IZBS, ITDZBA, ILDZBA, &
  & ZAA, LDZAA, TDZAA, &
  & 0._JPRBT, &
...
!!notinRAPS       !$ACC UPDATE SELF(IZCST)
...
!!               PAOA1(JK,ISL+JI-1,KMLOC) = IZCST((JK-1)/ISKIP+1,JI,KMLOC)*ZAMAX((JK-1)/ISKIP+1,KMLOC)/RRPELTMINV
               PAOA1(JK,ISL+JI-1,KMLOC) = IZCST((JK-1)/ISKIP+1+(JI-1+(KMLOC-1)*R_NDGNH)*IF_FS_INV)
            END IF
         END IF
      ENDDO
   ENDDO
...
!!            IZBS((JK-1)/ISKIP+1,J,KMLOC)=PIA(JK,IS+1+(J-1)*2,KMLOC)*RRPELTMINV/ZSMAX((JK-1)/ISKIP+1,KMLOC)
            IZBS((JK-1)/ISKIP+1+(J-1+(KMLOC-1)*TDZAS)*IF_FS_INV)=PIA(JK,IS+1+(J-1)*2,KMLOC)
         END IF
      END IF
    ENDDO
  ENDDO
ENDDO
!!notinRAPS     !$ACC UPDATE SELF(IZBS)
!!notinRAPS     !$ACC UPDATE SELF(ZSMAX)
!!notinRAPS     !$ACC UPDATE SELF(ZAS)
!!notinRAPS     !$ACC UPDATE SELF(IZBS_oli)
...
!!call CUDA_DGEMM_STRIDED_BATCHED_1D_OVERLOAD( &
  & 'N', 'T', &
  & ITDZCS, int(ILDZCS,kind=jpim), int(ILDZBS,kind=jpim), &
  & 1.0_JPRBT, &
  & IZBS, ITDZBS, ILDZBS, &
  & ZAS, LDZAS, TDZAS, &
  & 0._JPRBT, &
