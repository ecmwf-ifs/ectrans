...
#ifdef ACCGPU
!$ACC DATA PRESENT(PAOA1,PSOA1,D_MYMS,D_NPROCL,D_NSTAGT1B,D_NPNTGTB1,G_NDGLU,R_NDGL) PRESENT(FOUBUF_IN)
#endif
#ifdef ACCGPU
!$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(KM,ISL,IGLS,ISTAS,OFFSET1,OFFSET2) &
!$ACC& FIRSTPRIVATE(KF_OUT_LT)
#endif
DO KMLOC=1,D_NUMP
  DO JF=1,2*KF_OUT_LT
    KM = D_MYMS(KMLOC)
