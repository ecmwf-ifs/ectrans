...
ALLOCATE(ZGTF_REAL(KF_FS, D%NLENGTF))
#ifdef ACCGPU
!$ACC ENTER DATA CREATE(ZGTF_REAL)
#endif
...
ENDIF
CALL GSTATS(412,1)
CALL TRGTOL_CUDAAWARE(ZGTF_REAL,KF_FS,KF_GP,IVSET,KPTRGP,&
 &PGP,PGPUV,PGP3A,PGP3B,PGP2)
#ifdef OMPGPU
!$OMP END TARGET DATA
!$OMP END TARGET DATA
...
#endif
#else
CALL TRGTOL(ZGTF_REAL,KF_FS,KF_GP,KF_SCALARS_G,IVSET,KPTRGP,&
 &PGP,PGPUV,PGP3A,PGP3B,PGP2)
#endif
...
CALL GSTATS(1640,0)
IF (KF_FS > 0) THEN
  CALL FTDIR(ZGTF_REAL,ZGTF_COMPLEX,KF_FS)
  CALL FOURIER_OUT(ZGTF_COMPLEX,FOUBUF_IN,KF_FS)
ELSE
  ZGTF_COMPLEX => ZGTF_REAL
ENDIF

CALL GSTATS(1640,1)
IF (ASSOCIATED(ZGTF_COMPLEX)) THEN
#ifdef ACCGPU
  !$ACC EXIT DATA DELETE(ZGTF_COMPLEX)
#endif
  DEALLOCATE(ZGTF_COMPLEX)
ENDIF
CALL GSTATS(106,1)
!     ------------------------------------------------------------------
