...
CONTAINS
SUBROUTINE FTDIR_CTL(KF_UV_G,KF_SCALARS_G,KF_GP,KF_FS,FOUBUF_IN, &
...
USE LEDIR_MOD       ,ONLY : LEDIR_ALLOC_SIZE
use ieee_arithmetic
!
...
REAL(KIND=JPRBT),  INTENT(OUT), ALLOCATABLE :: FOUBUF_IN(:)
...
IALLOC_SIZE=KF_FS*D%NLENGTF


IF (NPROC == 1) THEN
  ! If we have only one process, we skip the intermediate buffer, so we need more space
  IALLOC_SIZE = IALLOC_SIZE + LEDIR_ALLOC_SIZE(KF_FS)/SIZEOF(REUSE_PTR(1))
ENDIF

IF (.NOT. ALLOCATED(REUSE_PTR)) THEN
  ALLOCATE(REUSE_PTR(IALLOC_SIZE))
#ifdef ACCGPU
  !$ACC ENTER DATA CREATE(REUSE_PTR)
#endif
ELSEIF (SIZE(REUSE_PTR) < IALLOC_SIZE) THEN
#ifdef ACCGPU
...
#ifdef USE_CUDA_AWARE_MPI_FT
WRITE(NOUT,*) 'ftdir_ctl:TRGTOL_CUDAAWARE'
CALL TRGTOL_CUDAAWARE(PREEL_REAL,KF_FS,KF_GP,IVSET,KPTRGP,&
 &PGP,PGPUV,PGP3A,PGP3B,PGP2)
#else
WRITE(NOUT,*) 'ftdir_ctl:TRGTOL *not* CUDAAWARE'
CALL TRGTOL(PREEL_REAL,KF_FS,KF_GP,KF_SCALARS_G,IVSET,KPTRGP,&
 &PGP,PGPUV,PGP3A,PGP3B,PGP2)
#endif
