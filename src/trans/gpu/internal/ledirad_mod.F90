...
REAL(KIND=JPRBT)    :: ZB(KDGLU,KIFC), ZCA((R%NTMAX-KM+2)/2,KIFC), ZCS((R%NTMAX-KM+3)/2,KIFC)
...
IF(KM == 0)THEN
  ...
  DO JGL=ISL,R%NDGNH
    DO J1=2,KFC,2
      PSIA(J1,JGL)=0.0_JPRBT
      PAIA(J1,JGL)=0.0_JPRBT
    ENDDO
  ENDDO
ELSE
  ISKIP = 1
...
!*       1. ANTISYMMETRIC PART.
  ...
  IF(ILA <= ITHRESHOLD .OR. .NOT.S%LUSEFLT) THEN
     ...
     IF(LLDOUBLE)THEN
        CALL DGEMM('N','N',KDGLU,KIFC,ILA,1.0_JPRBT,S%FA(KMLOC)%RPNMA,KDGLU,&
             &ZCA,ILA,0._JPRBT,ZB,KDGLU)
     ELSE
        CALL SGEMM('N','N',KDGLU,KIFC,ILA,1.0_JPRBT,S%FA(KMLOC)%RPNMA,KDGLU,&
             &ZCA,ILA,0._JPRBT,ZB,KDGLU)
     END IF
...
!*       1.3      SYMMETRIC PART.
  ...
  IF(ILS <= ITHRESHOLD .OR. .NOT.S%LUSEFLT) THEN
    ...
    IF(LLDOUBLE)THEN
       CALL DGEMM('N','N',KDGLU,KIFC,ILS,1.0_JPRBT,S%FA(KMLOC)%RPNMS,KDGLU,&
            &ZCS,ILS,0._JPRBT,ZB,KDGLU)
    ELSE
       CALL SGEMM('N','N',KDGLU,KIFC,ILS,1.0_JPRBT,S%FA(KMLOC)%RPNMS,KDGLU,&
            &ZCS,ILS,0._JPRBT,ZB,KDGLU)
