...
USE TPM_FIELDS      ,ONLY : F, ZEPSNM, ZIA
...
!$ACC&      COPYIN (R_NSMAX, D_MYMS,D_NUMP,F,F%RLAPIN,F%RN)   &
...
DO KMLOC=1,D_NUMP
  ...
  !$OMP&    SHARED(ZKM,ISMAX,F,ZN,ZLAPIN,ZZEPSNM,ZEPSNM,KMLOC)
#endif
#ifdef ACCGPU
  !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IJ) &
  !$ACC&    PRESENT(F,F%RN,F%RLAPIN,ZN,ZLAPIN,ZZEPSNM,ZEPSNM) &
