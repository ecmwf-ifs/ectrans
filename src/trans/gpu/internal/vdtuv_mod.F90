...
DO KMLOC=1,D_NUMP
  ...
  !$ACC&  FIRSTPRIVATE(KMLOC)
#endif
  DO JN=D_MYMS(KMLOC)-1,ISMAX+2
    IJ = ISMAX+3-JN
    ZN(IJ) = F_RN(JN)
...
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(IR,II) FIRSTPRIVATE(KFIELD,KMLOC,ZKM)
#endif
    DO J=1,KFIELD
      DO JI=2,ISMAX+3-D_MYMS(KMLOC)
        !ZKM = D_MYMS(KMLOC)
        IR = 2*J-1
        II = IR+1
