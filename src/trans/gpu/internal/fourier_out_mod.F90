...
!$ACC DATA PRESENT(D,G_NMEN,D_NPROCM,D_NSTAGT0B,D_NPNTGTB0,FOUBUF_IN,PREEL_COMPLEX,D_NSTAGTF,G_NLOEN) ASYNC(1)

! scale results and move into next transformation buffer

OFFSET_VAR=D_NPTRLS(MYSETW)

!$ACC PARALLEL LOOP PRIVATE(IGLG,IOFF_LAT,IPROC,ISTA,SCAL) DEFAULT(NONE) &
!$ACC& ASYNC(1) TILE(32,8,1)
DO KGL=1,D%NDGL_FS
