...
CONTAINS
SUBROUTINE FOURIER_OUT(PREEL_COMPLEX,FOUBUF_IN,KFIELDS)
...
!$ACC DATA PRESENT(D,G_NLOEN,D_NSTAGT0B,D_NPNTGTB0,FOUBUF_IN,D_NPROCM,PREEL_COMPLEX,G_NMEN,D_NSTAGTF) ASYNC(1)
#endif
...
#ifdef ACCGPU
!$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(IGLG,IOFF_COMPLEX,SCAL,JM,IPROC,ISTA) &
!$ACC&  FIRSTPRIVATE(IBEG,IEND,IINC,KFIELDS,OFFSET_VAR) DEFAULT(NONE) ASYNC(1)
#endif
DO KGL=IBEG,IEND,IINC
  DO JF=1,KFIELDS
    IGLG = OFFSET_VAR+KGL-1
    IOFF_COMPLEX = KFIELDS*D_NSTAGTF(KGL)+(JF-1)*(D%NSTAGTF(KGL+1)-D%NSTAGTF(KGL))
    ...
    DO JM=0,G_NMEN(IGLG)
      ...
      FOUBUF_IN(ISTA+2*JF-1) = SCAL * PREEL_COMPLEX(IOFF_COMPLEX+2*JM+1)
      FOUBUF_IN(ISTA+2*JF  ) = SCAL * PREEL_COMPLEX(IOFF_COMPLEX+2*JM+2)
    ENDDO
  ENDDO
ENDDO
#ifdef ACCGPU
