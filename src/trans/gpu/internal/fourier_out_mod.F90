...
ALLOCATE(FOUBUF_IN(D%NLENGT0B*KFIELDS))
#ifdef ACCGPU
!$ACC ENTER DATA CREATE(FOUBUF_IN)

!$ACC DATA PRESENT(D,G_NLOEN,D_NSTAGT0B,D_NPNTGTB0,FOUBUF_IN,D_NPROCM,PREEL,G_NMEN,D_NSTAGTF) ASYNC(1)
#endif
...
#ifdef ACCGPU
!$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(IGLG,IOFF,SCAL,JM,IPROC,ISTA) FIRSTPRIVATE(IBEG,IEND,IINC,KFIELDS,OFFSET_VAR) DEFAULT(NONE)
#endif
DO KGL=IBEG,IEND,IINC
  DO JF=1,KFIELDS/2
    ...
    DO JM=0,G_NMEN(IGLG)
      IPROC = D_NPROCM(JM)
      ISTA  = (D_NSTAGT0B(IPROC)+D_NPNTGTB0(JM,KGL))*KFIELDS
