...
MODULE TPM_HICFFT
  ...
  SUBROUTINE CREATE_PLAN_FFT(KPLAN,KTYPE,KN,KLOT)
    ...
    INTERFACE
      SUBROUTINE HICFFT_CREATE_PLAN(KPLAN,KTYPE,KN,KLOT) BIND(C,NAME="hicfft_create_plan_")
      USE, INTRINSIC :: ISO_C_BINDING
      TYPE(C_PTR), INTENT(OUT) :: KPLAN
    ...
    DO JL=1,HICT%N_PLANS(KN)
      IF( KLOT == CURR_HICFFT_PLAN%NLOT .AND. KTYPE == CURR_HICFFT_PLAN%NTYPE )THEN
        LLFOUND=.TRUE.
  ...
  SUBROUTINE EXECUTE_PLAN_FFT(KN,N,X_IN,X_OUT,PLAN_PTR)
    ...
    REAL(KIND=JPRB), TARGET :: X_IN
    REAL(KIND=JPRB), TARGET :: X_OUT
