...
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK, JPHOOK
  ...
  IF (KF_UV > 0) THEN
    ...
    CALL PRFI1B(ZIA(IVORL:IVORU,:,:),PSPVOR,KF_UV,KFLDPTRUV)
    CALL PRFI1B(ZIA(IDIVL:IDIVU,:,:),PSPDIV,KF_UV,KFLDPTRUV)
  ...
  IF(KF_SCALARS > 0)THEN
    IF(PRESENT(PSPSCALAR)) THEN
      ...
      CALL PRFI1B(ZIA(IFIRST:ILAST,:,:),PSPSCALAR(:,:),KF_SCALARS,KFLDPTRSC)
    ELSE
      IF(PRESENT(PSPSC2) .AND. NF_SC2 > 0) THEN
        IFIRST = ILAST+1
        ILAST  = IFIRST-1+2*NF_SC2
        CALL PRFI1B(ZIA(IFIRST:ILAST,:,:),PSPSC2(:,:),NF_SC2)
      ENDIF
      IF(PRESENT(PSPSC3A) .AND. NF_SC3A > 0) THEN
        ...
        DO J3=1,IDIM3
          CALL PRFI1B(ZIA(IFIRST:ILAST,:,:),PSPSC3A(:,:,J3),IDIM1)
        ENDDO
      ENDIF
      IF(PRESENT(PSPSC3B) .AND. NF_SC3B > 0) THEN
        ...
        DO J3=1,IDIM3
          ...
          CALL PRFI1B(ZIA(IFIRST:ILAST,:,:),PSPSC3B(:,:,J3),IDIM1)
        ENDDO
      ENDIF
    ENDIF
 ...
 ENDIF
  ...
  !stop 'Error: SPGL_PROC is not (yet) supported in GPU version'
