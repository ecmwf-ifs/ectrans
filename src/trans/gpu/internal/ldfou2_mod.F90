...
#ifdef ACCGPU
!$ACC DATA &
!$ACC& COPYIN(KF_UV,F,F%RACTHE) &
!$ACC& PRESENT(D_NUMP,D_MYMS,R_NDGNH,R_NDGL,G_NDGLU) &
!$ACC& PRESENT(PAIA)
#endif
#ifdef OMPGPU
!$OMP TARGET DATA &
!$OMP& MAP(TO:F,F%RACTHE,D,D_NUMP,D_MYMS,R_NDGNH,R_NDGL,G_NDGLU) &
!$OMP& MAP(TOFROM:PAIA)
#endif
...
!$ACC&      PRESENT(D_NUMP,R_NDGNH,KF_UV,D_MYMS,R_NDGNH,G_NDGLU,R_NDGL,PAIA,F,F%RACTHE)
#endif
DO KMLOC=1,D_NUMP
  DO JGL=1,R_NDGNH
    DO J=1,4*KF_UV
