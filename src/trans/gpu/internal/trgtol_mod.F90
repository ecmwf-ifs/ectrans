...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  USE TPM_DISTR       ,ONLY : D, NPRCIDS, NPRTRNS, MTAGGL,  &
       &                      MYSETV, MYSETW, MYPROC, NPROC,D_NSTAGTF
  ...
  INTEGER(KIND=JPIM) :: IFIRST, IFIRSTLAT, IGL, IGLL, ILAST,&
               ...
               &INSEND,INS,INR,IR, IUNIT, KF, KGL, KI,D_NDGL_FS
  ...
  ALLOCATE(PREEL_REAL_TMP,MOLD=PREEL_REAL)
  ...
  !$ACC DATA PRESENT(PREEL_REAL,D) CREATE(PREEL_REAL_TMP)
  !$ACC KERNELS
#endif
...
#ifdef ACCGPU
  !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) FIRSTPRIVATE(D_NDGL_FS,KF_FS) PRESENT(PREEL_REAL_TMP,D_NSTAGTF)
#endif
  DO KGL=1,D_NDGL_FS
    DO KF=1,KF_FS
      !$ACC LOOP SEQ
      DO KI=1,D_NSTAGTF(KGL+1)-D_NSTAGTF(KGL)
        PREEL_REAL(KF_FS*D_NSTAGTF(KGL)+(KF-1)*(D_NSTAGTF(KGL+1)-D_NSTAGTF(KGL))+KI) = &
            & PREEL_REAL_TMP(KF_FS*D_NSTAGTF(KGL)+(KI-1)*KF_FS+KF)
      ENDDO
    ENDDO
  ENDDO
#ifdef ACCGPU
