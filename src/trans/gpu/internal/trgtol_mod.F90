...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  !$OMP TARGET DATA MAP(ALLOC:PGLAT) MAP(TO:KPTRGP)
  ...
  IF(PRESENT(PGP)) THEN
    ...
    !$OMP& SHARED(NGPBLKS,IFLDS,MYSETW,MYPROC,IFLDOFF,PGLAT,PGP)
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS) &
    !$ACC&   FIRSTPRIVATE(NGPBLKS,KF_FS,MYSETW,MYPROC,NPROMA,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,IRECV_BUFR_TO_OUT_V) ASYNC(1)
#endif
  ...
  ELSE
#ifdef OMPGPU
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,ILAST,IFLD,JK) &
    !$OMP& SHARED(NGPBLKS,IFLDS,MYSETW,MYPROC,IFLDOFF, &
    !$OMP&        PGLAT,PGPUV,PGP2,PGP3A,PGP3B,IUVLEVS, &
    ...
    !$ACC& PRESENT(PGLAT,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#elif defined(_CRAYFTN)
    !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS,IOFF,PBOUND) &
    !$ACC& COPYIN(NGPBLKS,IFLDS,MYSETW,MYPROC,KF_FS,NPROMA,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,IRECV_BUFR_TO_OUT_V) &
    !$ACC& PRESENT(PGLAT,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#endif
#endif
    DO JL=1,ISEND_WSET_SIZE_V
  ...
  CALL GSTATS(1602,0)
    DO INS=1,ISEND_COUNTS
      ...
      !$OMP& SHARED(ISEND_FLD_END,NGPBLKS,IFLDA,ISETW,IPOS,IJPOS,LLINDER,LLPGPONLY, &
      ...
      !$ACC& PRESENT(ZCOMBUFS,KPTRGP,PGP,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#elif defined(_CRAYFTN)
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI) &
      !$ACC& COPYIN(INS,IFLDA,NPROMA,ISEND_WSET_SIZE_V,ISEND_FIELD_COUNT_V,ISEND_WSET_OFFSET_V,ICOMBUFS_OFFSET_V) &
      !$ACC& PRESENT(ZCOMBUFS,KPTRGP,PGP,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#endif
#endif
      DO JL=1,ISEND_WSET_SIZE_V
...
!$ACC WAIT(1)
  ...
  DO INR=1,IRECV_COUNTS
    ...
    ICOMBUFR_OFFSET_V = ICOMBUFR_OFFSET(INR)
      ...
      !$OMP&   SHARED(IRECV_FLD_END,ILEN,IRECV,ZCOMBUFR,PGLAT,INR)
#endif
#ifdef ACCGPU
