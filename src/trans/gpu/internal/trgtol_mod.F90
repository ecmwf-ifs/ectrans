...
#define TRGTOL_DTYPE MPI_REAL8
    ...
    DO INS=1,ISEND_COUNTS
      ...
      IF(PRESENT(PGP)) THEN
        ...
        !$OMP& PRIVATE(JK,JBLK,IFLD,JI) ECTRANS_DEVICE_ADDR_CLAUSE(ZCOMBUFS) &
        !$OMP& SHARED(ISEND_FIELD_COUNT_V,ISEND_WSET_SIZE_V,NPROMA,&
        !$OMP& ISEND_WSET_OFFSET_V,INS,ICOMBUFS_OFFSET_V,IFLDA,PGP) &
      ...
      ELSE
        ...
        !$OMP& JI,IOFF,PBOUND) ECTRANS_DEVICE_ADDR_CLAUSE(PGPUV,ZCOMBUFS,PGP2,PGP3A,PGP3B) &
        !$OMP& SHARED(ISEND_FIELD_COUNT_V,ISEND_WSET_SIZE_V,NPROMA,&
...
#ifdef OMPGPU
        !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) ECTRANS_OMP_DEFAULT_CLAUSE PRIVATE(JK,JBLK,IFLD,&
        !$OMP& IPOS) ECTRANS_DEVICE_ADDR_CLAUSE(PREEL_REAL) &
        !$OMP& SHARED(KF_FS,ISEND_WSET_SIZE_V,NPROMA,ISEND_WSET_OFFSET_V,IFLDA,&
        !$OMP& IRECV_BUFR_TO_OUT_V,IRECV_BUFR_TO_OUT,PGP) MAP(TO:KF_FS,IRECV_BUFR_TO_OUT_V)
#endif
#ifdef ACCGPU
...
#ifdef OMPGPU
        !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) ECTRANS_OMP_DEFAULT_CLAUSE PRIVATE(JK,JBLK,IFLD,&
        !$OMP& IPOS,IOFF,PBOUND) ECTRANS_DEVICE_ADDR_CLAUSE(PGPUV,PREEL_REAL,PGP2,PGP3A,PGP3B) &
        !$OMP& SHARED(KF_FS,ISEND_WSET_SIZE_V,NPROMA,ISEND_WSET_OFFSET_V,IFLDA,&
    ...
    DO INR=1,IRECV_COUNTS
      ...
      !$OMP& SHARED(KF_FS,ILEN,IRECV_BUFR_TO_OUT_V,IRECV_BUFR_TO_OUT,ICOMBUFR_OFFSET_V) &
      !$OMP& MAP(TO:IRECV_BUFR_TO_OUT_V,ICOMBUFR_OFFSET_V)
#endif
#ifdef ACCGPU
