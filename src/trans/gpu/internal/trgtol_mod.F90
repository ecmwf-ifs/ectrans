...
#define TRGTOL_DTYPE MPI_REAL8
    ...
    !DIR !DATA !COPYIN(IRECV_BUFR_TO_OUT) !PRESENT(PREEL_REAL) IF (KF_FS > 0) !ASYNC(1)
    !DIR !DATA !COPYIN(PGP_INDICES) !ASYNC(1)
    ...
    !DIR !WAIT(1)
    IF (PRESENT(PGP)) THEN
      !DIR !UPDATE_DEVICE(PGP) IF (.NOT. LLPGP_ON_GPU)
    ENDIF
    IF (PRESENT(PGPUV)) THEN
      !DIR !UPDATE_DEVICE(PGPUV) IF (.NOT. LLPGP_ON_GPU)
    ENDIF
    IF (PRESENT(PGP2)) THEN
      !DIR !UPDATE_DEVICE(PGP2) IF (.NOT. LLPGP_ON_GPU)
    ENDIF
    IF (PRESENT(PGP3A)) THEN
      !DIR !UPDATE_DEVICE(PGP3A) IF (.NOT. LLPGP_ON_GPU)
    ENDIF
    IF (PRESENT(PGP3B)) THEN
      !DIR !UPDATE_DEVICE(PGP3B) IF (.NOT. LLPGP_ON_GPU)
    ENDIF

    !DIR !DATA IF(PRESENT(PGP) .AND. KF_GP > 0) !PRESENT(PGP) !ASYNC(1)
    !DIR !DATA IF(PRESENT(PGPUV)) !PRESENT(PGPUV) !ASYNC(1)
    !DIR !DATA IF(PRESENT(PGP2)) !PRESENT(PGP2) !ASYNC(1)
    !DIR !DATA IF(PRESENT(PGP3A)) !PRESENT(PGP3A) !ASYNC(1)
    !DIR !DATA IF(PRESENT(PGP3B)) !PRESENT(PGP3B) !ASYNC(1)
    ...
    !....Pack loop.........................................................
    !DIR !DATA !PRESENT(ZCOMBUFS) IF (ISEND_COUNTS > 0) !ASYNC(1)
    ...
    DO INS=1,ISEND_COUNTS
      ...
      IF(PRESENT(PGP)) THEN
        !DIR !PARALLEL COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI) &
        !DIR& !SHARED(ISEND_FIELD_COUNT_V,ISEND_WSET_SIZE_V,NPROMA,ISEND_WSET_OFFSET_V,INS) &
        !DIR& !SHARED(ICOMBUFS_OFFSET_V,IFLDA,PGP,ZCOMBUFS) &
        !DIR& !FIRSTPRIVATE(INS,ISEND_FIELD_COUNT_V,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V) &
        !DIR& !FIRSTPRIVATE(ICOMBUFS_OFFSET_V,NPROMA) !ASYNC(1)
        ...
        ENDDO
      ELSE
        !DIR !PARALLEL COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI,IOFF,PBOUND) &
        !DIR& !SHARED(ISEND_FIELD_COUNT_V,ISEND_WSET_SIZE_V,NPROMA,ISEND_WSET_OFFSET_V,INS) &
        !DIR& !SHARED(ICOMBUFS_OFFSET_V,IFLDA,PGP_INDICES,PGPUV,ZCOMBUFS,PGP2,PGP3A,PGP3B) &
        !DIR& !FIRSTPRIVATE(INS,ISEND_FIELD_COUNT_V,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V) &
        !DIR& !FIRSTPRIVATE(ICOMBUFS_OFFSET_V,NPROMA) !ASYNC(1)
        DO JFLD=1,ISEND_FIELD_COUNT_V
          DO JL=1,ISEND_WSET_SIZE_V
    ...
    !DIR !DATA !PRESENT(ZCOMBUFR) IF (IRECV_COUNTS > 0) !ASYNC(1)
    IR=0

#ifdef USE_GPU_AWARE_MPI
    !DIR !USE_DEVICE(ZCOMBUFR, ZCOMBUFS)
#else
    !DIR !UPDATE_HOST(ZCOMBUFS) IF (ISEND_COUNTS > 0)
#endif
    ...
    IF(LLOCAL_CONTRIBUTION)THEN
      ...
      CALL GSTATS(1601,0)
      IF(PRESENT(PGP)) THEN
        !DIR !PARALLEL COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS) &
        !DIR& !SHARED(KF_FS,ISEND_WSET_SIZE_V,NPROMA,ISEND_WSET_OFFSET_V,IFLDA) &
        !DIR& !SHARED(IRECV_BUFR_TO_OUT_V,IRECV_BUFR_TO_OUT,PGP,PREEL_REAL) &
        !DIR& !FIRSTPRIVATE(KF_FS,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,IRECV_BUFR_TO_OUT_V) &
        !DIR& !FIRSTPRIVATE(NPROMA) !ASYNC(1)
        DO JFLD=1,KF_FS
          DO JL=1,ISEND_WSET_SIZE_V
            JK = MOD(ISEND_WSET_OFFSET_V+JL-1,NPROMA)+1
            JBLK = (ISEND_WSET_OFFSET_V+JL-1)/NPROMA+1
        ...
        ENDDO
      ELSE
        !DIR !PARALLEL COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS,IOFF,PBOUND) &
        !DIR& !SHARED(KF_FS,ISEND_WSET_SIZE_V,NPROMA,ISEND_WSET_OFFSET_V,IFLDA) &
        !DIR& !SHARED(IRECV_BUFR_TO_OUT_V,IRECV_BUFR_TO_OUT,PGP_INDICES,PGPUV,PREEL_REAL,PGP2) &
        !DIR& !SHARED(PGP3A,PGP3B) &
        !DIR& !FIRSTPRIVATE(KF_FS,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,IRECV_BUFR_TO_OUT_V) &
        !DIR& !FIRSTPRIVATE(NPROMA) !ASYNC(1)
        DO JFLD=1,KF_FS
          DO JL=1,ISEND_WSET_SIZE_V
    ...
    !DIR !END_HOST_DATA
#else
    !DIR !UPDATE_DEVICE(ZCOMBUFR) IF (IRECV_COUNTS > 0)
#endif
    IF (LSYNC_TRANS) THEN
      CALL GSTATS(431,0)
      CALL MPL_BARRIER(CDSTRING='')
    ...
    DO INR=1,IRECV_COUNTS
      ...
      !DIR !PARALLEL COLLAPSE(2) DEFAULT(NONE) PRIVATE(IPOS) &
      !DIR& !SHARED(KF_FS,ILEN,IRECV_BUFR_TO_OUT_V,IRECV_BUFR_TO_OUT,ICOMBUFR_OFFSET_V,ZCOMBUFR,&
      !DIR& !SHARED(PREEL_REAL) &
      !DIR& !FIRSTPRIVATE(KF_FS,ILEN,IRECV_BUFR_TO_OUT_V,ICOMBUFR_OFFSET_V) !ASYNC(1)
      DO JFLD=1,KF_FS
        DO JL=1,ILEN
          IPOS = IRECV_BUFR_TO_OUT(IRECV_BUFR_TO_OUT_V+JL,1)+ &
              & (JFLD-1)*IRECV_BUFR_TO_OUT(IRECV_BUFR_TO_OUT_V+JL,2)+1
    ...
    !DIR !END_DATA ! ZCOMBUFR
    !DIR !END_DATA ! IFLDA
    !DIR !END_DATA ! IRECV_BUFR_TO_OUT
    !DIR !END_DATA ! PGPINDICES
    !DIR !END_DATA ! ZCOMBUFS (present)
    !DIR !END_DATA ! PGP3B
    !DIR !END_DATA ! PGP3A
    !DIR !END_DATA ! PGP2
    !DIR !END_DATA ! PGPUV
