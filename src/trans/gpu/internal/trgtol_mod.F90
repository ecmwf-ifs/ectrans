...
#ifdef ACCGPU
  !$ACC DATA PRESENT(PGLAT) COPYIN(KPTRGP,INDOFF,KINDEX,IGPTRSEND,PGP_INDICES) ASYNC(1)
...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  IF(PRESENT(PGP)) THEN
    ...
    !$ACC& FIRSTPRIVATE(NGPBLKS,KF_FS,JK_MAX,MYSETW,MYPROC) ASYNC(1)
#endif
    DO JBLK=1,NGPBLKS
      DO JFLD=1,KF_FS
        DO JKL=1, JK_MAX
...
#ifdef OMPGPU
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) &
    !$OMP& SHARED(NGPBLKS,JK_MAX,IGPTRSEND,MYSETW,INDOFF,MYPROC,IGPTROFF,IFLDOFF, &
    !$OMP&        PGLAT,KINDEX,PGPUV,PGP2,PGP3A,PGP3B)
#endif
#ifdef ACCGPU
#if defined(__NVCOMPILER) || defined(__PGI)
    !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK,IOFF,PBOUND) &
    !$ACC& PRESENT(IGPTRSEND,INDOFF,IGPTROFF, &
    !$ACC&        PGLAT,KINDEX,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#elif defined(_CRAYFTN)
    !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK,IOFF,PBOUND) &
    !$ACC& COPYIN(NGPBLKS,KF_FS,JK_MAX,MYSETW,MYPROC) &
    !$ACC& PRESENT(IGPTRSEND,INDOFF,IGPTROFF, &
    !$ACC&        PGLAT,KINDEX,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#endif
#endif
  ...
  CALL GSTATS(1602,0)
    DO INS=1,INSEND
      ...
      IF(PRESENT(PGP)) THEN
#ifdef OMPGPU
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,IFIRST,ILAST) &
      !$OMP& SHARED(ISEND_FLD_END,NGPBLKS,IFLDA,ISETW,IPOS,JK_MAX,IGPTRSEND,LLINDER, &
      !$OMP&        ZCOMBUFS,INS,KPTRGP,PGP,PGPUV,PGP2,PGP3A,PGP3B,IUVLEVS, &
      !$OMP&        IUVPARS,IGP3BLEVS,IGP3BPARS)
#endif
#ifdef ACCGPU
#if defined(__NVCOMPILER) || defined(__PGI)
      !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,IFIRST,ILAST) COPYIN(INS,JK_MAX,IGPTROFF,IFLDA) &
      !$ACC& PRESENT(IGPTRSEND,ZCOMBUFS,KPTRGP,PGP,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#elif defined(_CRAYFTN)
      !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,IFIRST,ILAST) &
      !$ACC  COPYIN(NGPBLKS,ISEND_FLD_CNT,INS,JK_MAX,IFLDA,IPOS,ISETW,SEND_OFFSET) &
      !$ACC& PRESENT(IGPTRSEND,ZCOMBUFS,KPTRGP,PGP,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#endif
#endif
      DO JBLK=1,NGPBLKS
      ...
      ENDDO
      ELSE
        !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,IFLD,IFIRST,ILAST,IOFF,PBOUND) ASYNC(1) &
        !$ACC& FIRSTPRIVATE(NGPBLKS,ISEND_FLD_CNT,JK_MAX,ISETW,IPOS,INS,SEND_OFFSET)
        DO JBLK=1,NGPBLKS
          DO JFLD=1,ISEND_FLD_CNT
            DO JKL=1, JK_MAX
              IFLD=IFLDA(JFLD)
...
!$ACC WAIT(1)
  ...
  DO INR=1,INRECV
      ...
      !$ACC& FIRSTPRIVATE(KF_FS,INR,ILEN,INDOFFL,RECV_OFFSET) ASYNC(1)
#endif
      DO JFLD=1,KF_FS
        DO JL=1,ILEN
