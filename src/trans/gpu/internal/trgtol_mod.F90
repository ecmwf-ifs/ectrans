...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  IF(ISENDTOT(MYPROC) > 0 )THEN
    ...
    IF(PRESENT(PGP)) THEN
#ifdef OMPGPU
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) DEFAULT(NONE) PRIVATE(IPOS,ILAST,IFLD,JK) &
      !$OMP& SHARED(IFLDS,MYSETW,MYPROC,IFLDOFF,PREEL_REAL,PGP)
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS) &
    !$ACC&   FIRSTPRIVATE(KF_FS,NPROMA,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,IRECV_BUFR_TO_OUT_V) ASYNC(1)
#endif
    ...
    ELSE
#ifdef OMPGPU
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,ILAST,IFLD,JK) &
      !$OMP& SHARED(IFLDS,MYSETW,MYPROC,IFLDOFF, &
      !$OMP&        PREEL_REAL,PGPUV,PGP2,PGP3A,PGP3B,IUVLEVS, &
      ...
      !$ACC& PRESENT(PREEL_REAL,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#elif defined(_CRAYFTN)
    !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS,IOFF,PBOUND) &
    !$ACC& COPYIN(KF_FS,NPROMA,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,IRECV_BUFR_TO_OUT_V) &
    !$ACC& PRESENT(PREEL_REAL,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#endif
#endif
      DO JL=1,ISEND_WSET_SIZE_V
  ...
  DO INS=1,ISEND_COUNTS
    ...
    IF(PRESENT(PGP)) THEN
#ifdef OMPGPU
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,ILAST) &
      !$OMP& SHARED(ISEND_FLD_END,IFLDA,ISETW,IPOS,IJPOS,LLINDER,LLPGPONLY, &
      !$OMP&        ZCOMBUFS,INS,KPTRGP,PGP,PGPUV,PGP2,PGP3A,PGP3B,IUVLEVS, &
      !$OMP&        IUVPARS,IGP3BLEVS,IGP3BPARS)
#endif
#ifdef ACCGPU
#if defined(__NVCOMPILER) || defined(__PGI)
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI) COPYIN(INS,IFLDA) &
      !$ACC& PRESENT(ZCOMBUFS,PGP) ASYNC(1)
#elif defined(_CRAYFTN)
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI) &
      !$ACC& COPYIN(NPROMA,ISEND_WSET_SIZE_V,ISEND_FIELD_COUNT_V,ISEND_WSET_OFFSET_V,ICOMBUFS_OFFSET_V,IFLDA) &
      !$ACC& PRESENT(ZCOMBUFS,PGP) ASYNC(1)
#endif
#endif
      DO JL=1,ISEND_WSET_SIZE_V
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI,IOFF,PBOUND) COPYIN(IFLDA) ASYNC(1) &
      !$ACC&  FIRSTPRIVATE(ISEND_WSET_SIZE_V,ISEND_FIELD_COUNT_V,ISEND_WSET_OFFSET_V,NPROMA,ICOMBUFS_OFFSET_V)
#endif
      DO JL=1,ISEND_WSET_SIZE_V
        DO JFLD=1,ISEND_FIELD_COUNT_V
          JK = MOD(ISEND_WSET_OFFSET_V+JL-1,NPROMA)+1
  ...
  DO INR=1,IRECV_COUNTS
    ...
    !$OMP&   SHARED(IRECV_FLD_END,ILEN,IRECV,ZCOMBUFR,PREEL_REAL,INR)
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(II) &
      !$ACC& PRESENT(ZCOMBUFR,PREEL_REAL) &
      !$ACC& FIRSTPRIVATE(KF_FS,ILEN,IRECV_BUFR_TO_OUT_V,ICOMBUFR_OFFSET_V) ASYNC(1)
#endif
    DO JFLD=1,KF_FS
      DO JL=1,ILEN
  ...
  CALL GSTATS(1603,1)
   ...
   !$OMP END TARGET DATA  !! IRECV_BUFR_TO_OUT,PGPINDICES
#endif
  ...
  INTEGER(KIND=JPIM) :: IFIRST, IFIRSTLAT, IGL, IGLL, ILAST,&
               ...
               &INSEND,INS,INR,IR, IUNIT, KF, KGL, KI
