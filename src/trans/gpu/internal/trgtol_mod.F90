...
#ifdef ACCGPU
  !$ACC DATA PRESENT(PGLAT) COPYIN(KPTRGP,INDOFF,KINDEX,PGP_INDICES) ASYNC(1)
...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  IF(PRESENT(PGP)) THEN
    ...
    !$OMP& SHARED(NGPBLKS,IFLDS,MYSETW,INDOFF,MYPROC,IFLDOFF,KINDEX,PGLAT,PGP)
#endif
#ifdef ACCGPU
  ...
  ELSE
#ifdef OMPGPU
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) &
    !$OMP& SHARED(NGPBLKS,IFLDS,MYSETW,INDOFF,MYPROC,IFLDOFF, &
    !$OMP&        PGLAT,KINDEX,PGPUV,PGP2,PGP3A,PGP3B,IUVLEVS, &
    ...
    !$ACC& PRESENT(INDOFF,PGLAT,KINDEX,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
