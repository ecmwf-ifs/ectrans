...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  CALL GSTATS(1602,0)
    DO INS=1,INSEND
      ...
      !$ACC  COPYIN(NGPBLKS,ISEND_FLD_CNT,INS,JK_MAX,IFLDA,IPOS,ISETW) &
      !$ACC& PRESENT(IGPTRSEND,ZCOMBUFS,KPTRGP,PGP,PGPUV,PGP2,PGP3A,PGP3B) ASYNC(1)
#endif
#endif
      DO JBLK=1,NGPBLKS
      ...
      ELSE
        ...
        !$ACC& FIRSTPRIVATE(NGPBLKS,ISEND_FLD_CNT,JK_MAX,ISETW,IPOS,INS)
        DO JBLK=1,NGPBLKS
          DO JFLD=1,ISEND_FLD_CNT
            DO JKL=1, JK_MAX
              IFLD=IFLDA(JFLD)
...
!$ACC WAIT(1)
  ...
  DO INR=1,INRECV
      ...
      !$ACC& FIRSTPRIVATE(KF_FS,INR,ILEN,INDOFFL) ASYNC(1)
#endif
      DO JFLD=1,KF_FS
        DO JL=1,ILEN
