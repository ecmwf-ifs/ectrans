...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  IF(LLPGPONLY) THEN
    ...
    !$ACC& PRESENT(IGPTRSEND,INDOFF,IGPTROFF,IFLDOFF,KINDEX,PGLAT,PGP) &
    !$ACC& COPYIN(NGPBLKS,IFLDS,JK_MAX,MYPROC,MYSETW)
#endif
    DO JBLK=1,NGPBLKS
      DO JFLD=1,IFLDS
        DO JKL=1, JK_MAX
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(II) COPYIN(IRECV,ILEN,IRECV_FLD_END,INR) &
