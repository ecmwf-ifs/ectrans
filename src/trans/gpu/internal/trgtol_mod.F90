...
CONTAINS
  FUNCTION PREPARE_TRGTOL(ALLOCATOR,KF_GP,KF_FS) RESULT(HTRGTOL)
    ...
    NELEM = ALIGN(1_JPIB*KF_GP*D%NGPTOT*C_SIZEOF(DUMMY), 128)

    HTRGTOL%COMBUFR_START = NELEM + 1
    NELEM = NELEM + ALIGN(1_JPIB*KF_FS*D%NLENGTF*C_SIZEOF(DUMMY), 128)
    HTRGTOL%HCOMBUFS_COMBUFR = RESERVE_GAM(ALLOCATOR, NELEM, "HTRGTOL%HCOMBUFR_COMBUFS")
    
    HTRGTOL%REEL_START = 1
    NELEM = ALIGN(1_JPIB*KF_FS*D%NLENGTF*C_SIZEOF(DUMMY), 128) ! PREEL_REAL
    HTRGTOL%HREEL = RESERVE(ALLOCATOR, NELEM, "HTRGTOL%HREEL") 
...
#define TRGTOL_DTYPE MPI_REAL8
    ...
    CALL ASSIGN_PTR(PREEL_REAL, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HREEL),&
        & HTRGTOL%REEL_START, 1_JPIB*KF_FS*D%NLENGTF*C_SIZEOF(PREEL_REAL(1)))
    end block
    ...
    IF (ISEND_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION_GAM(ALLOCATOR, HTRGTOL%HCOMBUFS_COMBUFR),&
          & HTRGTOL%COMBUFS_START, ICOMBUFS_OFFSET(ISEND_COUNTS+1)*C_SIZEOF(ZCOMBUFS(1)))
    ENDIF
    ...
    IF (IRECV_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION_GAM(ALLOCATOR, HTRGTOL%HCOMBUFS_COMBUFR),&
          & HTRGTOL%COMBUFR_START, ICOMBUFR_OFFSET(IRECV_COUNTS+1)*C_SIZEOF(ZCOMBUFR(1)))
    ENDIF
#ifdef OMPGPU
#endif
#ifdef ACCGPU
