...
MODULE TRGTOL_MOD
  ...
  !$ACC DATA PRESENT(PGLAT) COPYIN(KPTRGP,INDOFF,KINDEX,LLGP2,LLGP3A,LLGP3B,LLUV,IGPTRSEND)
  ...
  CALL GSTATS(1601,0)
  IF(LLPGPONLY) THEN
    !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) COPYIN(IFLDOFF,IGPTROFF)
    DO JBLK=1,NGPBLKS
      DO JFLD=1,IFLDS
        DO JKL=1, JK_MAX
          IFIRST = IGPTRSEND(1,JBLK,MYSETW)
