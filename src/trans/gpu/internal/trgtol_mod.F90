...
#ifdef ACCGPU
  !$ACC DATA PRESENT(PGLAT) COPYIN(KPTRGP,INDOFF,KINDEX,LLGP2,LLGP3A,LLGP3B,LLUV,IGPTRSEND) ASYNC(1)
...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  CALL GSTATS(1602,0)
    DO INS=1,INSEND
...
#ifdef ACCGPU
#if defined(__NVCOMPILER) || defined(__PGI)
      !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,IFLDT,IFIRST,ILAST) COPYIN(INS,JK_MAX,IGPTROFF,IFLDA) &
      ...
      !$ACC  COPYIN(NGPBLKS,INS,JK_MAX,IFLDA,IPOS,ISETW,LLINDER,ISEND_FLD_CNT,IGPTROFF) &
      !$ACC& PRESENT(IGPTRSEND,LLUV,LLGP2,LLGP3A,LLGP3B,ZCOMBUFS,KPTRGP,PGP,PGPUV,PGP2,PGP3A,PGP3B, &
      !$ACC&        IGP2PARS,IGP3ALEVS,IGP3APARS) ASYNC(1)
#endif
#endif
      ...
      ELSE
        ...
        !$ACC& FIRSTPRIVATE(ISEND_FLD_CNT,NGPBLKS,JK_MAX,ISETW,IPOS,INS)
        DO JBLK=1,NGPBLKS
          DO JFLD=1,ISEND_FLD_CNT
            DO JKL=1, JK_MAX
              IFLDT=IFLDA(JFLD)
