...
ENDIF
CLABEL = TRANSFER(IHEADER(1:2),CLABEL)
IF(CLABEL /= 'LEGPOL  ') THEN
  WRITE(NERR,*) CLABEL
  CALL ABORT_TRANS('READ_LEGPOL:WRONG LABEL')
ENDIF
IF(IHEADER(3) /= R%NSMAX) CALL ABORT_TRANS('READ_LEGPOL:WRONG SPECTRAL TRUNCATION')
IF(IHEADER(4) /= R%NDGNH) CALL ABORT_TRANS('READ_LEGPOL:WRONG NO OF GAUSSIAN LATITUDES')
IRBYTES = IHEADER(5)
IF (IRBYTES /= INT(STORAGE_SIZE(1.0_JPRBT) / 8, JPIM)) THEN
