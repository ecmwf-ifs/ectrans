...
! compute m=0 in double precision:
  IF(KMLOC0 > 0) THEN
    ...
    !$OMP&     SHARED(R_NDGNH,KFC,G_NDGLU,DZBST0,DTDZBA,PAIA,KMLOC0,F_RW)
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(ISL) DEFAULT(NONE) &
    !$ACC&     COPYIN(KFC,DTDZBA,KMLOC0) &
    ...
    !$OMP&    SHARED(R_NTMAX,KFC,POA1,DZCAT0,DTDZCA,KMLOC0)
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(IA) DEFAULT(NONE) &
    !$ACC&    COPYIN(KFC,DTDZCA,KMLOC0) &
...
#ifdef OMPGPU
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) PRIVATE(ISL) DEFAULT(NONE) &
    !$OMP&     SHARED(R_NDGNH,KFC,G_NDGLU,DZBST0,DTDZBS,PAIA,KMLOC0,F_RW)
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(ISL) DEFAULT(NONE) &
    !$ACC&     COPYIN(KFC,DTDZBS,KMLOC0) &
    !$ACC&     PRESENT(R_NDGNH,G_NDGLU,DZBST0,PAIA,F_RW)
#endif
    DO J=1,MIN(R_NDGNH,G_NDGLU(0))
      DO JK=1,KFC,2
...
#ifdef OMPGPU
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) PRIVATE(ILS,IS) DEFAULT(NONE) &
    !$OMP&    SHARED(R_NTMAX,KFC,POA1,DZCST0,DTDZCS,KMLOC0)
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(ILS,IS) DEFAULT(NONE) &
    !$ACC&    COPYIN(KFC,DTDZCS,KMLOC0) &
