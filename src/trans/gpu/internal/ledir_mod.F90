...
USE HICBLAS_MOD      ,ONLY : HIP_DGEMM_BATCHED, HIP_SGEMM_BATCHED, &
 &       HIP_DGEMM_STRIDED_BATCHED,HIP_SGEMM_STRIDED_BATCHED, &
...
!!#define HIP_GEMM_BATCHED HIP_SGEMM_STRIDED_BATCHED
#define HIP_GEMM_BATCHED HIP_SGEMM_GROUPED_OVERLOAD
#else
!!#define HIP_GEMM_BATCHED HIP_DGEMM_BATCHED
!!#define HIP_GEMM_BATCHED HIP_DGEMM_STRIDED_BATCHED
#define HIP_GEMM_BATCHED HIP_DGEMM_GROUPED_OVERLOAD
#endif
...
INTEGER(KIND=JPIM) :: IA, IS, ISL, J, JK, ISTATS
...
!$ACC& COPYIN(KF_FS,KF_UV,TDZAA,TDZAS)
#endif
...
#endif
#ifdef ACCGPU
!$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(KM,IA)
#endif
DO KMLOC=1,D_NUMP
  DO JK=1,2*KF_FS
    KM = D_MYMS(KMLOC)
...
#endif
#ifdef ACCGPU
  !$ACC PARALLEL LOOP COLLAPSE(2) FIRSTPRIVATE(KMLOC0) DEFAULT(NONE)
#endif
  DO JGL=1,G_NDGLU(0)
    DO JF=1,2*KF_FS,2
...
#endif
#ifdef ACCGPU
  !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(IA) FIRSTPRIVATE(KMLOC0) DEFAULT(NONE)
#endif
  DO J=1,(R_NSMAX+2)/2
    DO JK=1,2*KF_FS,2
