...
CONTAINS
  SUBROUTINE LEDIR_STRIDES(KF_FS,IOUT_STRIDES0,IOUT_SIZE,IIN_STRIDES0,IIN_SIZE,&
    ...
    ASSOCIATE(D_OFFSETS_GEMM1=>D%OFFSETS_GEMM1, D_OFFSETS_LEDIR_OUT=>D%OFFSETS_LEDIR_OUT)

    IF (PRESENT(IOUT_STRIDES0)) &
        IOUT_STRIDES0 = ALIGN(2*KF_FS,A)
    IF (PRESENT(IOUT_SIZE)) &
        IOUT_SIZE = IOUT_STRIDES0*D_OFFSETS_LEDIR_OUT(D%NUMP+1)
...
#define HIP_GEMM HIP_DGEMM_GROUPED
    ...
    ASSOCIATE(D_NUMP=>D%NUMP, R_NSMAX=>R%NSMAX, R_NTMAX=>R%NTMAX, G_NDGLU=>G%NDGLU, &
            ...
            & D_OFFSETS_LEDIR_OUT=>D%OFFSETS_LEDIR_OUT, &
    ...
    !$OMP& MAP(PRESENT,ALLOC:ZAA,ZAS,POA1,D_OFFSETS_GEMM1,D_OFFSETS_LEDIR_OUT)
#endif
#ifdef ACCGPU
    !$ACC DATA &
    !$ACC& PRESENT(ZINPS,ZINPA,ZOUT,ZINPS0,ZINPA0,ZOUT0) &
    !$ACC& PRESENT(D,D_MYMS,D_NUMP,R,R_NTMAX,R_NSMAX) &
    !$ACC& PRESENT(ZAA,ZAS,POA1,D_OFFSETS_GEMM1,D_OFFSETS_LEDIR_OUT)
#endif
    ...
    DO KMLOC=1,D_NUMP
      ...
      BOFFSETS(KMLOC) = D%OFFSETS_LEDIR_LEGPOL(KMLOC)
      COFFSETS(KMLOC) = IOUT_STRIDES0*D_OFFSETS_LEDIR_OUT(KMLOC)
    ENDDO
    IF(IMLOC0(1) > 0) THEN
      NS(IMLOC0(1)) = 0
      KS(IMLOC0(1)) = 0
      ...
      DO JF=1,2*KF_FS
        ...
        IF (KM /= 0) THEN
          ...
          DO J=1,(R_NSMAX-KM+2)/2
            POA1(JF,IA+1+(J-1)*2,KMLOC) = ZOUT(JF+(J-1)*IOUT_STRIDES0+D_OFFSETS_LEDIR_OUT(KMLOC)*IOUT_STRIDES0)
          ENDDO
      ...
      DO JF=1,2*KF_FS
        ...
        IF (KM /= 0) THEN
          ...
          DO J=1,(R_NSMAX-KM+3)/2
            POA1(JF,IS+1+(J-1)*2,KMLOC) = ZOUT(JF+(J-1)*IOUT_STRIDES0+D_OFFSETS_LEDIR_OUT(KMLOC)*IOUT_STRIDES0)
          ENDDO
        ELSEIF (MOD(JF-1,2) == 0) THEN
#ifdef ACCGPU
          !$ACC LOOP SEQ
