...
IOUT_STRIDES0 = ALIGN(2*KF_FS,8)
IOUT_STRIDES1 = IOUT_STRIDES0 * ALIGN(MAX((R%NTMAX+2)/2,(R%NTMAX+3)/2),8)
IIN_STRIDES0 = ALIGN(2*KF_FS,8)
IIN_STRIDES1 = IIN_STRIDES0 * ALIGN(R_NDGNH,8)
IOUT0_STRIDES0 = ALIGN(KF_FS,8)
IOUT0_STRIDES1 = IOUT0_STRIDES0 * ALIGN(MAX((R%NTMAX+2)/2,(R%NTMAX+3)/2),8)
IIN0_STRIDES0 = ALIGN(KF_FS,8)
IIN0_STRIDES1 = IIN0_STRIDES0 * ALIGN(R_NDGNH,8)

ALLOCATE(ZINPA(IIN_STRIDES1*D_NUMP))
ALLOCATE(ZINPS(IIN_STRIDES1*D_NUMP))
ALLOCATE(ZOUT(IOUT_STRIDES1*D_NUMP))
ALLOCATE(ZINP0(IIN0_STRIDES1*1))
ALLOCATE(ZOUT0(IOUT0_STRIDES1*1))
...
CALL CUDA_GEMM_BATCHED( &
  ...
  & 2*KF_FS, NS(:), KS(:), &
  & 1.0_JPRBT, &
  & ZINPA, IIN_STRIDES0, IIN_STRIDES1, &
  & ZAA, SIZE(ZAA,1), SIZE(ZAA,1)*SIZE(ZAA,2), &
  & 0.0_JPRBT, &
  & ZOUT, IOUT_STRIDES0, IOUT_STRIDES1, &
  & D_NUMP)
  ...
  DO JK=1,2*KF_FS
    ...
    IF (KM /= 0) THEN
      ...
      DO J=1,(R%NSMAX-KM+2)/2
        POA1(JK,IA+1+(J-1)*2,KMLOC) = ZOUT(JK+(J-1)*IOUT_STRIDES0+(KMLOC-1)*IOUT_STRIDES1)
      ENDDO
    ENDIF
  ENDDO
ENDDO
...
IF(KMLOC0 > 0) THEN
  ...
  DO JGL=1,G_NDGLU(0)
    DO JF=1,2*KF_FS,2
      ZINP0((JF-1)/2+1+(JGL-1)*IIN0_STRIDES0) &
          & = ZINPA((JF-1)+1+(JGL-1)*IIN_STRIDES0+(KMLOC0-1)*IIN_STRIDES1)
    ENDDO
  ENDDO
  ...
  CALL CUDA_GEMM_BATCHED( &
    ...
    & 1.0_JPRD, &
    & ZINP0, IIN0_STRIDES0, IIN0_STRIDES1, &
    & ZAA0, SIZE(ZAA0,1), SIZE(ZAA0,1)*SIZE(ZAA0,2), &
    & 0.0_JPRD, &
    & ZOUT0, IOUT0_STRIDES0, IOUT_STRIDES1, &
    & 1)
  CALL cudaDeviceSynchronize()
  ...
  DO J=1,(R_NSMAX+2)/2
    DO JK=1,2*KF_FS,2
      ...
      POA1(JK,IA+1+(J-1)*2,KMLOC0) = ZOUT0((JK-1)/2+1+(J-1)*IOUT0_STRIDES0)
    ENDDO
  ENDDO
ENDIF
...
CALL CUDA_GEMM_BATCHED( &
  ...
  & 2*KF_FS, NS(:), KS(:), &
  & 1.0_JPRBT, &
  & ZINPS, IIN_STRIDES0, IIN_STRIDES1, &
  & ZAS, SIZE(ZAS,1), SIZE(ZAS,1)*SIZE(ZAS,2), &
  & 0.0_JPRBT, &
  & ZOUT, IOUT_STRIDES0, IOUT_STRIDES1, &
  & D_NUMP)
CALL cudaDeviceSynchronize()
  ...
  DO JK=1,2*KF_FS
    ...
    IF (KM /= 0) THEN
      ...
      DO J=1,(R_NSMAX-KM+3)/2
        POA1(JK,IS+1+(J-1)*2,KMLOC) = ZOUT(JK+(J-1)*IOUT_STRIDES0+(KMLOC-1)*IOUT_STRIDES1)
      ENDDO
    ENDIF
  ENDDO
ENDDO
...
IF(KMLOC0 > 0) THEN
  ...
  DO JGL=1,G_NDGLU(0)
    DO JF=1,2*KF_FS,2
      ZINP0((JF-1)/2+1+(JGL-1)*IIN0_STRIDES0) &
          & = ZINPS((JF-1)+1+(JGL-1)*IIN_STRIDES0+(KMLOC0-1)*IIN_STRIDES1)
    ENDDO
  ENDDO
  ...
  call CUDA_GEMM_BATCHED( &
    ...
    & 1.0_JPRD, &
    & ZINP0, IIN0_STRIDES0, IIN_STRIDES1, &
    & ZAS0, SIZE(ZAS0,1), SIZE(ZAS0,1)*SIZE(ZAS0,2), &
    & 0.0_JPRD, &
    & ZOUT0, IOUT0_STRIDES0, IOUT0_STRIDES1, &
    & 1)
  call cudaDeviceSynchronize()
  ...
  DO J=1,(R_NSMAX+3)/2
    DO JK=1,2*KF_FS,2
      ...
      POA1(JK,IS+1+(J-1)*2,KMLOC0) = ZOUT0((JK-1)/2+1+(J-1)*IOUT0_STRIDES0)
    ENDDO
  ENDDO
