...
END FUNCTION PREPARE_LEDIR_UNPACK
...
SUBROUTINE LEDIR_UNPACK_BUFFER(ALLOCATOR,HLEDIR_UNPACK,FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
  ...
  USE TPM_FIELDS      ,ONLY : F, F_RW, F_RACTHE
      ...
      DO JGL=1,R_NDGNH
        DO JF=1,KF_FS*2
          ...
          IF (JGL >= ISL) THEN
            ...
            ENDIF
            IF (KM /= 0) THEN
              ZINPA(JF+(JGL-ISL)*IIN_STRIDES0+(KMLOC-1)*IIN_STRIDES1)=PAIA*F_RW(JGL)
              ZINPS(JF+(JGL-ISL)*IIN_STRIDES0+(KMLOC-1)*IIN_STRIDES1)=PAIS*F_RW(JGL)
  ...
  !$ACC END DATA
#endif
END SUBROUTINE LEDIR_UNPACK_BUFFER
