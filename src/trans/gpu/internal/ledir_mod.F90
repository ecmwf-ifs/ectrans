...
CONTAINS
  SUBROUTINE SETUP_LEDIR(CONFIG)
    ...
    TYPE(LEDIR_CONFIG), INTENT(OUT) :: CONFIG
    ...
    DO KMLOC=1,D%NUMP
      ...
      IF (KM /= 0) THEN
        K_OFFSET = K_OFFSET + ALIGN(G%NDGLU(KM),A)
        ! N_OFFSET takes the max of the two GEMMs
        N_OFFSET = N_OFFSET + ALIGN((R%NSMAX-KM+3)/2,A)
      ENDIF
    ENDDO
  ...
  SUBROUTINE LEDIR_STRIDES(KF_FS,IOUT_STRIDES0,IOUT_SIZE,IIN_STRIDES0,IIN_SIZE,&
    ...
    IF (PRESENT(IOUT_SIZE)) &
        IOUT_SIZE = IOUT_STRIDES0*LEDIR_CONF%OFFSETS_N(D%NUMP+1)
    IF (PRESENT(IIN_STRIDES0)) &
        IIN_STRIDES0 = ALIGN(2*KF_FS,A)
    IF (PRESENT(IIN_SIZE)) &
        IIN_SIZE = IIN_STRIDES0*LEDIR_CONF%OFFSETS_K(D%NUMP+1)
  ...
  SUBROUTINE LEDIR(ZINPS,ZINPA,ZINPS0,ZINPA0,ZOUT,ZOUT0,POA1,KF_FS)
    ...
    !$ACC& PRESENT(ZAA,ZAS,POA1,LEDIR_CONF)
    ...
    DO KMLOC=1,D_NUMP
      ...
      KS(KMLOC) = G%NDGLU(KM)
      AOFFSETS(KMLOC) = IIN_STRIDES0*LEDIR_CONF%OFFSETS_K(KMLOC)
      BOFFSETS(KMLOC) = SIZE(ZAA,1)*SIZE(ZAA,2)*(KMLOC-1)
      COFFSETS(KMLOC) = IOUT_STRIDES0*LEDIR_CONF%OFFSETS_N(KMLOC)
    ENDDO
    IF(KMLOC0 > 0) THEN
      NS(KMLOC0) = 0
      KS(KMLOC0) = 0
      ...
      DO JF=1,2*KF_FS
        ...
        IF (KM /= 0) THEN
          ...
          DO J=1,(R%NSMAX-KM+2)/2
            POA1(JF,IA+1+(J-1)*2,KMLOC) = ZOUT(JF+(J-1)*IOUT_STRIDES0+LEDIR_CONF%OFFSETS_N(KMLOC)*IOUT_STRIDES0)
          ENDDO
      ...
      DO JF=1,2*KF_FS
        ...
        IF (KM /= 0) THEN
          ...
          DO J=1,(R_NSMAX-KM+3)/2
            POA1(JF,IS+1+(J-1)*2,KMLOC) = ZOUT(JF+(J-1)*IOUT_STRIDES0+LEDIR_CONF%OFFSETS_N(KMLOC)*IOUT_STRIDES0)
          ENDDO
        ELSEIF (MOD(JF-1,2) == 0) THEN
          !$ACC LOOP SEQ
          DO J=1,(R_NSMAX+3)/2
