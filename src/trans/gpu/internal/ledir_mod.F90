...
#ifdef OMPGPU
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) PRIVATE(KM,ISKIP,ILA,IA,ILS) DEFAULT(NONE) &
...
#ifdef OMPGPU
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) PRIVATE(KM,ISKIP,ILA,IS,ILS) DEFAULT(NONE) &
  !$OMP&    SHARED(D_NUMP,R_NTMAX,KFC,D_MYMS,POA1,DZCST,DLDZCS,DTDZCS)
#endif
#ifdef ACCGPU
  !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(KM,ISKIP,ILA,ILS,IS) DEFAULT(NONE) &
