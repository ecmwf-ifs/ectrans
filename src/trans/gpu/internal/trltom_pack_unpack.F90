      ...
      DO JGL=1,R_NDGNH
        DO JF=1,KF_FS*2
          ...
          IF (JGL >= ISL) THEN
            ...
            ENDIF
            IF (KM /= 0) THEN
              ZINPA(JF+(JGL-ISL)*IIN_STRIDES0+IIN_STRIDES0*LEDIR_CONF%OFFSETS_K(KMLOC))=PAIA*F_RW(JGL)
              ZINPS(JF+(JGL-ISL)*IIN_STRIDES0+IIN_STRIDES0*LEDIR_CONF%OFFSETS_K(KMLOC))=PAIS*F_RW(JGL)
