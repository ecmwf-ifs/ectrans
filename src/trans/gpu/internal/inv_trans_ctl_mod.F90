...
!     ------------------------------------------------------------------
  ...
  IF (KF_FS > 0) CALL FOURIER_IN(ALLOCATOR,HFOURIER_IN,FOUBUF,PREEL_COMPLEX,KFIELD/2,KF_FS)

  !    2.  Fourier space computations

  ! fill the rest of PREEL_COMPLEX
  IF (KF_FS > 0) CALL FSC(ALLOCATOR,HFSC,PREEL_COMPLEX, KF_FS, KF_UV, KF_SCALARS, KUV_OFFSET, KSCALARS_OFFSET, &
