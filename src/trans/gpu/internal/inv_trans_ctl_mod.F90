...
INTEGER(KIND=JPIM) :: IOFFD,IOFFU,IOFFV,IOFFUVD,IOFFSC,IOFFSCNS,IOFFSCEW,IOFF,IF_GPB,IST
...
IF(NPROMATR > 0 .AND. IF_GPB > NPROMATR) THEN
  print *, "This is currently not supported and/or tested"
  stop 24
 ...
 & KVSETUV,KVSETSC)
  ...
  DO JBLK=1,IBLKS
    ...
    CALL LTINV_CTL(IF_UV,IF_SCALARS, &
     & PSPVOR=PSPVOR,PSPDIV=PSPDIV,PSPSCALAR=PSPSCALAR,FOUBUF=FOUBUF,&
     & KFLDPTRUV=IPTRSPUV,KFLDPTRSC=IPTRSPSC,FSPGL_PROC=FSPGL_PROC,KFIELD=KFIELD)

    IF(IF_UV_G > 0 .AND. IF_SCALARS_G > 0) THEN
      CALL FTINV_CTL(KFIELD,IF_UV_G,IF_SCALARS_G,&
       & IF_UV,IF_SCALARS,IF_GP,&
       & FOUBUF,KVSETUV=IVSETUV(ISTUV_G:IENUV_G),&
       & KVSETSC=IVSETSC(ISTSC_G:IENSC_G),KPTRGP=IPTRGP,&
       & PGP=PGP)
    ELSEIF(IF_UV_G > 0) THEN
      CALL FTINV_CTL(KFIELD,IF_UV_G,IF_SCALARS_G,&
       & IF_UV,IF_SCALARS,IF_GP,&
       & FOUBUF,KVSETUV=IVSETUV(ISTUV_G:IENUV_G),KPTRGP=IPTRGP,&
       & PGP=PGP)
    ELSEIF(IF_SCALARS_G > 0) THEN
      CALL FTINV_CTL(KFIELD,IF_UV_G,IF_SCALARS_G,&
       & IF_UV,IF_SCALARS,IF_GP,&
       & FOUBUF,KVSETSC=IVSETSC(ISTSC_G:IENSC_G),KPTRGP=IPTRGP,&
       & PGP=PGP)
    ENDIF
  ENDDO
...
ELSE
  ...
  CALL LTINV_CTL(KF_UV,KF_SCALARS, &
          & PSPVOR,PSPDIV,PSPSCALAR,PSPSC3A,PSPSC3B,PSPSC2,FOUBUF=FOUBUF, FSPGL_PROC=FSPGL_PROC, KFIELD=KFIELD)

  ! from FOUBUF to PGPXXX
  CALL FTINV_CTL(KFIELD,KF_UV_G,KF_SCALARS_G,&
   & KF_UV,KF_SCALARS,KF_GP,&
   & FOUBUF, KVSETUV=KVSETUV,KVSETSC=KVSETSC,&
