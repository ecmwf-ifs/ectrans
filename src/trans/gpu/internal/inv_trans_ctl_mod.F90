...
INTEGER(KIND=JPIM) :: KFIELD

REAL(KIND=JPRB), ALLOCATABLE :: FOUBUF(:)
...
!     ------------------------------------------------------------------
  ...
  HLTINV = PREPARE_LTINV(ALLOCATOR,KF_GP,KF_FS)
  HTRMTOL = PREPARE_TRMTOL(ALLOCATOR,KF_FS)
  HFOURIER_IN = PREPARE_FOURIER_IN(ALLOCATOR)
  HFSC = PREPARE_FSC(ALLOCATOR)
  HFTINV = PREPARE_FTINV(ALLOCATOR)
  HTRLTOG = PREPARE_TRLTOG(ALLOCATOR)
  ...
  CALL TRMTOL_CUDAAWARE(ALLOCATOR,HTRMTOL,FOUBUF_IN,FOUBUF,KFIELD)
  CALL GSTATS(152,1)
  ...
  IF (2*IFIRST /= KFIELD) CALL ABORT_TRANS('Size mismatch: LT and FT do not agree on input size')
  IF (LUVDER) IFIRST = IFIRST+2*KF_UV ! U and V derivatives
  IF (LSCDERS) IFIRST = IFIRST + KF_SCALARS ! Scalars EW Derivatives
  IF (IFIRST /= KF_FS) CALL ABORT_TRANS('Size mismatch: Wrong computation KF_FS')

  IF (.NOT. ALLOCATED(REUSE_PTR)) THEN
    ALLOCATE(REUSE_PTR(KF_FS*D%NLENGTF))
    !$ACC ENTER DATA CREATE(REUSE_PTR)
  ELSEIF (SIZE(REUSE_PTR) < KF_FS*D%NLENGTF) THEN
    !$ACC EXIT DATA DELETE(REUSE_PTR)
    DEALLOCATE(REUSE_PTR)
    ALLOCATE(REUSE_PTR(KF_FS*D%NLENGTF))
    !$ACC ENTER DATA CREATE(REUSE_PTR)
  ENDIF
  PREEL_COMPLEX => REUSE_PTR
  ...
  CALL FOURIER_IN(ALLOCATOR,HFOURIER_IN,FOUBUF,PREEL_COMPLEX,KFIELD/2,KF_FS)

  !    2.  Fourier space computations

  ! fill the rest of PREEL_COMPLEX
  CALL FSC(ALLOCATOR,HFSC,PREEL_COMPLEX, KF_FS, KF_UV, KF_SCALARS, KUV_OFFSET, KSCALARS_OFFSET, &
      & KSCALARS_NSDER_OFFSET, KUV_EWDER_OFFSET, KSCALARS_EWDER_OFFSET)

  !   3.  Fourier transform
  ! inplace operation
  IF (KF_FS > 0) CALL FTINV(ALLOCATOR, HFTINV, PREEL_COMPLEX,PREEL_REAL,KF_FS)
  ...
  CALL GSTATS(157,0)
  CALL TRLTOG_CUDAAWARE(ALLOCATOR,HTRLTOG,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,IVSET,&
