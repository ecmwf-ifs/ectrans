...
IF( NPROC > 1 )THEN
  DO JGL=1,ILEN
    IDIVID(JGL) = NINT(360000.0_JPRBT/REAL( G%NLOEN(D%NFRSTLAT(MY_REGION_NS)+JGL-1),JPRBT))
  ENDDO
      ...
      DO JNPTSRE=1,IPTSRE
        ...
        DO JGL=1,ILEN
          IF (IXPTLAT(JGL)  <=  ILSTPTLAT(JGL)) THEN
            ZLON = REAL(IXPTLAT(JGL)-1,JPRBT)*2.0*ZPI/REAL(G%NLOEN(D%NFRSTLAT(MY_REGION_NS)+JGL-1),JPRBT)
            ILATMD1 = NINT(ZLON*180000.0_JPRBT/ZPI)
