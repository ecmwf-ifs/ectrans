...
USE TPM_TRANS       ,ONLY : NF_SC2, NF_SC3A, NF_SC3B, REUSE_PTR
...
USE LEDIR_MOD       ,ONLY : LEDIR_UNPACK_BUFFER, PREPARE_LEDIR_UNPACK, LEDIR_UNPACK_HANDLE
...
INTEGER(KIND=C_INT8_T), POINTER :: PTR(:)
  ...
  IF(ALLOCATED(REUSE_PTR)) &
    CALL C_F_POINTER(C_LOC(REUSE_PTR), PTR, (/ SIZEOF(REUSE_PTR) /) )
  CALL INSTANTIATE_ALLOCATOR(ALLOCATOR, PTR)
  IF (ASSOCIATED(PTR)) &
    CALL C_F_POINTER(C_LOC(PTR), REUSE_PTR, (/ SIZEOF(PTR)/SIZEOF(REUSE_PTR(1)) /) )
  ...
  ELSE
    ...
    CALL LEDIR_UNPACK_BUFFER(ALLOCATOR,HLEDIR_UNPACK,PREEL_COMPLEX,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
  ENDIF
  CALL GSTATS(153,1)
