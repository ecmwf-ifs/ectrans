...
USE FTDIR_CTL_MOD   ,ONLY : GRID_TO_FOURIER
...
USE LEDIR_MOD   ,ONLY : LEDIR_POINTERS, LEDIR_UNPACK_BUFFER, LEDIR_ALLOC_SIZE
!
...
IALLOC_SZ=KF_FS*D%NLENGTF


IF (NPROC == 1) THEN
  ! If we have only one process, we skip the intermediate buffer, so we need more space
  IALLOC_SZ = IALLOC_SZ + LEDIR_ALLOC_SIZE(KF_FS)/SIZEOF(REUSE_PTR(1))
ELSE
  IALLOC_SZ = MAX(IALLOC_SZ, LEDIR_ALLOC_SIZE(KF_FS)/SIZEOF(REUSE_PTR(1)))
ENDIF

IF (.NOT. ALLOCATED(REUSE_PTR)) THEN
  ALLOCATE(REUSE_PTR(IALLOC_SZ))
  !$ACC ENTER DATA CREATE(REUSE_PTR)
ELSEIF (SIZE(REUSE_PTR) < IALLOC_SZ) THEN
  !$ACC EXIT DATA DELETE(REUSE_PTR)
  DEALLOCATE(REUSE_PTR)
  ALLOCATE(REUSE_PTR(IALLOC_SZ))
  !$ACC ENTER DATA CREATE(REUSE_PTR)
ENDIF
PREEL_REAL => REUSE_PTR
...
ELSE
  ...
  CALL GRID_TO_FOURIER(KF_UV_G,KF_SCALARS_G,KF_GP,KF_FS,PREEL_REAL,&
  ...
  CALL GSTATS(1640,0)
  IF (KF_FS > 0) THEN
    CALL FTDIR(PREEL_REAL,PREEL_COMPLEX,KF_FS)
  ELSE
  ...
  CALL GSTATS(153,0)
  IF (NPROC > 1) THEN
    CALL FOURIER_OUT(PREEL_COMPLEX,FOUBUF_IN,KF_FS)
    ...
    CALL LEDIR_POINTERS(REUSE_PTR(1:),KF_FS,ZINPS,ZINPA,ZOUT,ZINPS0,ZINPA0,ZOUT0)
    IF (KF_FS > 0) CALL LEDIR_UNPACK_BUFFER(FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
  ...
  ELSE
    ...
    IALLOC_POS = 1+KF_FS*D%NLENGTF
    CALL LEDIR_POINTERS(REUSE_PTR(IALLOC_POS:),KF_FS,ZINPS,ZINPA,ZOUT,ZINPS0,ZINPA0,ZOUT0)
    IF (KF_FS > 0) CALL LEDIR_UNPACK_BUFFER(FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
  ENDIF
  CALL GSTATS(153,1)

  IF (KF_FS > 0) THEN
    CALL LTDIR(ZINPS,ZINPA,ZOUT,ZINPS0,ZINPA0,ZOUT0,KF_FS,KF_UV,KF_SCALARS, &
          & PSPVOR,PSPDIV,PSPSCALAR,&
          & PSPSC3A,PSPSC3B,PSPSC2)
     !! is this barrier still useful?
     IF (LSYNC_TRANS) THEN
