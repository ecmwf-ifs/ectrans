...
USE LEDIR_MOD   ,ONLY : LEDIR_POINTERS, LEDIR_PACK_BUFFER, LEDIR_ALLOC_SIZE
!
...
ELSE
  ...
  CALL FTDIR_CTL(KF_UV_G,KF_SCALARS_G,KF_GP,KF_FS,PREEL_COMPLEX,&
  ...
  IF (NPROC > 1) THEN
    ...
    ALLOC_SZ = LEDIR_ALLOC_SIZE(KF_FS)
    ALLOC_POS  =1
    ...
    IF (KF_FS > 0) CALL LEDIR_PACK_BUFFER(FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
