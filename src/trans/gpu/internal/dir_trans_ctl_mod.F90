...
USE TPM_GEN         ,ONLY : NPROMATR, NOUT, LSYNC_TRANS
...
USE LTDIR_MOD       ,ONLY : LTDIR, PREPARE_LTDIR
USE LEDIR_MOD       ,ONLY : LEDIR_HANDLE
USE FTDIR_CTL_MOD   ,ONLY : GRID_TO_FOURIER, PREPARE_GRID_TO_FOURIER, GRID_TO_FOURIER_HANDLE
USE MPL_MODULE      ,ONLY : MPL_BARRIER
USE ABORT_TRANS_MOD ,ONLY : ABORT_TRANS
USE TRLTOM_MOD      ,ONLY : TRLTOM_CUDAAWARE, TRLTOM_HANDLE, PREPARE_TRLTOM
USE FOURIER_OUT_MOD ,ONLY : FOURIER_OUT, FOURIER_OUT_HANDLE, PREPARE_FOURIER_OUT

USE TPM_DISTR       ,ONLY : D, NPROC
USE LEDIR_MOD       ,ONLY : LEDIR_POINTERS, LEDIR_UNPACK_BUFFER, LEDIR_ALLOC_SIZE, PREPARE_LEDIR_UNPACK, LEDIR_UNPACK_HANDLE
...
INTEGER(KIND=8)  :: IALLOC_SZ, IALLOC_POS
...
TYPE(GRID_TO_FOURIER_HANDLE) :: HGRID_TO_FOURIER
...
TYPE(LEDIR_HANDLE) :: HLEDIR

  IF(NPROMATR > 0) THEN
    CALL ABORT_TRANS('ERROR, not implemented right now (NPROMATR > 0)')
  ENDIF

  ! Prepare everything
  ALLOCATOR = MAKE_BUFFERED_ALLOCATOR()
  HGRID_TO_FOURIER = PREPARE_GRID_TO_FOURIER(ALLOCATOR,KF_GP,KF_FS)
  HFTDIR = PREPARE_FTDIR()
  IF (NPROC > 1) THEN
    HFOURIER_OUT = PREPARE_FOURIER_OUT(ALLOCATOR, KF_FS)
    HTRLTOM = PREPARE_TRLTOM(ALLOCATOR, KF_FS)
    HLEDIR_UNPACK = PREPARE_LEDIR_UNPACK(ALLOCATOR, KF_FS)
  ELSE
    HLEDIR_UNPACK = PREPARE_LEDIR_UNPACK(ALLOCATOR, KF_FS)
  ENDIF
  HLEDIR = PREPARE_LTDIR(ALLOCATOR, KF_FS, KF_UV)

  CALL INSTANTIATE_ALLOCATOR(ALLOCATOR)

  ! from the PGP arrays to PREEL_REAL
  CALL GRID_TO_FOURIER(ALLOCATOR,HGRID_TO_FOURIER,KF_UV_G,KF_SCALARS_G,KF_GP,KF_FS,PREEL_REAL,&
  ...
  IF (NPROC > 1) THEN
    ...
    IF (KF_FS > 0) THEN
      CALL FOURIER_OUT(ALLOCATOR,HFOURIER_OUT,PREEL_COMPLEX,FOUBUF_IN,KF_FS)
      CALL TRLTOM_CUDAAWARE(ALLOCATOR,HTRLTOM,FOUBUF_IN,FOUBUF,KF_FS)
      CALL LEDIR_UNPACK_BUFFER(ALLOCATOR,HLEDIR_UNPACK,FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
    ENDIF
  ...
  IF (KF_FS > 0) THEN
    CALL LTDIR(ALLOCATOR,HLEDIR,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV,KF_SCALARS, &
...
#endif
#ifdef ACCGPU
  !$ACC EXIT DATA DELETE(ALLOCATOR%PTR)
#endif
