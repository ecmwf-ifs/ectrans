...
!$OMP& SHARED(PREEL_COMPLEX) &
!$OMP& ECTRANS_LOOP_BOUNDS_CLAUSE(IBEG,IEND,IINC,KF_UV) FIRSTPRIVATE(OFFSET_VAR,KF_FS,KUV_OFFSET)
#endif
#ifdef ACCGPU
...
IF (KSCALARS_NSDER_OFFSET >= 0) THEN
  ...
  !$OMP& SHARED(PREEL_COMPLEX) &
  !$OMP& ECTRANS_LOOP_BOUNDS_CLAUSE(IBEG,IEND,IINC,KF_SCALARS) &
  !$OMP& FIRSTPRIVATE(OFFSET_VAR,KF_FS,KSCALARS_NSDER_OFFSET)
#endif
#ifdef ACCGPU
...
IF (KUV_EWDER_OFFSET >= 0) THEN
  ...
  !$OMP& SHARED(PREEL_COMPLEX) &
  !$OMP& ECTRANS_LOOP_BOUNDS_CLAUSE(IBEG,IEND,IINC,KF_UV,ILOEN_MAX) &
  !$OMP& FIRSTPRIVATE(OFFSET_VAR,KF_FS,KUV_OFFSET,KUV_EWDER_OFFSET)
#endif
#ifdef ACCGPU
...
IF (KSCALARS_EWDER_OFFSET > 0) THEN
  ...
  !$OMP& SHARED(PREEL_COMPLEX) &
  !$OMP& ECTRANS_LOOP_BOUNDS_CLAUSE(IBEG,IEND,IINC,KF_SCALARS,ILOEN_MAX) &
  !$OMP& FIRSTPRIVATE(OFFSET_VAR,KF_FS,KSCALARS_EWDER_OFFSET,KSCALARS_OFFSET)
#endif
#ifdef ACCGPU
