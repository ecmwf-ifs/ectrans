...
_DIR _DATA _PRESENT(D,D_NPTRLS,D_NSTAGTF,PREEL_COMPLEX,F,F_RACTHE,G,G_NMEN,G_NLOEN,R,R_NSMAX)
...
!*       1.1      U AND V.
_DIR _PARALLEL COLLAPSE(3) DEFAULT(NONE) PRIVATE(IGLG,IOFF_LAT,IOFF_UV,ZACHTE2,JM,JF,KGL) &
_DIR& _FIRSTPRIVATE(IBEG,IEND,IINC,KF_UV,OFFSET_VAR,KF_FS,KUV_OFFSET) &
_DIR& _SHARED(IBEG,IEND,IINC,KF_UV,R,OFFSET_VAR,G,D,KF_FS,KUV_OFFSET,F,PREEL_COMPLEX) _ASYNC(1)
...
IF (KSCALARS_NSDER_OFFSET >= 0) THEN
  _DIR _PARALLEL COLLAPSE(3) DEFAULT(NONE) &
  _DIR& PRIVATE(IGLG,IOFF_LAT,IOFF_KSCALARS_NSDER,ZACHTE2,KGL,JF,JM) &
  _DIR& _SHARED(IBEG,IEND,IINC,KF_SCALARS,R,OFFSET_VAR,G,D,KF_FS,KSCALARS_NSDER_OFFSET,F) &
  _DIR& _SHARED(PREEL_COMPLEX) &
  _DIR& _FIRSTPRIVATE(IBEG,IEND,IINC,OFFSET_VAR,KF_SCALARS,KSCALARS_NSDER_OFFSET,KF_FS) _ASYNC(1)
  DO KGL=IBEG,IEND,IINC
    DO JF=1,KF_SCALARS
      DO JM=0,R_NSMAX !(note that R_NSMAX <= G_NMEN(IGLG) for all IGLG)
        IGLG = OFFSET_VAR+KGL-1
...
IF (KUV_EWDER_OFFSET >= 0) THEN
  _DIR _PARALLEL COLLAPSE(3) DEFAULT(NONE) &
  _DIR& PRIVATE(IGLG,IOFF_LAT,IOFF_UV,IOFF_UV_EWDER,RET_REAL,RET_COMPLEX,ZACHTE2,JM,JF,KGL) &
  _DIR& _SHARED(IBEG,IEND,IINC,KF_UV,ILOEN_MAX,OFFSET_VAR,G,D,KF_FS,KUV_OFFSET,KUV_EWDER_OFFSET) &
  _DIR& _SHARED(F,PREEL_COMPLEX) &
  _DIR& _FIRSTPRIVATE(IBEG,IEND,IINC,OFFSET_VAR,KF_UV,KUV_EWDER_OFFSET,KUV_OFFSET,KF_FS,ILOEN_MAX) &
  _DIR& _ASYNC(1)
  DO KGL=IBEG,IEND,IINC
    DO JF=1,2*KF_UV
      DO JM=0,ILOEN_MAX/2
...
IF (KSCALARS_EWDER_OFFSET > 0) THEN
  _DIR _PARALLEL COLLAPSE(3) DEFAULT(NONE) &
  _DIR& PRIVATE(IGLG,IOFF_LAT,IOFF_SCALARS_EWDER,IOFF_SCALARS,ZACHTE2,RET_REAL,RET_COMPLEX) &
  _DIR& _SHARED(IBEG,IEND,IINC,KF_SCALARS,ILOEN_MAX,OFFSET_VAR,G,D,KF_FS,KSCALARS_EWDER_OFFSET) &
  _DIR& _SHARED(KSCALARS_OFFSET,F,PREEL_COMPLEX) &
  _DIR& _FIRSTPRIVATE(IBEG,IEND,IINC,KF_SCALARS,OFFSET_VAR,KSCALARS_EWDER_OFFSET,KSCALARS_OFFSET) &
  _DIR& _FIRSTPRIVATE(KF_FS,ILOEN_MAX) _ASYNC(1)
  DO KGL=IBEG,IEND,IINC
    DO JF=1,KF_SCALARS
      DO JM=0,ILOEN_MAX/2
        IGLG = OFFSET_VAR+KGL-1
