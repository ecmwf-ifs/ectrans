...
#ifdef ACCGPU
!$ACC DATA &
!$ACC& PRESENT(D,G,F,D_NSTAGTF,G_NMEN,F_RACTHE,PUV,PSCALARS,PSCALARS_NSDER,PUV_EWDER,PSCALARS_EWDER)
#endif
#ifdef OMPGPU
...
DO KGL=IBEG,IEND,IINC
  DO JF=1,2*KF_UV
    ...
    !$ACC LOOP SEQ
    DO JM=0,G_NMEN(IGLG)
      PUV(2*JF-1,JM+IOFF) = PUV(2*JF-1,JM+IOFF)*ZACHTE2
      PUV(2*JF,  JM+IOFF) = PUV(2*JF  ,JM+IOFF)*ZACHTE2
    ENDDO
  ENDDO
ENDDO
...
IF (ASSOCIATED(PSCALARS_NSDER)) THEN
  ...
  DO KGL=IBEG,IEND,IINC
    DO JF=1,KF_SCALARS
      ...
      !$ACC LOOP SEQ
      DO JM=0,G_NMEN(IGLG)
        PSCALARS_NSDER(2*JF-1,JM+IOFF) = PSCALARS_NSDER(2*JF-1,JM+IOFF)*ZACHTE2
        PSCALARS_NSDER(2*JF,  JM+IOFF) = PSCALARS_NSDER(2*JF,  JM+IOFF)*ZACHTE2
      ENDDO
    ENDDO
  ENDDO
ENDIF
...
IF (ASSOCIATED(PUV_EWDER)) THEN
  ...
  DO KGL=IBEG,IEND,IINC
    DO JF=1,2*KF_UV
      ...
      !$ACC LOOP SEQ
      DO JM=0,G_NMEN(IGLG)
        PUV_EWDER(2*JF-1,JM+IOFF) = -PUV(2*JF,  JM+IOFF)*ZACHTE2*REAL(JM,JPRBT)
        PUV_EWDER(2*JF,  JM+IOFF) =  PUV(2*JF-1,JM+IOFF)*ZACHTE2*REAL(JM,JPRBT)
      ENDDO
    ENDDO
  ENDDO
ENDIF
...
IF (ASSOCIATED(PSCALARS_EWDER)) THEN
  ...
  DO KGL=IBEG,IEND,IINC
    DO JF=1,KF_SCALARS
      ...
      !$ACC LOOP SEQ
      DO JM=0,G_NMEN(IGLG)
        PSCALARS_EWDER(2*JF-1,JM+IOFF) = -PSCALARS(2*JF  ,JM+IOFF)*ZACHTE2*REAL(JM,JPRBT)
        PSCALARS_EWDER(2*JF,  JM+IOFF) =  PSCALARS(2*JF-1,JM+IOFF)*ZACHTE2*REAL(JM,JPRBT)
      ENDDO
    ENDDO
  ENDDO
ENDIF
