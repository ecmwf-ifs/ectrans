...
!$OMP& MAP(TO:IBEG,IEND,IINC,OFFSET_VAR,KF_UV,KUV_OFFSET,KF_FS) &
...
IF (KSCALARS_NSDER_OFFSET >= 0) THEN
  ...
  ! TODO: Make this asynchronous (pending correctness)
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) &
  !$OMP& MAP(TO:IBEG,IEND,IINC,OFFSET_VAR,KF_SCALARS,KSCALARS_NSDER_OFFSET,KF_FS) &
...
IF (KUV_EWDER_OFFSET >= 0) THEN
  ...
  ! TODO: Make this asynchronous (pending correctness)
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) &
  !$OMP& MAP(TO:IBEG,IEND,IINC,OFFSET_VAR,KF_UV,KUV_EWDER_OFFSET,KUV_OFFSET,KF_FS,ILOEN_MAX) &
...
IF (KSCALARS_EWDER_OFFSET > 0) THEN
  ...
  ! TODO: Make this asynchronous (pending correctness)
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) &
  !$OMP& MAP(TO:IBEG,IEND,IINC,KF_SCALARS,OFFSET_VAR,KSCALARS_EWDER_OFFSET,KSCALARS_OFFSET,KF_FS,ILOEN_MAX) &
