...
DO KGL=IBEG,IEND,IINC
  DO JF=1,2*KF_UV
    ...
    !$ACC LOOP SEQ
    DO JM=0,G_NMEN(IGLG)
      PREEL_COMPLEX(KUV_OFFSET+2*JF-1+2*KF_FS*(JM+IOFF_COMPLEX)) = &
          & PREEL_COMPLEX(KUV_OFFSET+2*JF-1+2*KF_FS*(JM+IOFF_COMPLEX))*ZACHTE2
      PREEL_COMPLEX(KUV_OFFSET+2*JF  +2*KF_FS*(JM+IOFF_COMPLEX)) = &
          & PREEL_COMPLEX(KUV_OFFSET+2*JF  +2*KF_FS*(JM+IOFF_COMPLEX))*ZACHTE2
    ENDDO
  ENDDO
ENDDO
...
IF (KSCALARS_NSDER_OFFSET >= 0) THEN
  ...
  DO KGL=IBEG,IEND,IINC
    DO JF=1,KF_SCALARS
      ...
      !$ACC LOOP SEQ
      DO JM=0,G_NMEN(IGLG)
        PREEL_COMPLEX(KSCALARS_NSDER_OFFSET+2*JF-1+2*KF_FS*(JM+IOFF_COMPLEX)) = &
            & PREEL_COMPLEX(KSCALARS_NSDER_OFFSET+2*JF-1+2*KF_FS*(JM+IOFF_COMPLEX))*ZACHTE2
        PREEL_COMPLEX(KSCALARS_NSDER_OFFSET+2*JF  +2*KF_FS*(JM+IOFF_COMPLEX)) = &
            & PREEL_COMPLEX(KSCALARS_NSDER_OFFSET+2*JF  +2*KF_FS*(JM+IOFF_COMPLEX))*ZACHTE2
      ENDDO
    ENDDO
  ENDDO
ENDIF
...
IF (KUV_EWDER_OFFSET >= 0) THEN
  ...
  DO KGL=IBEG,IEND,IINC
    DO JF=1,2*KF_UV
      ...
      !$ACC LOOP SEQ
      DO JM=0,G_NMEN(IGLG)
        PREEL_COMPLEX(KUV_EWDER_OFFSET+2*JF-1+2*KF_FS*(JM+IOFF_COMPLEX)) = &
            & -PREEL_COMPLEX(KUV_OFFSET+2*JF  +2*KF_FS*(JM+IOFF_COMPLEX))*ZACHTE2*REAL(JM,JPRBT)
        PREEL_COMPLEX(KUV_EWDER_OFFSET+2*JF  +2*KF_FS*(JM+IOFF_COMPLEX)) =  &
            &  PREEL_COMPLEX(KUV_OFFSET+2*JF-1+2*KF_FS*(JM+IOFF_COMPLEX))*ZACHTE2*REAL(JM,JPRBT)
      ENDDO
      !$ACC LOOP SEQ
      DO JM=G_NMEN(IGLG)+1,(G_NLOEN(IGLG)+4)/2-1
        PREEL_COMPLEX(KUV_EWDER_OFFSET+2*JF-1+2*KF_FS*(JM+IOFF_COMPLEX)) = 0._JPRBT
        PREEL_COMPLEX(KUV_EWDER_OFFSET+2*JF  +2*KF_FS*(JM+IOFF_COMPLEX)) = 0._JPRBT
      ENDDO
    ENDDO
  ENDDO
ENDIF
...
IF (KSCALARS_EWDER_OFFSET > 0) THEN
  ...
  DO KGL=IBEG,IEND,IINC
    DO JF=1,KF_SCALARS
      ...
      !$ACC LOOP SEQ
      DO JM=0,G_NMEN(IGLG)
        PREEL_COMPLEX(KSCALARS_EWDER_OFFSET+2*JF-1+2*KF_FS*(JM+IOFF_COMPLEX)) = &
            & -PREEL_COMPLEX(KSCALARS_OFFSET+2*JF  +2*KF_FS*(JM+IOFF_COMPLEX))*ZACHTE2*REAL(JM,JPRBT)
        PREEL_COMPLEX(KSCALARS_EWDER_OFFSET+2*JF  +2*KF_FS*(JM+IOFF_COMPLEX)) = &
            &  PREEL_COMPLEX(KSCALARS_OFFSET+2*JF-1+2*KF_FS*(JM+IOFF_COMPLEX))*ZACHTE2*REAL(JM,JPRBT)
      ENDDO
      !$ACC LOOP SEQ
      DO JM=G_NMEN(IGLG)+1,(G_NLOEN(IGLG)+4)/2-1
        PREEL_COMPLEX(KSCALARS_EWDER_OFFSET+2*JF-1+2*KF_FS*(JM+IOFF_COMPLEX)) = 0._JPRBT
        PREEL_COMPLEX(KSCALARS_EWDER_OFFSET+2*JF  +2*KF_FS*(JM+IOFF_COMPLEX)) = 0._JPRBT
      ENDDO
    ENDDO
  ENDDO
ENDIF
