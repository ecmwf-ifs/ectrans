...
MODULE LTDIR_CTL_MOD
  ...
  USE TRLTOM_MOD      ,ONLY : TRLTOM_CUDAAWARE
#else
  USE TRLTOM_MOD      ,ONLY : TRLTOM
#endif

  USE TPM_FIELDS      ,ONLY : ZOA1,ZEPSNM
 
  IMPLICIT NONE
 
  INTEGER(KIND=JPIM),INTENT(IN) :: KF_FS,KF_UV,KF_SCALARS
  REAL(KIND=JPRBT),  INTENT(INOUT), ALLOCATABLE :: FOUBUF_IN(:)
  ...
  IF(KF_FS>0) THEN
    CALL LTDIR(KF_FS,KF_UV,KF_SCALARS, &
          & PSPVOR,PSPDIV,PSPSCALAR,&
          & PSPSC3A,PSPSC3B,PSPSC2 , &
          & KFLDPTRUV,KFLDPTRSC)
  ENDIF
