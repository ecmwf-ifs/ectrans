...
CONTAINS
  ...
  SUBROUTINE TRLTOMAD_PACK(ALLOCATOR,HTRLTOM_PACK,PREEL_COMPLEX,FOUBUF_IN,KF_FS)
    ...
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(ECTRANS_OMP_DEFAULT) SHARED(N,PREEL_COMPLEX)
#endif
    DO J=1,N
      PREEL_COMPLEX(J) = 0
    ENDDO
...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(ECTRANS_MAP_PRESENT_ALLOC:G,G_NMEN,D,D_NPNTGTB0,FOUBUF_IN,PREEL_COMPLEX,D_NSTAGTF,&
...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(ECTRANS_MAP_PRESENT_ALLOC:ZINPS,ZINPA,ZINPS0,ZINPA0) &
    !$OMP& MAP(ECTRANS_MAP_PRESENT_ALLOC:F,F_RW,F_RACTHE) &
    !$OMP& MAP(ECTRANS_MAP_PRESENT_ALLOC:D,D_MYMS,D_NUMP,R,R_NDGNH,R_NDGL,G,G_NDGLU) &
    !$OMP& MAP(ECTRANS_MAP_PRESENT_ALLOC:D_NPNTGTB1,D_OFFSETS_GEMM1,FOUBUF)
