...
IF (KF_UV > 0) THEN
   ...
   IDIVS = 2*KF_UV+1
   IDIVE = 4*KF_UV
   !$ACC UPDATE HOST(POA2(IVORS:IVORE,:,:))
   !$OMP TARGET UPDATE FROM(POA2(IVORS:IVORE,:,:))
   CALL UPDSPB(KF_UV,POA2(IVORS:IVORE,:,:),PSPVOR,KFLDPTRUV)
   !$ACC UPDATE HOST(POA2(IDIVS:IDIVE,:,:))
   !$OMP TARGET UPDATE FROM(POA2(IDIVS:IDIVE,:,:))
...
IF (KF_SCALARS > 0) THEN
  IF(PRESENT(PSPSCALAR)) THEN
    ...
    !$ACC UPDATE HOST(POA1(IST:IEND,:,:))
    !$OMP TARGET UPDATE FROM(POA1(IST:IEND,:,:))
