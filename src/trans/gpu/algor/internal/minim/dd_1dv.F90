      SUBROUTINE DD_1DV (PROSCA,CTONB_1DV,CTCAB_1DV,N,SSCALE,NM,DEPL,   &
     &               AUX,JMIN,JMAX,                                     &
...
!
      ...
      REAL(KIND=JPRB) :: YBAR(N,N)
      REAL(KIND=JPRB) :: SBAR(N,N)
...
!
      ...
      IF (LHOOK) CALL DR_HOOK('DD_1DV',0,ZHOOK_HANDLE)
      JFIN=JMAX
...
!
      DO 100 J=JFIN,JMIN,-1
          ...
          CALL PROSCA (N,DEPL,SBAR(1,JP),PS)
          R=PS
          ALPHA(JP)=R
          DO 20 I=1,N
...
!
      DO 200 J=JMIN,JFIN
          JP=J
          IF (JP.GT.NM) JP=JP-NM
          CALL PROSCA (N,DEPL,YBAR(1,JP),PS)
          R=ALPHA(JP)-PS
          DO 120 I=1,N
              DEPL(I)=DEPL(I)+R*SBAR(I,JP)
120       CONTINUE
200   CONTINUE
      IF (LHOOK) CALL DR_HOOK('DD_1DV',1,ZHOOK_HANDLE)
      ENDSUBROUTINE DD_1DV
!
!-----------------------------------------------------------------------
!
