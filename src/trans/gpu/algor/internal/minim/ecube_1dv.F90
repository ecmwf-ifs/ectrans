      SUBROUTINE ECUBE_1DV(T,F,FP,TA,FA,FPA,TLOWER,TUPPER)
!AUTOPROMOTE
...
!
      ...
      IF (LHOOK) CALL DR_HOOK('ECUBE_1DV',0,ZHOOK_HANDLE)
...
!
      DISCRI=SQRT(DISCRI)
  120 IF (T-TA.LT.0.0_JPRB) DISCRI=-DISCRI
      SIGN=(T-TA)/ABS(T-TA)
  ...
  900 T=MAX(T,TLOWER)
      T=MIN(T,TUPPER)
      IF (LHOOK) CALL DR_HOOK('ECUBE_1DV',1,ZHOOK_HANDLE)
      ENDSUBROUTINE ECUBE_1DV
!
!-----------------------------------------------------------------------
!
