...
void execute_fft(typename Type::real *data_real, typename Type::cmplx *data_complex,
  ...
  HIC_CHECK(hipGraphLaunch(graphCache.at(kfield), 0));
      /* for (int i = 0; i < nfft; ++i) { */
        ...
        /* cufftHandle plan; */
        /* fftSafeCall(cufftCreate(&plan)); */
        /* int dist = 1; */
        /* int embed[] = {1}; */
        /* fftSafeCall(cufftPlanMany(&plan, 1, &nloen, embed, kfield, dist, embed, */
                                  /* kfield, dist, Direction, kfield)); */
        /* int offset = offsets[i]; */
        /* real *data_real_l = &data_real[kfield * offset]; */
        /* cmplx *data_complex_l = &data_complex[kfield * offset / 2]; */
        /* if (Direction == CUFFT_R2C) */
          /* fftSafeCall(cufftExecR2C(plan, data_real_l, data_complex_l)) */
        /* else */
          /* fftSafeCall(cufftExecC2R(plan, data_complex_l, data_real_l)); */
        /* fftSafeCall(cufftDestroy(plan)); */
      /* } */
