...
!ON CPU
REAL(KIND=JPRBT),ALLOCATABLE :: ZGTFL(:,:)
...
ALLOCATE(ZAVE(IF_FS,IEND))
ALLOCATE(ZMINGL(IF_FS,IEND))
ALLOCATE(ZMAXGL(IF_FS,IEND))
...
IF( IF_FS > 0 )THEN
    ...
    DO JF=1,IF_FS
      ZMINGL(JF,IBEG:IEND)=ZGTFL(JF,D_NSTAGTF(1)+1)
      ZMAXGL(JF,IBEG:IEND)=ZGTFL(JF,D_NSTAGTF(1)+1)
    ENDDO
...
! FIRST DO SUMS IN EACH FULL LATITUDE
  ...
  DO JGL=IBEG,IEND
    ...
    DO JF=1,IF_FS
      ...
      DO JL=1,G_NLOEN(IGL)
        ZAVE(JF,JGL)=ZAVE(JF,JGL)+ZGTFL(JF,D_NSTAGTF(JGL)+JL)
        ZMINGL(JF,JGL)=MIN(ZMINGL(JF,JGL),ZGTFL(JF,D_NSTAGTF(JGL)+JL))
        ZMAXGL(JF,JGL)=MAX(ZMAXGL(JF,JGL),ZGTFL(JF,D_NSTAGTF(JGL)+JL))
      ENDDO
    ENDDO
  ENDDO
