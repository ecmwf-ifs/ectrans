...
IF(IUBOUND(1) /= IPROMA) THEN
  WRITE(NOUT,*)'DIST_GRID:FIRST DIMENSION OF PGP /= IPROMA',IUBOUND(1),IPROMA
  CALL ABORT_TRANS('DIST_GRID:FIRST DIMENSION OF PGP /= IPROMA')
ENDIF
IF(IUBOUND(2) /= KFDISTG) THEN
  WRITE(NOUT,*)'DIST_GRID:SECOND DIMENSION OF PGP /= KFDISTG',IUBOUND(2),KFDISTG
  CALL ABORT_TRANS('DIST_GRID:SECOND DIMENSION OF PGP /= KFDISTG')
ENDIF
IF(IUBOUND(3) /= IGPBLKS) THEN
  WRITE(NOUT,*)'DIST_GRID:THIRD DIMENSION OF PGP /= IGPBLKS',IUBOUND(3),IGPBLKS
  CALL ABORT_TRANS('DIST_GRID:THIRD DIMENSION OF PGP /= IGPBLKS')
ENDIF
...
IF(IFSEND > 0) THEN
  ...
  IF(UBOUND(PGPG,1) /= D%NGPTOTG) THEN
    WRITE(NOUT,*)'DIST_GRID:FIRST DIMENSION OF PGPG /= D%NGPTOTG',UBOUND(PGPG,1),D%NGPTOTG
    CALL ABORT_TRANS('DIST_GRID:FIRST DIMENSION OF PGPG /= D%NGPTOTG')
  ENDIF
  IF(UBOUND(PGPG,2) /= IFSEND) THEN
    WRITE(NOUT,*)'DIST_GRID:SECOND DIMENSION OF PGPG /= IFSEND',UBOUND(PGPG,2),IFSEND
    CALL ABORT_TRANS('DIST_GRID:SECOND DIMENSION OF PGPG TOO SMALL')
  ENDIF
ENDIF
