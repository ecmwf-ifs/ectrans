...
IF(IFSEND > 0 ) THEN
  IF(.NOT.PRESENT(PSPEC).AND.ISPEC2>0) THEN
...
CALL GATH_SPEC_CONTROL(PSPECG=PSPECG,KFGATHG=KFGATHG,KTO=KTO,KVSET=IVSET,PSPEC=PSPEC,LDIM1_IS_FLD=LLDIM1_IS_FLD,&
 & KSMAX=ISMAX,KSPEC2=ISPEC2,KSPEC2_G=ISPEC2_G,KPOSSP=IPOSSP,KDIM0G=IDIM0G,LDZA0IP=LDZA0IP)
DEALLOCATE(IDIM0G)
