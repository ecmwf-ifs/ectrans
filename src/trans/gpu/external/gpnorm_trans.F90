...
! Consistency checks
IF(PRESENT(LPGP_ON_GPU)) LLGPU_ON_GPU = LPGP_ON_GPU

IF (.NOT. LLGPU_ON_GPU) THEN
...
CALL TRGTOL(ALLOCATOR,HTRGTOL,PREEL_REAL,IF_FS,IF_GP,0,IF_SCALARS_G,&
  & KVSETSC=IVSET,PGP=PGP,LPGP_ON_GPU=LPGP_ON_GPU)
LGPNORM=.FALSE.
...
#ifdef OMPGPU
      !$OMP TARGET DATA USE_DEVICE_PTR(ZRCV)
#endif
#ifdef ACCGPU
...
#ifdef USE_GPU_AWARE_MPI
#ifdef OMPGPU
    !$OMP TARGET DATA USE_DEVICE_PTR(ZSND)
#endif
#ifdef ACCGPU
