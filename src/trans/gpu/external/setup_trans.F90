...
IF( .NOT.D%LGRIDONLY ) THEN
  ...
  ALLOCATE(FG%ZAA(D%OFFSETS_LEINV_LEGPOL(D%NUMP+1)))
  ALLOCATE(FG%ZAS(D%OFFSETS_LEINV_LEGPOL(D%NUMP+1)))
  ...
  DO JMLOC=1,D%NUMP
    ...
    IF (KM /= 0) THEN
      CALL C_F_POINTER(C_LOC(FG%ZAA(1+D%OFFSETS_LEINV_LEGPOL(JMLOC))), LOCAL_ARR, &
          & (/D%LEGENDRE_MATRIX_STRIDES(JMLOC),ILA/))
      LOCAL_ARR(1:KDGLU,1:ILA) = S%FA(JMLOC)%RPNMA(1:KDGLU,1:ILA)

      CALL C_F_POINTER(C_LOC(FG%ZAS(1+D%OFFSETS_LEINV_LEGPOL(JMLOC))), LOCAL_ARR, &
...
#ifdef _CRAYFTN
  !$ACC ENTER DATA COPYIN(D, D%NUMP, D%MYMS, D%NPNTGTB0, D%NPNTGTB1, D%NSTAGT0B, D%NSTAGT1B) &
  !$ACC&           COPYIN(D%NSTAGTF, D%NPROCM, D%NPROCL, D%NPTRLS, D%MSTABF, D%NASM0) &
  !$ACC&           COPYIN(D%OFFSETS_GEMM1, D%OFFSETS_LEINV_IN, D%OFFSETS_LEDIR_OUT, D%NDGL_FS) &
  !$ACC&           ASYNC(1)
#else
  !$ACC ENTER DATA COPYIN(D, D%MYMS, D%NPNTGTB0, D%NPNTGTB1, D%NSTAGT0B, D%NSTAGT1B, D%NSTAGTF) &
  !$ACC&           COPYIN(D%NPROCM, D%NPROCL, D%NPTRLS, D%MSTABF, D%NASM0, D%OFFSETS_LEINV_IN) &
  !$ACC&           COPYIN(D%OFFSETS_LEDIR_OUT) ASYNC(1)
#endif
  ...
  !$OMP TARGET ENTER DATA MAP(TO: D, D%NUMP, D%MYMS, D%NPNTGTB0, D%NPNTGTB1, D%NSTAGT0B) &
  !$OMP&                  MAP(TO: D%NSTAGT1B, D%NSTAGTF, D%NPROCM, D%NPROCL, D%NPTRLS, D%MSTABF) &
  !$OMP&                  MAP(TO: D%NASM0, D%OFFSETS_LEINV_IN, D%OFFSETS_LEDIR_OUT, D%NDGL_FS)
