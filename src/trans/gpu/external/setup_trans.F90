...
USE TPM_FLT,                     ONLY: FLT_RESOL, S, FLATTEN_BUTTERFLY_STRUCT
USE TPM_CTL,                     ONLY: C
...
USE OPENACC,                     ONLY: ACC_DEVICE_KIND, ACC_GET_DEVICE_TYPE, ACC_GET_NUM_DEVICES, &
  &                                    ACC_SET_DEVICE_NUM
#endif
USE BACKENDS_MOD,                ONLY: JP_BACKEND_GPU_SP, JP_BACKEND_GPU_DP, Y_BACKENDS
USE RESOLS_MOD,                  ONLY: Y_RESOLS
USE DEALLOC_RESOL_MOD,           ONLY: DEALLOC_RESOL
USE BACKEND_TEARDOWN_MOD,        ONLY: BACKEND_TEARDOWN
...
! Initialise global-transform, GPU backend-specific structures
IF (.NOT. ALLOCATED(FIELDS_GPU_RESOL)) ALLOCATE(FIELDS_GPU_RESOL(NMAX_RESOL))
IF (.NOT. ALLOCATED(FLT_RESOL)) ALLOCATE(FLT_RESOL(NMAX_RESOL))

! Determine backend
I_BACKEND = MERGE(JP_BACKEND_GPU_DP, JP_BACKEND_GPU_SP, JPRB == JPRD)

! Set the deallocator for this resolution
Y_RESOLS(IDEF_RESOL)%BACKEND = I_BACKEND
Y_RESOLS(IDEF_RESOL)%DESTROY => DEALLOC_RESOL

! If this is the first time this backend is used, set its backend teardown function and enable it
IF (.NOT. Y_BACKENDS(I_BACKEND)%ENABLED) THEN
  Y_BACKENDS(I_BACKEND)%ENABLED = .TRUE.
  Y_BACKENDS(I_BACKEND)%TEARDOWN => BACKEND_TEARDOWN
ENDIF
...
IF( .NOT.D%LGRIDONLY ) THEN
  ...
  IF (S%LUSEFLT) THEN
    ...
    CALL FLATTEN_BUTTERFLY_STRUCT(S%FA, "ANTI", FG%YD_BUTTERFLY_FLAT_A)
    CALL FLATTEN_BUTTERFLY_STRUCT(S%FA, "SYMM", FG%YD_BUTTERFLY_FLAT_S)
  ENDIF
  ...
  IF (S%LUSEFLT) THEN
    !$ACC ENTER DATA COPYIN(FG%YD_BUTTERFLY_FLAT_A) ASYNC(1)
    !$ACC ENTER DATA COPYIN(FG%YD_BUTTERFLY_FLAT_A%N_ORDER) &
    !$ACC&           COPYIN(FG%YD_BUTTERFLY_FLAT_A%N_LEVELS) &
    !$ACC&           COPYIN(FG%YD_BUTTERFLY_FLAT_A%IBETALEN_MAX) ASYNC(1)
    !$ACC ENTER DATA COPYIN(FG%YD_BUTTERFLY_FLAT_S) ASYNC(1)
    !$ACC ENTER DATA COPYIN(FG%YD_BUTTERFLY_FLAT_S%N_ORDER) &
    !$ACC&           COPYIN(FG%YD_BUTTERFLY_FLAT_S%N_LEVELS)&
    !$ACC&           COPYIN(FG%YD_BUTTERFLY_FLAT_S%IBETALEN_MAX) ASYNC(1)
  ENDIF
#ifdef _CRAYFTN
  ...
  IF (S%LUSEFLT) THEN
    !$OMP TARGET ENTER DATA MAP(TO:FG%YD_BUTTERFLY_FLAT_A)
    !$OMP TARGET ENTER DATA MAP(TO:FG%YD_BUTTERFLY_FLAT_A%N_ORDER) &
    !$OMP&                  MAP(TO:FG%YD_BUTTERFLY_FLAT_A%N_LEVELS) &
    !$OMP&                  MAP(TO:FG%YD_BUTTERFLY_FLAT_A%IBETALEN_MAX)
    !$OMP TARGET ENTER DATA MAP(TO:FG%YD_BUTTERFLY_FLAT_S)
    !$OMP TARGET ENTER DATA MAP(TO:FG%YD_BUTTERFLY_FLAT_S%N_ORDER) &
    !$OMP&                  MAP(TO:FG%YD_BUTTERFLY_FLAT_S%N_LEVELS) &
    !$OMP&                  MAP(TO:FG%YD_BUTTERFLY_FLAT_S%IBETALEN_MAX)
  ENDIF
