...
USE OPENACC,                     ONLY: ACC_DEVICE_KIND, ACC_GET_DEVICE_TYPE, ACC_GET_NUM_DEVICES, &
  &                                    ACC_SET_DEVICE_NUM
#endif
USE BACKENDS_MOD,                ONLY: Y_RESOL_DEALLOCATORS, JP_BACKEND_GPU_SP, JP_BACKEND_GPU_DP, &
  &                                    Y_BACKEND_TEARDOWNS
...
Y_RESOL_DEALLOCATORS(IDEF_RESOL)%BACKEND = I_BACKEND
Y_RESOL_DEALLOCATORS(IDEF_RESOL)%F => DEALLOC_RESOL

! If this is the first time this backend is used, set its backend teardown function and enable it
IF (.NOT. Y_BACKEND_TEARDOWNS(I_BACKEND)%ENABLED) THEN
  Y_BACKEND_TEARDOWNS(I_BACKEND)%ENABLED = .TRUE.
  Y_BACKEND_TEARDOWNS(I_BACKEND)%F => BACKEND_TEARDOWN
ENDIF
