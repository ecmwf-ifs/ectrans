...
SUBROUTINE SETUP_TRANS(KSMAX,KDGL,KDLON,KLOEN,LDSPLIT,PSTRET,&
 ...
 &                     KPREPLAN_DIR_FFT_NFIELDS,KPREPLAN_INV_FFT_NFIELDS)
...
USE TPM_DISTR,                   ONLY: D, DISTR_RESOL, NPROC, MYPROC, MYSETW
...
#endif
USE TPM_HICFFT,                  ONLY: PREPLAN_DIR_FFT, PREPLAN_INV_FFT
...
INTEGER(KIND=JPIM) ,OPTIONAL,INTENT(IN) :: KPREPLAN_DIR_FFT_NFIELDS(:)
INTEGER(KIND=JPIM) ,OPTIONAL,INTENT(IN) :: KPREPLAN_INV_FFT_NFIELDS(:)
...
IF( .NOT.D%LGRIDONLY ) THEN
  ...
  ! Pre-plan FFTs
  IF (PRESENT(KPREPLAN_DIR_FFT_NFIELDS)) THEN
    DO KFIELD = 1, SIZE(KPREPLAN_DIR_FFT_NFIELDS)
      CALL PREPLAN_DIR_FFT(KPREPLAN_DIR_FFT_NFIELDS(KFIELD), NCUR_RESOL, &
        &                  G%NLOEN(D%NPTRLS(MYSETW):D%NPTRLS(MYSETW)+D%NDGL_FS-1), &
        &                  D%NSTAGTF(1:D%NDGL_FS+1), 1.0_JPRB)
    ENDDO
  ENDIF
  IF (PRESENT(KPREPLAN_INV_FFT_NFIELDS)) THEN
    DO KFIELD = 1, SIZE(KPREPLAN_INV_FFT_NFIELDS)
      CALL PREPLAN_INV_FFT(KPREPLAN_INV_FFT_NFIELDS(KFIELD), NCUR_RESOL, &
        &                  G%NLOEN(D%NPTRLS(MYSETW):D%NPTRLS(MYSETW)+D%NDGL_FS-1), &
        &                  D%NSTAGTF(1:D%NDGL_FS+1), 1.0_JPRB)
    ENDDO
  ENDIF
