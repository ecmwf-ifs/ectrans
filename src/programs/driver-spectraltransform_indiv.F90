...
DO JB=D%NPTRFRSTLAT(MY_REGION_NS),D%NPTRLSTLAT(MY_REGION_NS)
  WRITE(300+MYPROC,*) "MY_REGION", JB-D%NPTRFRSTLAT(MY_REGION_NS)+D%NFRSTLAT(MY_REGION_NS), D%NSTA(JB,MY_REGION_EW), D%NONL(JB,MY_REGION_EW)
ENDDO
IF (MYPROC == 1) THEN
  DO JB=1,NDGL
    WRITE(300+MYPROC,*) "LATITUDE", JB, ASIN(F%RMU(JB))/3.14159265358979323846264338327950288*180, NLOEN(JB)
  ENDDO
ENDIF
...
ALLOCATE(ZWINDS(NPROMA,NFLEVG,6,NGPBLKS))
...
DO JSTEP=1,1
  ...
  ZWINDS = 0
  ZGMV = 0
  CALL INV_TRANS(PSPSC2=ZSP(1:1,:),PSPSC3A=ZT,&
     & PGP2=ZGMVS(:,1:3,:),PGP3A=ZGMV(:,:,5:7,:), &
     & LDSCDERS=.TRUE.,LDVORGP=.FALSE.,LDDIVGP=.FALSE.,LDUVDER=.FALSE.,&
     & KRESOL=1,KPROMA=NPROMA,KVSETSC2=IVSETSC(1:1),&
     & KVSETSC3A=IVSET)
  CALL DUMP_GRIDPOINT_FIELD_3D(JJSTEP, MYPROC, ZGMVS(:,:,:), 'S', NOUTDUMP)
  CALL DUMP_GRIDPOINT_FIELD_4D(JJSTEP, MYPROC, ZGMV(:,:,:,:),  'M', NOUTDUMP)

  ! all; split, all options on
  JJSTEP = 2
  ZGMVS = 0
  ZWINDS = 0
  ZGMV = 0
  CALL INV_TRANS(PSPVOR=ZVOR,PSPDIV=ZDIV,PSPSC2=ZSP(1:1,:),PSPSC3A=ZT,&
     & PGPUV=ZWINDS(:,:,1:4,:),PGP2=ZGMVS(:,1:3,:),PGP3A=ZGMV(:,:,5:7,:),&
     & LDSCDERS=.TRUE.,LDVORGP=.TRUE.,LDDIVGP=.TRUE.,LDUVDER=.FALSE.,&
     & KRESOL=1,KPROMA=NPROMA,KVSETUV=IVSET,KVSETSC2=IVSETSC(1:1),&
     & KVSETSC3A=IVSET)
  CALL DUMP_GRIDPOINT_FIELD_3D(JJSTEP, MYPROC, ZGMVS(:,:,:), 'S', NOUTDUMP)
  CALL DUMP_GRIDPOINT_FIELD_4D(JJSTEP, MYPROC, ZWINDS(:,:,:,:),  'W', NOUTDUMP)
  CALL DUMP_GRIDPOINT_FIELD_4D(JJSTEP, MYPROC, ZGMV(:,:,:,:),  'M', NOUTDUMP)
  ...
  ZGMV = 0
  CALL INV_TRANS(PSPVOR=ZVOR,PSPDIV=ZDIV,PSPSC2=ZSP(1:1,:),PSPSC3A=ZT,&
     & PGPUV=ZWINDS(:,:,2:4,:),PGP2=ZGMVS(:,1:3,:),PGP3A=ZGMV(:,:,5:7,:),&
  ...
  ! scalar only; with derivatives
  JJSTEP = 5
  ZGMVS = 0
  ZWINDS = 0
  ZGMV = 0
  CALL INV_TRANS(PSPSCALAR=ZSP(1:1,:),&
     & PGP=ZGMVS(:,1:3,:), &
     & LDSCDERS=.TRUE.,&
 ...
 ! scalar only
  ...
  ZWINDS = 0
  ZGMV = 0
  CALL INV_TRANS(PSPSC2=ZSP(1:1,:),&
     & PGP2=ZGMVS(:,1:3,:),&
     & LDSCDERS=.TRUE.,&
     & KRESOL=1,KPROMA=NPROMA,KVSETSC2=IVSETSC(1:1))
  CALL DUMP_GRIDPOINT_FIELD_3D(JJSTEP, MYPROC, ZGMVS(:,:,:), 'S', NOUTDUMP)

  ! scalar split only; with derivatives
  JJSTEP = 8
  ZGMVS = 0
  ZWINDS = 0
  ZGMV = 0
  CALL INV_TRANS(PSPSC2=ZT(:,:,1),&
     & PGP2=ZGMV(:,:,5,:), &
     & KRESOL=1,KPROMA=NPROMA,KVSETSC2=IVSET)
  CALL DUMP_GRIDPOINT_FIELD_4D(JJSTEP, MYPROC, ZGMV(:,:,:,:),  'M', NOUTDUMP)
  ...
  CALL DIR_TRANS(PSPSCALAR=ZSP(1:1,:), &
     & PGP=ZGMVS(:,1:1,:), &
     & KRESOL=1,KPROMA=NPROMA,KVSETSC=IVSETSC(1:1))
  CALL DUMP_GRIDPOINT_FIELD_2D(JJSTEP, MYPROC, ZSP(:,:), 'P', NOUTDUMP)
  ...
  ZVOR = 0
  ZDIV = 0
  CALL DIR_TRANS(PSPSC2=ZSP(1:1,:),PSPSC3A=ZT,&
      & PGP2=ZGMVS(:,1:1,:),PGP3A=ZGMV(:,:,5:5,:), &
      & KRESOL=1,KPROMA=NPROMA,KVSETSC2=IVSETSC(1:1),&
      & KVSETSC3A=IVSET)
  CALL DUMP_GRIDPOINT_FIELD_2D(JJSTEP, MYPROC, ZSP(:,:), 'P', NOUTDUMP)
  CALL DUMP_GRIDPOINT_FIELD_3D(JJSTEP, MYPROC, ZT(:,:,:),  'T', NOUTDUMP)
  ...
  ZVOR = 0
  ZDIV = 0
  CALL DIR_TRANS(PSPVOR=ZVOR(1:NFLEVL,:),PSPDIV=ZDIV(1:NFLEVL,:),&
      & PGPUV=ZWINDS(:,1:NFLEVG,3:4,:), &
      & KRESOL=1,KPROMA=NPROMA,KVSETUV=IVSET(1:NFLEVG))
  CALL DUMP_GRIDPOINT_FIELD_2D(JJSTEP, MYPROC, ZVOR(:,:), 'V', NOUTDUMP)
  CALL DUMP_GRIDPOINT_FIELD_2D(JJSTEP, MYPROC, ZDIV(:,:),  'D', NOUTDUMP)
  ...
  ZVOR = 0
  ZDIV = 0
  CALL DIR_TRANS(PSPSCALAR=ZSP(1:1,:),PGP=ZGMVS(:,1:1,:),KVSETSC=IVSETSC(1:1),&
      & KPROMA=NPROMA)
  CALL DUMP_GRIDPOINT_FIELD_2D(JJSTEP, MYPROC, ZSP(:,:), 'V', NOUTDUMP)
  ...
  ZVOR = 0
  ZDIV = 0
  CALL DIR_TRANS(PSPVOR=ZVOR,PSPDIV=ZDIV,PSPSC2=ZSP(1:1,:),PSPSC3A=ZT,&
     & PGPUV=ZWINDS(:,:,3:4,:),PGP2=ZGMVS(:,1:1,:),PGP3A=ZGMV(:,:,5:5,:), &
     & KRESOL=1,KPROMA=NPROMA,KVSETUV=IVSET,KVSETSC2=IVSETSC(1:1),&
     & KVSETSC3A=IVSET)
  CALL DUMP_GRIDPOINT_FIELD_2D(JJSTEP, MYPROC, ZVOR(:,:), 'V', NOUTDUMP)
  CALL DUMP_GRIDPOINT_FIELD_2D(JJSTEP, MYPROC, ZDIV(:,:),  'D', NOUTDUMP)
...
REAL(KIND=JPRB), ALLOCATABLE :: FLD_R(:,:)

WRITE(FILENAME(1:1),'(A1)') FLDCHAR
WRITE(FILENAME(3:5),'(I3.3)') JSTEP
WRITE(FILENAME(7:10),'(I4.4)') MYPROC

ALLOCATE(FLD_R(SIZE(FLD,1), SIZE(FLD,2)))
OPEN(NOUTDUMP, FILE='./ref/' // FILENAME, FORM="UNFORMATTED")
READ(NOUTDUMP) FLD_R
CLOSE(NOUTDUMP)

WRITE(400+MYPROC, *), FILENAME, MAXVAL(ABS(FLD_R-FLD))
...
ALLOCATE(FLD_R(SIZE(FLD,1), SIZE(FLD,2), SIZE(FLD,3)))
OPEN(NOUTDUMP, FILE='./ref/' // FILENAME, FORM="UNFORMATTED")
READ(NOUTDUMP) FLD_R
CLOSE(NOUTDUMP)
...
ALLOCATE(FLD_R(SIZE(FLD,1), SIZE(FLD,2),SIZE(FLD,3),SIZE(FLD,4)))
OPEN(NOUTDUMP, FILE='./ref/' // FILENAME, FORM="UNFORMATTED")
READ(NOUTDUMP) FLD_R
CLOSE(NOUTDUMP)
