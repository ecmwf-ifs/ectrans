...
! ALLOCATE(D%NSTAGT0B(SIZE(D%NSTAGT0B)))
! ALLOCATE(D%NSTAGT1B(SIZE(D%NSTAGT1B)))
! ALLOCATE(D%NPNTGTB0(0:SIZE(D%NPNTGTB0,1)-1,SIZE(D%NPNTGTB0,2)))
! ALLOCATE(D%NPNTGTB1(SIZE(D%NPNTGTB1,1),SIZE(D%NPNTGTB1,2)))
! ALLOCATE(D%MYMS(SIZE(D%MYMS)))
! ALLOCATE(D%NPROCL(SIZE(D%NPROCL)))
! ALLOCATE(D%NASM0(0:SIZE(D%NASM0)-1))
! ALLOCATE(D%NSTAGTF(SIZE(D%NSTAGTF)))
! ALLOCATE(D%MSTABF(SIZE(D%MSTABF)))
! ALLOCATE(D%NPROCM(0:SIZE(D%NPROCM)-1))
! ALLOCATE(D%NPTRLS(SIZE(D%NPTRLS)))

! ALLOCATE(G%NDGLU(0:SIZE(G%NDGLU)-1))
! ALLOCATE(G%NMEN(SIZE(G%NMEN)))
! ALLOCATE(G%NLOEN(SIZE(G%NLOEN)))
...
! DO I=1,SIZE(G%NMEN)
   ! G%NMEN(I)=G%NMEN(I)
   ! IF (G%NMEN(I) .GT. G%NMEN_MAX) G%NMEN_MAX=G%NMEN(I)
! END DO

! G%NLOEN_MAX=0
! DO I=1,SIZE(G%NLOEN)
   ! G%NLOEN(I)=G%NLOEN(I)
   ! IF (G%NLOEN(I) .GT. G%NLOEN_MAX) G%NLOEN_MAX=G%NLOEN(I)
! END DO

! DO I=1,SIZE(D%NSTAGT0B)
   ! D%NSTAGT0B(I)=D%NSTAGT0B(I)
! END DO

! DO I=1,SIZE(D%NSTAGT1B)
   ! D%NSTAGT1B(I)=D%NSTAGT1B(I)
! END DO

! DO I=1,SIZE(D%NPROCL)
   ! D%NPROCL(I)=D%NPROCL(I)
! END DO
...
! ALLOCATE(DALD%NESM0(LBOUND(DALD%NESM0,1):UBOUND(DALD%NESM0,1)))
! DO I=LBOUND(DALD%NESM0,1),UBOUND(DALD%NESM0,1)
  ! DALD%NESM0(I)=DALD%NESM0(I)
! ENDDO

! ALLOCATE(DALD%NCPL2M(LBOUND(DALD%NCPL2M,1):UBOUND(DALD%NCPL2M,1)))
! DO I=LBOUND(DALD%NCPL2M,1),UBOUND(DALD%NCPL2M,1)
  ! DALD%NCPL2M(I)=DALD%NCPL2M(I)
! ENDDO

! ALLOCATE(DALD%NPME(LBOUND(DALD%NPME,1):UBOUND(DALD%NPME,1)))
! DO I=LBOUND(DALD%NPME,1),UBOUND(DALD%NPME,1)
  ! DALD%NPME(I)=DALD%NPME(I)
! ENDDO

! ALLOCATE(FALD%RLEPINM(SIZE(FALD%RLEPINM)))
