...
USE TPMALD_FIELDS   ,ONLY : ALDFIELDS_RESOL, FALD
...
RALD%NLOENS_LON=(/ R%NDLON /)
RALD%NOFFSETS_LON=(/ 0 , R%NDLON+2 /)
RALD%NLOENS_LAT=(/ R%NDGL /)
RALD%NOFFSETS_LAT=(/ 0 , R%NDGL+2 /)
...
#endif
#ifdef OMPGPU
!$OMP TARGET ENTER DATA MAP(TO:F,D,D%NUMP,D%MYMS,R,R%NDGNH,R%NSMAX,G,G%NDGLU)
...
#ifdef ACCGPU
!$ACC ENTER DATA COPYIN(R,D,G,DALD,FALD,RALD)
!$ACC ENTER DATA COPYIN(R%NSMAX,R%NTMAX,R%NDGL,R%NDGNH,D%NSTAGT0B,D%NSTAGT1B,&
...
#endif
#ifdef OMPGPU
!$OMP TARGET ENTER DATA MAP(TO:R%NSMAX,R%NTMAX,R%NDGL,R%NDGNH,D%NSTAGT0B,D%NSTAGT1B)
!$OMP TARGET ENTER DATA MAP(TO:D%NPNTGTB1,D%NPROCL,D%NUMP,D%NDGL_FS,D%MYMS,D%NASM0,D%NSTAGTF,D%MSTABF)
!$OMP TARGET ENTER DATA MAP(TO:D%NPNTGTB0,D%NPROCM,D%NPTRLS,G%NDGLU,G%NMEN,G%NLOEN)
!$OMP TARGET ENTER DATA MAP(TO:DALD%NESM0,DALD%NCPL2M, DALD%NPME, FALD%RLEPINM)
!$OMP TARGET ENTER DATA MAP(TO:RALD%NLOENS_LON,RALD%NOFFSETS_LON,RALD%NLOENS_LAT,RALD%NOFFSETS_LAT
#endif
