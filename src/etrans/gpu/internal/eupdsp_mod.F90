MODULE EUPDSP_MOD
CONTAINS
SUBROUTINE EUPDSP(KF_UV,KF_SCALARS,PFFT,PVODI, &
...
IF (KF_SCALARS > 0) THEN
  IF(PRESENT(PSPSCALAR)) THEN
    ...
    CALL EUPDSPB(KF_SCALARS,PFFT(:,:,IST:IEND),PSPSCALAR,KFLDPTRSC)
  ELSE
    IF(PRESENT(PSPSC2) .AND. NF_SC2 > 0) THEN
      IDIM1 = NF_SC2
      IEND  = IST+2*IDIM1-1
      CALL EUPDSPB(IDIM1,PFFT(:,:,IST:IEND),PSPSC2)
      IST=IST+2*IDIM1
    ENDIF
    IF(PRESENT(PSPSC3A) .AND. NF_SC3A > 0) THEN
      ...
      DO J3=1,IDIM3
        IEND = IST+2*IDIM1-1
        CALL EUPDSPB(IDIM1,PFFT(:,:,IST:IEND),PSPSC3A(:,:,J3))
        IST=IST+2*IDIM1
      ENDDO
    ENDIF
    IF(PRESENT(PSPSC3B) .AND. NF_SC3B > 0) THEN
      ...
      DO J3=1,IDIM3
        IEND = IST+2*IDIM1-1
        CALL EUPDSPB(IDIM1,PFFT(:,:,IST:IEND),PSPSC3B(:,:,J3))
        IST=IST+2*IDIM1
      ENDDO
    ENDIF
  ENDIF
