...
#else
USE TPM_FFTW_SP, ONLY: EXEC_FFTW
#endif
...
REAL(KIND=JPRB), DIMENSION(KSIZES), INTENT(IN) :: PSPEC
INTEGER(KIND=INT64), INTENT(IN) :: KSIZEG
REAL(KIND=JPRB), DIMENSION(KSIZEG), INTENT(OUT) :: PGPT

INTEGER(KIND=INT64) :: NFTM, NDGLSUR
REAL(KIND=JPRB), DIMENSION(:,:), ALLOCATABLE :: SP2
...
REAL(KIND=JPRB),DIMENSION(:),INTENT(IN) :: IN
INTEGER(KIND=INT64),INTENT(IN)          :: X, Y, N
REAL(KIND=JPRB),DIMENSION(N*2*(X+1))    :: OU

INTEGER(KIND=INT64),DIMENSION(2*(X+1),(N/2)) :: MINQ
INTEGER(KIND=INT64),DIMENSION((N/2),2*(X+1)) :: TMINQ
REAL(KIND=JPRB),DIMENSION(2*(X+1),(N/2))     :: OMINQ, EMINQ
REAL(KIND=JPRB),DIMENSION((N/2),2*(X+1))     :: TOMINQ, TEMINQ
REAL(KIND=JPRB),DIMENSION(N*(X+1))           :: OINI, EINI
REAL(KIND=JPRB), PARAMETER                   :: ZZERO=0.0
...
REAL(KIND=JPRB),DIMENSION(:),INTENT(IN)      :: TO,TE
REAL(KIND=JPRB),DIMENSION(SIZE(TO)+SIZE(TE)) :: T
...
REAL(KIND=JPRB),DIMENSION(:),INTENT(IN)          :: T
REAL(KIND=JPRB),DIMENSION(SIZE(T)/2),INTENT(OUT) :: TO,TE
