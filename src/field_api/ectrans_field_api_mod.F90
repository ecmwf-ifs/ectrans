...
FUNCTION LG(YLFL,LGV,LDACC,LDRDONLY) RESULT(IOFF)
  ...
  LGV_PROVIDED = PRESENT(LGV)
  IF (PRESENT(LDACC)) THEN
    LLACC = LDACC
  ELSE
    LLACC = .FALSE.
  ENDIF
  IF (PRESENT(LDRDONLY)) THEN
    LLRDONLY = LDRDONLY
  ELSE
    LLRDONLY = .FALSE.
  ENDIF
...
IF (LGV_PROVIDED .AND. SIZE(LGV) /= IOFF)  CALL ABOR1("LG FAILURE: LSV has incorrect size")
...
IF (LSV_PROVIDED .AND. SIZE(LSV) /= IOFF)  CALL ABOR1("LS FAILURE: LSV has incorrect size")
