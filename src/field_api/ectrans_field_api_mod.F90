...
USE ECTRANS_FIELD_VIEW_MOD, ONLY: MEMORY_TYPE_HOST, MEMORY_TYPE_DEVICE
USE ECTRANS_FIELD_VIEW_MOD, ONLY: INDEXING_TYPE_DFFD, INDEXING_TYPE_DFD, INDEXING_TYPE_DF, INDEXING_TYPE_FD
USE ECTRANS_FIELD_VIEW_MOD, ONLY: FIELD_TYPE_TEST, FIELD_TYPE_SP, FIELD_TYPE_GP
...
SUBROUTINE SET_INDEXING_TYPE(YLFV, FIELD_TYPE)
  TYPE(FIELD_VIEW), INTENT(INOUT) :: YLFV
  INTEGER, INTENT(IN) :: FIELD_TYPE
  IF (FIELD_TYPE_TEST(FIELD_TYPE, FIELD_TYPE_SP)) THEN
    IF (YLFV%RANK == 2) THEN
      YLFV%INDEXING_TYPE = INDEXING_TYPE_FD
    ELSE
      CALL ABOR1("Error: Spectral fields must be rank 2")
    ENDIF
  ELSE IF (FIELD_TYPE_TEST(FIELD_TYPE, FIELD_TYPE_GP)) THEN
    IF (YLFV%RANK == 3) THEN
      YLFV%INDEXING_TYPE = INDEXING_TYPE_DFD
    ELSE IF (YLFV%RANK == 4) THEN
      YLFV%INDEXING_TYPE = INDEXING_TYPE_DFFD
    ELSE
      CALL ABOR1("Error: Gridpoint fields must be rank 3 or 4")
    ENDIF
  ELSE
    CALL ABOR1("Error: FIELD_TYPE_GP OR FIELD_TYPE_SP could not be extracted from FIELD_TYPE")
  ENDIF
END SUBROUTINE

SUBROUTINE SET_MEMORY_TYPE(YLFV, LDACC)
  TYPE(FIELD_VIEW), INTENT(INOUT) :: YLFV
  LOGICAL, INTENT(IN) :: LDACC
  IF (LDACC) THEN
    YLFV%MEMORY_TYPE = MEMORY_TYPE_DEVICE
  ELSE
    YLFV%MEMORY_TYPE = MEMORY_TYPE_HOST
  ENDIF
END SUBROUTINE
...
SUBROUTINE MAKE_FIELD_VIEW(YLFV, YLF, FIELD_TYPE, LDACC, LDRDONLY)
  ...
  CALL SET_INDEXING_TYPE(YLFV, FIELD_TYPE)
  CALL SET_IVSET(YLFV, YLF%IVSET)
  CALL SET_MEMORY_TYPE(YLFV, LDACC)
END SUBROUTINE
