...
USE ECTRANS_FIELD_VIEW_MOD, ONLY: FIELD_VIEW, FIELD_VIEW_GET_DATA_PTR, FIELD_VIEW_GET_NAME_PTR, GET_IVSET
...
SUBROUTINE LG2RB(LG2RBV,YLF,IVSET)
  ...
  LG2RBV (1)%P => ZZ2(:,:)
  CALL FIELD_VIEW_GET_NAME_PTR(YLF,LG2RBV(1)%NAME)
...
SUBROUTINE LG3RB(LG3RBV,YLF,IVSET)
  ...
  DO JLEV = LBOUND (ZZ3, 2), UBOUND (ZZ3, 2)
    ...
    CALL FIELD_VIEW_GET_NAME_PTR(YLF,LG3RBV(JCOUNT)%NAME)
    IF (ASSOCIATED(IVSET)) THEN
...
SUBROUTINE LG4RB(LG4RBV, YLF, IVSET)
  ...
  DO JFLD = LBOUND (ZZ4, 3), UBOUND (ZZ4, 3)
    DO JLEV = LBOUND (ZZ4, 2), UBOUND (ZZ4, 2)
      ...
      CALL FIELD_VIEW_GET_NAME_PTR(YLF,LG4RBV(JCOUNT)%NAME)
      IF (ASSOCIATED(IVSET)) THEN
...
FUNCTION LG(YLFL,LGV,IVSET) RESULT(IOFF)
  ...
  IF (LGV_PROVIDED) THEN
    IF (SIZE(LGV) /= IOFF)  CALL ABORT_TRANS("LG FAILURE: LSV has incorrect size")
  ENDIF
...
SUBROUTINE LS2RB(LS2RBV,YLF)
  ...
  DO JLEV = LBOUND (ZZ2, 1), UBOUND (ZZ2, 1)
    ...
    CALL FIELD_VIEW_GET_NAME_PTR(YLF,LS2RBV(JCOUNT)%NAME)
    JCOUNT = JCOUNT + 1
  ENDDO
...
SUBROUTINE LS3RB(LS3RBV,YLF)
  ...
  DO JFLD = LBOUND (ZZ3, 3), UBOUND (ZZ3, 3)
    DO JLEV = LBOUND (ZZ3, 1), UBOUND (ZZ3, 1)
      ...
      CALL FIELD_VIEW_GET_NAME_PTR(YLF,LS3RBV(JCOUNT)%NAME)
      JCOUNT = JCOUNT + 1
    ENDDO
  ENDDO
...
DO JFLD = 1, SIZE (YLFL)
  ...
  SELECT CASE (YLF%RANK)
   ...
   CASE (3)
    ...
    CASE (4)
        CALL ABORT_TRANS("LS not implemeted for FIELD_4RB")
    case DEFAULT
...
SUBROUTINE SET_STRIDES_2RB(ARRAY, STRIDES)
  ...
  ELSE
    CALL ABORT_TRANS("We should not be here, as SIZE(ARRAY) > 1")
  ENDIF
  STRIDE_BYTES = TRANSFER(ADDR2, STRIDE_BYTES) - TRANSFER(ADDR1, STRIDE_BYTES)
  STRIDES(1) = STRIDE_BYTES / (STORAGE_SIZE(ARRAY) / 8)
...
SUBROUTINE MAKE_FIELD_VIEW_SPEC(YLFV, YLF, FIELD_TYPE, LDACC, LDRDONLY)
  ...
  SELECT TYPE (FIELD => YLF%PTR)
    ...
    CLASS DEFAULT
      CALL ABORT_TRANS("MAKE_FIELD_VIEW FAILURE: CLASS UNKNOWN")
  END SELECT
