...
#ifdef define(ACCGPU) || defined (OMPGPU)
  LLACC = .TRUE.
#else
  LLACC = .FALSE.
#endif
...
DO IFIELD=1,NFIELD_UV
  CALL MAKE_FIELD_VIEW(YLFVGU(IFIELD),   YDFU(IFIELD),     LDACC=LLACC, LDRDONLY=.TRUE.)
  CALL MAKE_FIELD_VIEW(YLFVGV(IFIELD),   YDFV(IFIELD),     LDACC=LLACC, LDRDONLY=.TRUE.)
  CALL MAKE_FIELD_VIEW(YLFVSVOR(IFIELD), YDFSPVOR(IFIELD), LDACC=LLACC, LDRDONLY=.FALSE.)
  CALL MAKE_FIELD_VIEW(YLFVSDIV(IFIELD), YDFSPDIV(IFIELD), LDACC=LLACC, LDRDONLY=.FALSE.)
ENDDO
DO IFIELD=1,NFIELD_SCALAR
  CALL MAKE_FIELD_VIEW(YLFVGSCALAR(IFIELD), YDFSCALAR(IFIELD),   LDACC=LLACC, LDRDONLY=.TRUE.)
  CALL MAKE_FIELD_VIEW(YLFVSSCALAR(IFIELD), YDFSPSCALAR(IFIELD), LDACC=LLACC, LDRDONLY=.FALSE.)
ENDDO
