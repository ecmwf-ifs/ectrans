...
!     WRITE(*,'("CREATE_PLAN_FFTW: END: DESTROYING A PLAN AT THE START OF THE LIST")')
  ...
  IF( KTYPE==1 )THEN
    CALL DFFTW_PLAN_MANY_DFT_C2R(IPLAN,IRANK,IN,KLOT,ZDUM,IEMBED,ISTRIDE,ICDIST,&
      & ZDUM,IEMBED,ISTRIDE,IRDIST,FFTW_ESTIMATE+FFTW_UNALIGNED+FFTW_NO_SIMD)
  ELSEIF( KTYPE==-1 )THEN
    CALL DFFTW_PLAN_MANY_DFT_R2C(IPLAN,IRANK,IN,KLOT,ZDUM,IEMBED,ISTRIDE,IRDIST,&
      & ZDUM,IEMBED,ISTRIDE,ICDIST,FFTW_ESTIMATE+FFTW_UNALIGNED+FFTW_NO_SIMD)
  ELSE
...
SUBROUTINE EXEC_FFTW(KTYPE,KRLEN,KCLEN,KOFF,KFIELDS,PREEL)
...
REAL(KIND=JPRB)    :: ZFFT(KCLEN,KFIELDS)
INTEGER(KIND=JPIM) :: JJ,JF
INTEGER(KIND=JPIB) :: IPLAN

CALL CREATE_PLAN_FFTW(IPLAN,KTYPE,KRLEN,KFIELDS)
DO JF=1,KFIELDS
  DO JJ=1,KCLEN
...
ENDDO
IF (KTYPE==1) THEN
  CALL DFFTW_EXECUTE_DFT_C2R(IPLAN,ZFFT,ZFFT)
  IF (KFIELDS /= 1) THEN
    DO JJ=1,KRLEN
      DO JF=1,KFIELDS
  ...
  ELSE
    DO JJ=1,KRLEN
      PREEL(1,KOFF+JJ-1)=ZFFT(JJ,1)
    ENDDO
  ENDIF
ELSEIF (KTYPE==-1) THEN
  CALL DFFTW_EXECUTE_DFT_R2C(IPLAN,ZFFT,ZFFT)
  IF (KFIELDS /= 1) THEN
    DO JJ=1,KRLEN
      DO JF=1,KFIELDS
  ...
  ELSE
    DO JJ=1,KRLEN
      PREEL(1,KOFF+JJ-1)=ZFFT(JJ,1)/REAL(KRLEN,JPRB)
    ENDDO
  ENDIF
ELSE
  CALL ABORT_TRANS('TPM_FFTW:EXEC_FFTW : WRONG VALUE KTYPE')
ENDIF

END SUBROUTINE EXEC_FFTW

SUBROUTINE EXEC_EFFTW(KTYPE,KRLEN,KCLEN,KOFF,KFIELDS,PREEL)

! Support for LAM FFTs on Y direction (no needs for data transposition)
! I am not quite sure this subroutine works. REK
...
ELSEIF (KTYPE==-1) THEN
  ...
  ENDDO
ELSE
  CALL ABORT_TRANS('TPM_FFTW:EXEC_EFFTW : WRONG VALUE KTYPE')
ENDIF
